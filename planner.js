...
function isPlannerPayloadHeavy_(payload){
  ...
  return (gen + pub) >= 25 || drafts >= 80 || gen >= 15;
}
...
async function runPlannerBootstrapInBackground_(){
  try {
    ...
    if(bootstrapData.result === 'success' && bootstrapData.payload){
      reconcileBootstrapLight_(getPersistPayload(), bootstrapData.payload,
        parseInt(bootstrapData.serverRevision, 10) || getPayloadRevision_(bootstrapData.payload));
...
async function runPlannerBootstrapSync_(){
  ...
  state._bootstrapUnlockedEarly = false;

  // 로컬 저장이 이미 크면 오버레이로 잠그지 않음 (JSON 병합이 메인 스레드를 멈춤)
  var localRaw = '';
  try { localRaw = localStorage.getItem(SK) || ''; } catch(eSz){}
  if(localRaw.length > 200000){
    state._bootstrapUnlockedEarly = true;
    unlockPlannerBootstrapLocal_({
      toast: '데이터가 많아 이 기기 기준으로 바로 열었어요. 서버 동기화는 백그라운드에서 진행합니다.'
    });
    plannerSyncBootstrapRunning_ = false;
  ...
  setPlannerBootstrapUi_('loading', '서버를 확인하는 중이에요.\n기다리지 않으려면 「이 기기 데이터로 열기」를 누르세요.');
  var autoUnlock = setTimeout(function(){
    if(plannerSyncBootstrapReady_ || state._bootstrapUnlockedEarly) return;
    state._bootstrapUnlockedEarly = true;
    unlockPlannerBootstrapLocal_({
      toast: '서버 확인이 길어 이 기기 데이터로 먼저 열었어요. 동기화는 백그라운드에서 이어갑니다.'
    });
  }, 2000);
  try {
    var bootstrapData = await verifyPlannerServerReachable_();
    await yieldPlannerBootstrapUi_('서버 응답을 받았습니다. 정리하는 중…');
    ...
    if(bootstrapData.result === 'success' && bootstrapData.payload){
      ...
      if(state._bootstrapUnlockedEarly || isPlannerPayloadHeavy_(remotePayload) || isPlannerPayloadHeavy_(localPayload)){
        reconcileMode = reconcileBootstrapLight_(localPayload, remotePayload, remoteRevision);
      } else {
        await yieldPlannerBootstrapUi_('서버·이 기기 데이터를 맞추는 중…');
        reconcileMode = reconcileLocalWithServerPayload_(localPayload, remotePayload, remoteRevision);
      }
      await yieldPlannerBootstrapUi_('저장하는 중…');
    } else if(bootstrapData.result === 'empty'){
    ...
    if(reconcileMode !== 'conflict' && reconcileMode !== 'deferred-heavy' && !hasPendingLocalSyncChanges_()){
      state.syncBaseEntityUpdatedAt = Object.assign({}, state.syncEntityUpdatedAt || {});
      state.syncDirtyEntityKeys = {};
      state.syncNeedsSnapshot = false;
      state.syncOutbox = [];
    ...
    plannerSyncBootstrapReady_ = true;
    if(bootstrapData.result === 'empty' && !state._bootstrapUnlockedEarly){
      await yieldPlannerBootstrapUi_('빈 서버에 최초 데이터를 올리는 중…');
    ...
    if(!state._bootstrapUnlockedEarly){
      setPlannerBootstrapUi_('done', '');
      if(reconcileMode === 'adopted' && typeof setAppToast === 'function'){
        setAppToast('서버 최신 데이터로 맞췄어요.', { duration: 3200, variant: 'ok' });
      } else if(reconcileMode === 'deferred-heavy' && typeof setAppToast === 'function'){
        setAppToast('데이터가 많아 이 기기 기준으로 열었어요. 필요하면 「지금 동기화」를 눌러 주세요.', { duration: 5200, variant: 'ok' });
      }
    }
    ...
    if(state._bootstrapUnlockedEarly || plannerSyncBootstrapReady_){
      if(typeof setAppToast === 'function') setAppToast('서버 확인 실패: ' + msg + '\n이 기기 데이터로 계속 편집할 수 있어요.', { duration: 6500, variant: 'err' });
    } else {
      plannerSyncBootstrapReady_ = false;
      setPlannerBootstrapUi_('failed', '서버 동기화가 완료되지 않았어요.\n' + msg + '\n\n「이 기기 데이터로 열기」로 편집을 시작할 수 있어요.');
    }
  } finally {
...
function openPlannerConflict_(local, remote, serverRevision){
  ...
  // 충돌이 많거나 데이터가 크면 모달 대신 이 기기 우선 자동 적용 (버튼 무반응·멈춤 방지)
  // true + pending=null → 호출측은 'merged'/autoResolved 로 처리 (false면 후속 merge로 로컬 유지가 덮일 수 있음)
  if(conflicts.length > 10 || isPlannerPayloadHeavy_(local) || isPlannerPayloadHeavy_(remote)){
    autoResolveConflictsKeepLocal_(local, remote, serverRevision, conflicts);
    return true;
  }
...
/** 대량 충돌: UI 없이 로컬(특히 생성 초안) 우선으로 맞추고 스냅샷 푸시 예약 */
function autoResolveConflictsKeepLocal_(local, remote, serverRevision, conflicts){
  try {
    var merged = mergePlannerPayloads_(local || getPersistPayload(), remote || {});
    ...
    (conflicts || []).forEach(function(conflict){
      ...
      var useLocal = true;
      if(key.indexOf('generated:') === 0 || key.indexOf('published:') === 0){
        useLocal = hasUsableAiDraftContent_(unwrapSyncEntityValue_(conflict.local)) ||
          !hasUsableAiDraftContent_(unwrapSyncEntityValue_(conflict.remote));
      }
      setSyncEntity_(merged, key, useLocal ? conflict.local : conflict.remote);
      times[key] = useLocal ? new Date().toISOString() : (conflict.remoteAt || new Date().toISOString());
      if(useLocal){
    ...
    merged.syncRevision = parseInt(serverRevision, 10) || getPayloadRevision_(remote);
    applyPersistPayload(merged, { skipRestorePending: true, deferHeavyRepair: true });
    state.syncRevision = merged.syncRevision;
    state.syncBaseEntityUpdatedAt = Object.assign({}, (remote && remote.syncEntityUpdatedAt) || {});
    state.syncDirty = true;
    state.syncNeedsSnapshot = true;
    // outbox 기준으로 dirty 키를 다시 세우되, 스냅샷 플래그는 유지 (로컬 유지분 업로드)
    rebuildDirtyStateFromOutbox_();
    state.syncNeedsSnapshot = true;
    state.syncDirty = true;
    ...
    save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true });
    if(typeof setAppToast === 'function'){
      setAppToast('충돌 ' + (conflicts || []).length + '건을 이 기기 우선으로 자동 맞췄어요' +
        (kept ? ' (로컬 유지 ' + kept + '건)' : '') + '.\n서버 반영은 백그라운드에서 이어갑니다.', { duration: 6000, variant: 'ok' });
    }
    schedulePlannerGasPush_(true);
    setTimeout(function(){ try { runDeferredHeavyPlanRepairs_(); } catch(e){} }, 50);
  } catch(err){
    console.warn('[충돌 자동해결]', err);
...
window.applyAllConflictsKeepLocal_ = function(){
  ...
  plannerSyncConflictPending_ = null;
  setTimeout(function(){
    autoResolveConflictsKeepLocal_(snap.local, snap.remote, snap.serverRevision, snap.conflicts || []);
...
function openPlannerOutboxConflict_(serverConflicts, remote, serverRevision){
  ...
  if(conflicts.length > 10 || isPlannerPayloadHeavy_(localPayload) || isPlannerPayloadHeavy_(remote)){
    autoResolveConflictsKeepLocal_(localPayload, remote, serverRevision, conflicts);
...
window.applyPlannerConflictChoices_ = async function(){
  ...
  var applyBtn = document.getElementById('sync-conflict-apply');
  // 이전에 멈춘 '적용 중' 상태여도 다시 누를 수 있게 강제 활성화
  if(applyBtn){
    applyBtn.disabled = true;
    applyBtn.textContent = '적용 중…';
  }
  // 클릭 반응을 바로 보여 주기 위해 모달을 먼저 닫음
  var overlayEarly = document.getElementById('sync-conflict-overlay');
  if(overlayEarly) overlayEarly.classList.remove('open');
  if(typeof setAppToast === 'function') setAppToast('선택 내용을 적용하는 중…', { duration: 2500, variant: 'ok' });
  await new Promise(function(resolve){ setTimeout(resolve, 0); });
  try {
    if(pending._forceAllLocal){
      autoResolveConflictsKeepLocal_(pending.local, pending.remote, pending.serverRevision, pending.conflicts || []);
      return;
    }
    if(pending.mode === 'outbox'){
      ...
      pending.conflicts.forEach(function(conflict, i){
        ...
        // 목록에 안 보이는(>12) 항목·라디오 없음 → 이 기기 유지 (서버로 조용히 덮지 않음)
      ...
      if(!plannerSyncBootstrapReady_) plannerSyncBootstrapReady_ = true;
      save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true });
      setTimeout(function(){ try { runDeferredHeavyPlanRepairs_(); } catch(eDef){} }, 50);
      if(state.syncDirty){
        if(applyBtn) applyBtn.textContent = '서버 저장 중…';
        var outboxResult = await plannerGasPushNow_();
        if(!outboxResult || !outboxResult.ok){
          if(typeof setAppToast === 'function') setAppToast(
            outboxResult && outboxResult.skipped
              ? '선택은 이 기기에 저장했어요. 서버 동기화는 아직 준비되지 않아 나중에 다시 시도합니다.'
              : '선택 내용은 이 기기에 보관했고 서버 저장을 다시 시도합니다.',
            { duration: 5200, variant: 'err' }
          );
          return;
        }
      }
    ...
    if(pending.dirtyGeneration != null && pending.dirtyGeneration !== plannerDirtyGeneration_){
      plannerSyncConflictPending_ = null;
      var reopened = openPlannerConflict_(getPersistPayload(), pending.remote, pending.serverRevision);
      // 충돌 UI가 다시 안 열린 경우(없음·자동해결)만 오버레이 닫고 푸시
      if(!reopened || !plannerSyncConflictPending_){
      ...
      if(typeof setAppToast === 'function') setAppToast(
        plannerSyncConflictPending_
          ? '충돌 확인 중 새 편집이 감지되어 최신 내용으로 다시 확인합니다.'
          : '충돌을 최신 편집 기준으로 맞췄어요. 서버 반영을 이어갑니다.',
        { duration: 4800, variant: plannerSyncConflictPending_ ? 'err' : 'ok' }
      );
      return;
    }
    ...
    pending.conflicts.forEach(function(conflict, i){
      ...
      if(useLocal){
        ...
        var rk = key;
        if(rk.indexOf('deleted:') === 0) clearDeletedIds2.push(rk.slice('deleted:'.length));
        if(rk.indexOf('draft:') === 0 && conflict.remote !== undefined) clearDeletedIds2.push(rk.slice('draft:'.length));
      }
    });
    merged.syncEntityUpdatedAt = times;
    ...
    applyPersistPayload(merged, {
      skipRestorePending: true,
      clearDeletedDraftIds: clearDeletedIds2,
      deferHeavyRepair: true
    });
    state.syncRevision = pending.serverRevision;
    ...
    if(!plannerSyncBootstrapReady_) plannerSyncBootstrapReady_ = true;
    save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true });
    setTimeout(function(){ try { runDeferredHeavyPlanRepairs_(); } catch(eDef2){} }, 50);
    if(state.syncDirty){
      if(applyBtn) applyBtn.textContent = '서버 저장 중…';
      ...
      if(!result || !result.ok){
        if(typeof setAppToast === 'function') setAppToast(
          result && result.skipped
            ? '선택은 이 기기에 저장했어요. 서버 동기화는 아직 준비되지 않아 나중에 다시 시도합니다.'
            : '선택 내용은 이 기기에 저장됐지만 서버 저장은 완료되지 않았어요.\n자동으로 다시 시도합니다.',
          { duration: 5600, variant: 'err' }
        );
        return;
      }
    }
    ...
    // 모달을 먼저 닫은 뒤 실패하면 pending이 남아 동기화가 영구 차단됨 → 복구
    if(plannerSyncConflictPending_ === pending){
    ...
    if(typeof setAppToast === 'function'){
      setAppToast('충돌 적용 중 오류가 났어요.\n' + ((err && err.message) || err) +
        '\n선택 창을 다시 열었어요. 「전부 이 기기로 적용」을 권장합니다.', { duration: 7000, variant: 'err' });
    }
  } finally {
...
function applyPersistPayload(s, opts){
  ...
  // 마이그레이션마다 save 하지 않고 한 번만 기록 (대용량 초안에서 멈춤 방지)
  if(migrated){
    if(opts && opts.authoritativeAdopt) state._postAdoptMigration = true;
    else {
      // skipMarkDirty 경로라서 마이그레이션 반영분은 스냅샷으로 올리도록 표시
      // save 전에 플래그를 세워 디스크(_syncDirty)에도 남긴다
      state.syncDirty = true;
      state.syncNeedsSnapshot = true;
    }
