...
function buildOpsDeployFiles_(){
  return '배포 대상(확인 후 업로드):\n' +
    '- planner.js / 플래너.html / planner-sw.js\n' +
    '- 프로필.html\n' +
    '- 강사용.html (해당 시)';
}
...
function ensureUserAddedDraftsInMisc_(catId){
  var cat = CATEGORIES[catId];
  if(!cat || !getSubGoalPlan_(catId)) return;
  var toMigrate = (cat.drafts || []).filter(function(d){
    return d && isUserAddedDraftId_(d.id) && String(d.roadmapStepId || '') !== SUBGOAL_MISC_ID;
  });
...
function collectSyncEntities_(payload){
  ...
  if(payload.opsManual && (parseInt(state.syncEntitySchemaVersion, 10) || 1) < 2){
    ...
    ['collapsed','review','newBranchMeta'].forEach(function(field){
  ...
  if(branding.yearPlan){
    ...
    (branding.yearPlan.periods || []).forEach(function(period, i){
      out['plan:year:period:' + i] = { index: i, value: period };
    });
  }
  if(branding.dailySharePlan){
    ...
    (branding.dailySharePlan.themes || []).forEach(function(theme, i){
      out['plan:daily:theme:' + i] = { index: i, value: theme };
    });
  }
  Object.keys(branding.subGoalPlans || {}).forEach(function(catId){
    ...
    (plan.steps || []).forEach(function(step, i){
      out['plan:subgoal:' + catId + ':step:' + i] = { index: i, value: step };
    });
  });
  ...
  if(parts[0] === 'plan'){
    ...
    if(parts[1] === 'year'){
      ...
      if(parts[2] === 'meta'){
        ...
        var yearIndex = parseInt(parts[3], 10) || 0;
        if(missing) periods.splice(yearIndex, 1);
        else periods[yearIndex] = cloneSyncValue_(wrapped && wrapped.value);
      ...
      if(parts[2] === 'meta'){
        ...
        var themeIndex = parseInt(parts[3], 10) || 0;
        if(missing) themes.splice(themeIndex, 1);
        else themes[themeIndex] = cloneSyncValue_(wrapped && wrapped.value);
      ...
      if(parts[3] === 'meta'){
        ...
        var stepIndex = parseInt(parts[4], 10) || 0;
        if(missing) steps.splice(stepIndex, 1);
        else steps[stepIndex] = cloneSyncValue_(wrapped && wrapped.value);
...
function isValidClientSyncEntityKey_(key){
  ...
  return key === 'prompt:base' ||
    /^prompt:\d+:[^:]+$/.test(key) ||
    /^plan:year:(?:meta|period:\d+)$/.test(key) ||
    /^plan:daily:(?:meta|theme:\d+)$/.test(key) ||
    /^plan:subgoal:\d+:(?:meta|step:\d+)$/.test(key) ||
    /^branding:(?:field|categoryGoal|seriesGoal):.+$/.test(key) ||
    /^setting:.+$/.test(key) ||
    /^(?:milestone|deleted|pinned|override|draft|published|generated):.+$/.test(key) ||
    key === 'ops:root' ||
    key === 'ops:meta' ||
    /^ops:(?:checked|note|collapsed|review|newBranchMeta):.+$/.test(key);
}
...
function buildPendingSyncMutations_(payload){
  ...
  return (state.syncOutbox || []).slice(0, 200).map(function(op){
    return {
...
function updateSyncStatusUI_(){
  ...
  if(sub){
    ...
    else if(info.overall === 'warn') sub.textContent = info.retryScheduled ? '저장 실패 · 재시도' : '서버 확인 필요';
...
function renderSyncStatusBodyHTML_(info){
  ...
  html += row(
    ...
    'revision <strong>' + info.localRev + '</strong> · 직원 <strong>' + escapeHtml(info.employeeId || '(미설정)') + '</strong> · 마지막 저장 <strong>' + escapeHtml(localFmt) + '</strong>'
  );
  if(info.serverOn){
  ...
  if(hasDriveConnection_()){
    ...
    html += row(PLANNER_TEAM_MODE ? 'Google Drive (개인 백업)' : 'Google Drive', dBadge, dText, dMeta);
  } else {
  ...
  html += '<div class="sync-status-row-meta" style="padding:0 2px;color:#9CA3AF;">revision은 서버가 동기화 성공 때마다 발급합니다. 로컬 변경은 업로드 대기로 별도 표시됩니다.</div>';
  return html;
}
...
async function runPlannerBootstrapSync_(){
  ...
  setPlannerBootstrapUi_('loading', '최신 데이터를 먼저 받은 뒤 편집을 시작합니다.');
  try {
    ...
    if(bootstrapData.result === 'success' && bootstrapData.payload){
      ...
      ensureOutboxFromLegacyDirty_(localPayload);
      ...
      if(state.syncDirty && remoteRevision !== (parseInt(state.syncRevision, 10) || 0)){
        if((state.syncOutbox || []).length) applyServerPayloadPreservingOutbox_(remotePayload, remoteRevision);
        else reconcileRemoteBeforeWrite_(localPayload, remotePayload, remoteRevision);
      } else {
        var mergedPayload = mergePlannerPayloads_(localPayload, remotePayload);
        applyPersistPayload(mergedPayload, { skipRestorePending: preferRemotePayload_(localPayload, remotePayload) });
        state.syncRevision = remoteRevision;
        state.syncEntityRevisions = ensureSyncEntityRevisions_(remotePayload);
        state.syncEntityTombstones = Object.assign({}, remotePayload.syncEntityTombstones || {});
        state.syncBaseEntityUpdatedAt = Object.assign({}, mergedPayload.syncEntityUpdatedAt || {});
      }
    ...
    if(!save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true })){
...
window.applyPlannerConflictChoices_ = async function(){
  ...
  if(pending.mode === 'outbox'){
    ...
    var outboxMerged = overlayOutboxOnPayload_(pending.remote || {});
    applyPersistPayload(outboxMerged, { skipRestorePending: true });
  ...
  pending.conflicts.forEach(function(conflict, i){
    ...
    else delete state.syncDirtyEntityKeys[conflict.key];
  });
...
function reconcileRemoteBeforeWrite_(local, remote, serverRevision){
  var remoteRev = parseInt(serverRevision, 10) || getPayloadRevision_(remote);
  if(state.syncDirty && remoteRev !== (parseInt(state.syncRevision, 10) || 0)){
    if(openPlannerConflict_(local, remote, remoteRev)) return false;
  }
  var merged = mergePlannerPayloads_(local, remote);
  applyPersistPayload(merged, { skipRestorePending: true });
  state.syncRevision = remoteRev;
  if(state.syncDirty){
    state.syncBaseEntityUpdatedAt = Object.assign({}, remote.syncEntityUpdatedAt || {});
  } else {
    state.syncBaseEntityUpdatedAt = Object.assign({}, merged.syncEntityUpdatedAt || {});
  }
  save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true });
  return true;
}
...
async function syncAllSourcesIfNewerCore_(reason){
  ...
  if(state.syncDirty && remoteRevision !== (parseInt(state.syncRevision, 10) || 0)){
    ...
    var reconciled = reconcileRemoteBeforeWrite_(localPayload, remote, remoteRevision);
  ...
  state.syncEntityRevisions = ensureSyncEntityRevisions_(remote);
  state.syncEntityTombstones = Object.assign({}, remote.syncEntityTombstones || {});
  state.syncBaseEntityUpdatedAt = Object.assign({}, wasDirty ? (remote.syncEntityUpdatedAt || {}) : (merged.syncEntityUpdatedAt || {}));
  state.syncDirty = wasDirty;
  try {
  ...
  updateSyncStatusUI_();
  if(wasDirty && plannerSyncBootstrapReady_) schedulePlannerGasPush_(true);
...
function applyPersistPayload(s, opts){
  ...
  if(migrateUniversalContentFlowPrompts_()) save({ skipDriveUpload: true, skipGasPush: true });
}
...
function save(opts) {
  ...
  var changedEntities = o.skipEntityStamp ? [] : stampChangedSyncEntities_(previous || {}, beforeStamp);
...
function updateApiBadge() {
  document.getElementById('api-dot').className = 'api-dot' + (isPlannerAiAvailable_() ? ' on' : '');
  var label = 'API 키 설정';
...
function normalizeFlowProposal_(raw, idx){
  ...
  return {
    id: idx,
    title: String((raw && (raw.title || raw.topic)) || '').trim(),
    angle: String((raw && raw.angle) || '').trim(),
...
async function generateArticleFlowProposals_(catId, keywords, imagePayload, sourceNote, youtubeAnalysis, prevFlows){
  ...
  if(sourceNote && String(sourceNote).trim()){
    noteBlock = '\n[참고 메모 — 영상·테크닉·링크 등]\n' + String(sourceNote).trim();
    if(!youtubeAnalysis) noteBlock += '\n(AI는 영상 파일을 직접 볼 수 없습니다. 메모·링크 설명을 바탕으로 제안하세요.)';
  }
  if(youtubeAnalysis && String(youtubeAnalysis).trim()){
    noteBlock += '\n\n[YouTube 영상 분석 — Gemini]\n' + String(youtubeAnalysis).trim();
  }
  var kwLine = keywords
    ? ('입력 (키워드·아이디어·문장): ' + keywords)
    : (images.length ? '입력 (키워드·아이디어·문장): (없음 — 사진·메모만 참고)' : (sourceNote ? '입력: (없음 — 참고 메모만)' : '입력: '));
...
'각 흐름에 **참고영상 메모, 키워드·아이디어, 선택한 카테고리의 컨셉·독자·글 형식**을 빠짐없이 함께 반영하세요.\n' +
...
'JSON만 출력:\n{"flows":[{"title":"...","angle":"...","steps":["...","...","..."]},{"title":"...","angle":"...","steps":["...","...","..."]},{"title":"...","angle":"...","steps":["...","...","..."]}]}';
  var text = await callClaudePlanner_(prompt, { image: images.length ? images : null, maxTokens: 2200 });
  var raw = text.replace(/^```json\s*/i, '').replace(/```\s*$/g, '').trim();
  var start = raw.indexOf('{');
  var end = raw.lastIndexOf('}');
  if(start < 0 || end <= start) throw new Error('AI 응답에서 글 흐름 JSON을 찾지 못했어요');
  var obj = JSON.parse(raw.slice(start, end + 1));
  var flows = Array.isArray(obj.flows) ? obj.flows : [];
  if(flows.length < 3) throw new Error('글 흐름 제안이 3개가 아니에요. 다시 시도해 주세요.');
  return flows.slice(0, 3).map(function(f, i){ return normalizeFlowProposal_(f, i); });
}
...
window.onNewItemImage = async function(input){
  ...
  try {
    ...
    state.newItem.imageAnalyzing = false;
    renderMain();
    if(state.apiKey && state.newItem.refImages && state.newItem.refImages.length && typeof setAppToast === 'function'){
...
function renderAddForm(){
  ...
  return `<div class="add-form-layout"><div class="add-form">
    ...
    <div class="form-field">
      ...
      <textarea id="new-item-ref-note-input" class="form-input form-textarea${hasPhoto ? ' image-analysis-textarea' : ''}" rows="${hasPhoto ? '9' : '5'}" oninput="state.newItem.refNote=this.value" placeholder="${refNotePlaceholder}" ${analyzing ? 'disabled' : ''}>${escapeHtml(state.newItem.refNote || '')}</textarea>
