...
function buildImageGptVisualsJsonExample_(catId, opts){
  ...
  var mangoBriefEx =
    ...
    '"photoTip":"실사·시연 참고 사진 1장 이상(셀프케어 전신 컷 권장).",\n' +
    '"mangoInputPaste":"intro와 동일(끝에 [디자인] 흑·백·얇은 빨강/네이비·초록금지 포함)"\n' +
    '}';
  return `"images": {\n"gptVisuals": [],\n${mangoBriefEx}\n}`;
}
...
- 큰 대전제(한 줄 훅)를 먼저 두고, 그 아래만 짧게. (표지형·원 포인트 슬라이드 환영 — 「순서가 만드는 움직임」급)
...
2) intro (=「소개」): **반드시 채움**, **최대 ${MANGO_DETAIL_INTRO_MAX}자**. 망고보드 소개란용 **쉬운 말** 기획. 빈 값 금지.
3) mangoInputPaste = intro와 **동일**.
4) intro 본문은 아래 흐름(7장) + **맨 끝 [디자인]·[레이아웃] 블록**.

intro 권장 흐름 (쉬운 말로, 표 없이):
1. 표지 — 큰 대전제 한 줄 훅 (질문형 OK)
2. 공감 — 일상에서 겪는 불편 (전문용어 금지)
3. 누구를 위한지 — 한 줄
4. 셀프 확인·따라 하기 — 동작·초·회를 짧은 STEP/안내 카드로 (표 금지)
5. 원리 — 「왜 그런지」를 2~3문장, 쉬운 비유
6. 주의 — 병원 가야 할 때 한 줄 + 무리 금지
7. FAQ 1~2 + CTA (상담·프로필, 예약 압박 금지)

intro 본문(표지~FAQ)은 아래를 반영해 쉬운 말로:
...
- slidePlan: 정확히 7개 — 각 장은 한 메시지, 표 없음
...
function ensureMangoIntroHasDesignBlock_(intro){
  ...
  var hasLayout = /\[레이아웃\]/.test(body) && /한발서기|중년 남성|민트 Good|참고사진/.test(body);
...
function buildMangoBriefFromBlog_(content, topic){
  ...
  var lines = [];
  lines.push('1. 표지·훅 (큰 글씨 한 줄)');
  lines.push(trimMangoSection_(title || topic || hook || '유연한데 왜 자꾸 삐끗할까요?', 120));
  lines.push('');
  lines.push('2. 공감 (쉬운 말)');
  lines.push(trimMangoSection_(problem || '스트레칭은 잘하는데, 특정 동작에서 힘이 빠지거나 삐끗한 느낌이 반복돼요.', 320));
  lines.push('');
  lines.push('3. 이런 분께');
  lines.push(trimMangoSection_(
    '몸이 잘 늘어나는데도 자주 삐끗하거나, 움직임이 불안정하게 느껴지는 분',
    160
  ));
  lines.push('');
  lines.push('4. 따라 하기 (실제 동작·초·회)');
  lines.push(trimMangoSection_(selfCare || '무리해서 늘리지 말고, 지금 할 수 있는 범위에서 10초만 정확히 버텨 보세요. 흔들리면 멈추세요.', 420));
  lines.push('');
  lines.push('5. 왜 그런지 (쉬운 말, 표 없이)');
  lines.push(trimMangoSection_(explanation || '범위만 키우면 버티는 힘이 따라가지 못할 수 있어요. 먼저 조절을 만든 뒤 범위를 넓히는 순서가 안전합니다.', 420));
  lines.push('');
  lines.push('6. 주의');
  lines.push('갑자기 아파지거나 저림·마비가 커지면 따라 하지 말고 병원 진료를 먼저 받으세요.');
  lines.push('');
  lines.push('7. FAQ·다음');
  lines.push(trimMangoSection_(cta || '궁금하면 프로필·상담으로 문의해 주세요. 일반 정보이며 치료·진단이 아닙니다.', 280));
  lines.push('');
  lines.push('슬라이드 힌트: 표 금지 · 주의·안내형 · 픽토그램 OK · 전/후는 일상 동작(한발서기만 X) · 여성 모델');
  var intro = ensureMangoIntroHasDesignBlock_(lines.join('\n').replace(/\n{3,}/g, '\n\n').trim());
  return {
...
function renderMangoDetailInputCard_(brief){
  ...
  html += '<p style="font-size:12px;color:#6B7280;margin:0 0 10px;line-height:1.55;">주황 버튼으로 <strong>제목+소개</strong> 복사 → 망고보드 소개란에 붙여넣고 첫 줄만 제품명으로. 결과물 기준: <strong>흑·백+얇은 빨강</strong>(민트·Good 뱃지 금지), 표 대신 주의·안내형, 쉬운 말, 여성 모델·일상 동작 전/후(한발서기만 반복 X), 픽토그램 OK.</p>';
...
<div class="prompt-section">
  ...
  <div class="prompt-hint">망고보드 상세페이지: 제품명 30자 · 소개 2000자(7장 흐름). GPT 영문 이미지 프롬프트는 사용하지 않습니다.</div>
</div>` : ''}
