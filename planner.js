...
function pruneTopicsWithoutDraftCore_(catId, opts){
  ...
  var saveResult = { ok: true, stripped: 0 };
  if(!opts.skipSave){
    saveResult = persistPlannerUnderQuota_({
      skipPush: !!opts.skipUpload,
      onlyCatId: catId,
      allowAllTabs: true
    });
  }
  ...
  return {
    ...
    kept: kept.length,
    saved: !!(saveResult && saveResult.ok),
    stripped: (saveResult && saveResult.stripped) || 0
  };
}

window.pruneTopicsWithoutDraft_ = function(catId){
  if(state._pruneTopicsBusy) return;
  ...
  try {
    ...
    if(bytes > 4 * 1024 * 1024){
      quotaHint = '\n\n※ 저장 공간이 거의 가득 차 있어, 이 프로그램의 미발행 AI 초안 본문을 먼저 비워 저장합니다.\n(다른 탭 초안은 가능하면 유지하고, 그래도 안 될 때만 추가로 정리합니다.)';
    }
  } catch(eQ){}
  if(!confirm(name + '에서 초안·발행 본문이 없는 주제 ' + n + '개를 삭제할까요?\n(초안 있는 주제·발행 완료 주제는 그대로 둡니다)' + quotaHint)) return;

  // 모바일: confirm 확인 좌표의 ghost click 이 버튼을 다시 눌러 창이 반복되는 것 방지
  ...
  // 한도 FAB 은 disabled 로 죽이지 않음 (다시 안 켜져서 눌러도 무반응이 됨)
  setQuotaFabBusy_(true);
  if(typeof setAppToast === 'function') setAppToast('주제 삭제·저장 정리 중… 잠시만 기다려 주세요.', { duration: 5000, variant: 'ok' });
  ...
  setTimeout(function(){
    ...
    try {
      ...
      if(typeof setAppToast === 'function'){
        setAppToast('정리 중 오류가 났어요. 하단 「초안 본문 비우고 저장」을 눌러 주세요.', { duration: 7000, variant: 'err' });
      }
      state._pruneTopicsBusy = false;
      document.querySelectorAll('.program-prune-banner button').forEach(function(btn){
        try { btn.disabled = false; } catch(eB2){}
      });
      setQuotaFabBusy_(false);
      ensureQuotaRecoveryFab_();
      return;
    ...
    state._pruneTopicsBusy = false;
    document.querySelectorAll('.program-prune-banner button').forEach(function(btn){
      try { btn.disabled = false; } catch(eB3){}
    });
    if(typeof setAppToast === 'function'){
      if(result.saved){
        setAppToast(
          '초안 없는 주제 ' + result.removed + '개를 삭제했어요. 남은 주제 ' + result.kept + '개.' +
          (result.stripped ? '\n저장 한도 때문에 이 프로그램 미발행 초안 본문도 정리했습니다.' : ''),
          { duration: 6500, variant: 'ok' }
        );
        setQuotaFabBusy_(false);
        if(!plannerStorageWriteFailed_) removeQuotaRecoveryFab_();
      } else {
        setAppToast(
          '주제 ' + result.removed + '개는 화면에서 지웠지만, 브라우저 저장 한도 때문에 디스크에 못 남겼어요.\n아래 「초안 본문 비우고 저장」을 눌러 주세요.',
