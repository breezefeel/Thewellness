...
const MANGO_DESIGN_TONE_LINE =
  '색: 흑·백·차콜 미니멀. 포인트는 빨강(또는 네이비)만 얇게. ' +
  '민트·세이지·초록·티얼·연두 배경·민트 Good 뱃지·알록달록·만화·코믹 금지. 로고는 후처리.';
...
- 각 장 카피: 1헤드 + 1~2문장 또는 불릿 3개 이내. 민트·초록 배경·Good 민트 뱃지 남발 금지.
...
const MANGO_LAYOUT_PASTE_BLOCK =
  ...
  '흑·백+얇은 빨강. 초록·민트 배경·민트 Good 뱃지 금지.\n' +
  '참고사진: 여성·전신·일상 동작 컷.';
...
- intro **맨 끝**에 [디자인]+[레이아웃] (흑·백·얇은 빨강, 민트 Good 뱃지 금지, 표 금지, 쉬운 말, 여성 모델, 한발서기만 X·일상 전/후, 초록 금지)
...
function ensureMangoIntroHasDesignBlock_(intro){
  ...
  var hasLayout = /\[레이아웃\]/.test(body) && /말줄임|장마다 헤드|한발서기|중년 남성|민트 Good|참고사진/.test(body);
  var hasDesign = /\[디자인\]/.test(body) && /흑[·.\s]*백/.test(body) && /민트 Good|연두 배경|빨강/.test(body);
...
function buildMangoBriefFromBlog_(content, topic){
  ...
  var blocks = [
    ...
    mangoFormatSlideBlock_(6, '주의', '이땐 따라 하지 마세요', cautionBullets, '주의 안내 카드(빨강 포인트)'),
...
function fillMangoBriefFromContent_(content, opts){
  ...
  // 복사·망고보드용: 소개 끝에 흑·백·빨간 포인트 디자인 블록 보장
...
function renderMangoDetailInputCard_(brief){
  ...
  html += '<p style="font-size:12px;color:#6B7280;margin:0 0 10px;line-height:1.55;">주황 버튼으로 <strong>제목+소개</strong> 복사 → 망고보드 소개란에 붙여넣고 첫 줄만 제품명으로. 소개는 <strong>장별 헤드+짧은 불릿</strong>(블로그 원문·… 붙여넣기 X). 결과물: <strong>흑·백+얇은 빨강</strong>, 주의·안내형, 여성 모델·일상 전/후, 픽토그램 OK.</p>';
...
function buildImageTabBody(content){
  ...
  html += '<p class="empty-note" style="padding:8px 0 0;font-size:11px;color:#9CA3AF;line-height:1.55;">수정 후 <strong>발행완료</strong>를 누르면 저장돼요. 참고 사진은 실사·시연 컷을 올리면 되고, 로고·흑백 타이포는 망고보드에서 올립니다.</p>';
