...
const MANGO_DESIGN_TONE_LINE =
  '색: 배경 #eddcd2(따뜻한 베이지·살구). ' +
  '글·헤드 #5c4033~#4a3728 브라운. 카드·박스 #f7efe9·#f5ebe4. ' +
  '포인트·구분선 #c4785a(테라코타)·#b07a62. ' +
...
- 색은 #eddcd2 계열(웜 베이지·브라운·테라코타). 흑백 모노·민트·초록 배경 금지.
...
const MANGO_LAYOUT_PASTE_BLOCK =
  ...
  '색: 배경 #eddcd2 · 글 브라운 · 포인트 테라코타(#c4785a). 흑백 모노·민트·초록 금지.\n' +
  '참고사진: 여성·전신·일상 동작 컷.';
...
- intro **맨 끝**에 [디자인]+[레이아웃] (배경 #eddcd2·브라운·테라코타, 흑백 모노·민트 금지, 표 금지, 쉬운 말, 여성 모델, 한발서기만 X·일상 전/후)
...
function ensureMangoIntroHasDesignBlock_(intro){
  ...
  var hasLayout = /\[레이아웃\]/.test(body) && /#eddcd2|테라코타|말줄임|장마다 헤드|한발서기|중년 남성|참고사진/.test(body);
  var hasDesign = /\[디자인\]/.test(body) && /#eddcd2/.test(body) && /(테라코타|브라운|베이지)/.test(body);
  if(hasDesign && hasLayout) return body;
  var stripped = body
    .replace(/\n*\[디자인\][\s\S]*?(?=\n*\[레이아웃\]|$)/i, '')
    .replace(/\n*\[레이아웃\][\s\S]*$/i, '')
    .replace(/\n*흑[·.\s]*백[·.\s]*미니멀[^\n]*/g, '')
...
function buildMangoBriefFromBlog_(content, topic){
  ...
  var blocks = [
    ...
    mangoFormatSlideBlock_(6, '주의', '이땐 따라 하지 마세요', cautionBullets, '주의 안내 카드(테라코타 포인트)'),
...
function fillMangoBriefFromContent_(content, opts){
  ...
  // 복사·망고보드용: 소개 끝에 #eddcd2 웜톤 디자인 블록 보장
...
function renderMangoDetailInputCard_(brief){
  ...
  html += '<p style="font-size:12px;color:#6B7280;margin:0 0 10px;line-height:1.55;">주황 버튼으로 <strong>제목+소개</strong> 복사 → 망고보드 소개란에 붙여넣고 첫 줄만 제품명으로. 소개는 <strong>장별 헤드+짧은 불릿</strong>(블로그 원문·… 붙여넣기 X). 결과물: 배경 <strong>#eddcd2</strong>·브라운·테라코타(흑백 모노 X), 주의·안내형, 여성 모델·일상 전/후, 픽토그램 OK.</p>';
