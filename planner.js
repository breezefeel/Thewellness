...
function buildImageGptVisualsJsonExample_(catId, opts){
  ...
  if(id === 7){
    return `"images": {
"gptVisuals": [
{"role":"problem","title":"① 공감·표지","blogSlot":"problem","mangoUse":"표지·도입","aspect":"1:1","prompt":"영문. 단지·일상 공감 장면. 글자·로고 없음."},
{"role":"selfCare","title":"② 셀프 동작","blogSlot":"selfCare","mangoUse":"동작 안내","aspect":"1:1","prompt":"영문. community.selfCare와 동일 동작. 글자·로고 없음."}
],
"mangoBrief": null
}`;
  }
  var slidePlan7 =
    '[{"slide":1,"role":"표지","copyHint":"훅 한 줄"},{"slide":2,"role":"문제 공감","copyHint":"일상 불편"},{"slide":3,"role":"대상","copyHint":"누구에게"},{"slide":4,"role":"셀프케어","copyHint":"동작·초·회"},{"slide":5,"role":"원리","copyHint":"왜 도움이 되는지"},{"slide":6,"role":"주의·FAQ","copyHint":"병원 신호·질문"},{"slide":7,"role":"CTA","copyHint":"상담·다음 행동"}]';
  var mangoBriefEx =
    '"mangoBrief": {\n' +
    '"productName":"주제 최대30자",\n' +
    '"intro":"소개란용 본문. 7장 흐름이 보이게. 최대2000자",\n' +
    '"features":["특장점1","특장점2","특장점3"],\n' +
    '"slidePlan":' + slidePlan7 + ',\n' +
    '"designTone":"흑·백·미니멀·전문가. 만화·코믹 금지. 로고는 후처리.",\n' +
    '"photoTip":"망고보드에 올릴 참고 사진: ② 셀프케어(시연) 전신 컷 1장 이상. 잘리지 않게.",\n' +
    '"mangoInputPaste":"intro와 동일 문자열(소개란 붙여넣기, 2000자 이내)"\n' +
    '}';
  if(opts.expert){
    return `"images": {
"gptVisuals": [
{"role":"problem","title":"① 맥락·도입","blogSlot":"hook","mangoUse":"표지","aspect":"1:1","placementNote":"블로그 hook","prompt":"영문. blog.hook 장면. 글자·로고 없음."},
{"role":"selfCare","title":"② 시연·테크닉","blogSlot":"outline","mangoUse":"참고사진1순위","aspect":"1:1","placementNote":"망고 업로드 최우선","prompt":"영문. outline·draft 시연과 일치. 글자·로고 없음."},
{"role":"explanation","title":"③ 원리·디테일","blogSlot":"draft","mangoUse":"원리","aspect":"1:1","placementNote":"블로그 원리","prompt":"영문. 손 위치·정렬 디테일. 글자·로고 없음."}
],
${mangoBriefEx}
}`;
  }
  return `"images": {
"gptVisuals": [
{"role":"problem","title":"① 문제 제기·표지","blogSlot":"problem","mangoUse":"표지·공감","aspect":"1:1","placementNote":"블로그 상단","prompt":"영문. blog.problem 일상 공감. 동작 시연 아님. 글자·로고 없음."},
{"role":"selfCare","title":"② 셀프 케어","blogSlot":"selfCare","mangoUse":"참고사진1순위","aspect":"1:1","placementNote":"망고보드 업로드 최우선","prompt":"영문. blog.selfCare와 동일 동작. 전신·잘림 금지."},
{"role":"explanation","title":"③ 원리 설명","blogSlot":"explanation","mangoUse":"원리","aspect":"1:1","placementNote":"블로그 원리","prompt":"영문. 정렬·디테일. 도표·글자 금지."}
],
${mangoBriefEx}
}`;
}
...
1) productName (=「제품명 혹은 주제」): **반드시 채움**, **최대 ${MANGO_DETAIL_TITLE_MAX}자**. blog.title을 짧게 줄인 말. 빈 값 금지. 이모지·과장 금지.
2) intro (=「소개」): **반드시 채움**, **최대 ${MANGO_DETAIL_INTRO_MAX}자**. 망고보드 소개란에 붙일 **본문 기획 글**입니다. 빈 값·디자인 톤만 쓰기 금지.
3) mangoInputPaste = intro와 **글자 하나까지 동일**.
4) photoTip: 참고 사진 안내 1~2문장 (별도 필드). designTone: 흑·백·미니멀 한 줄 (별도 필드). **intro에 [디자인]·흑백·로고 후처리 문구를 넣지 마세요.**

intro 필수 구성 (합계 ${MANGO_DETAIL_INTRO_MAX}자 이내, 번호·불릿 OK — 이게 곧 7장 상세페이지 카피):
1. 표지·한 줄 훅 (독자가 멈추는 한 문장)
2. 문제 공감 (blog.problem 핵심)
3. 누구를 위한 내용인지
4. 셀프 케어 동작·초·회·분 (blog.selfCare 핵심)
5. 원리·왜 도움이 되는지 (blog.explanation 앞부분 TL;DR)
6. 주의·병원 권고가 필요한 경우 한 줄
7. FAQ 1~2 + CTA(상담·프로필, 과장 예약 압박 금지)`;

const DEFAULT_BLOG_INSTA_IMAGE_PROMPT = `이미지는 **정확히 3장** — 블로그 글 흐름·망고보드 상세페이지 참고 사진용.

[공통 — 필수]
- 각 prompt는 **영문 한 덩어리** (90~200단어). 사진만. **글자·로고·워터마크·인포그래픽 UI 금지**
- 실사 editorial / documentary. 자연광. AI 매끈 피부·크리미 의료 일러스트 금지
- 망고보드 상세페이지는 **참고 사진 최소 1장** 필요 → 아래 ② selfCare 컷을 최우선 첨부용으로
- 후처리에서 **흑·백 타이포·REAL MOVEMENT 로고**를 올릴 수 있게 피사체 중앙·여유
- 비율: **1:1 또는 4:5 세로**

[① problem — 문제 제기 / 표지·공감]
- blog.problem의 일상 불편·공감 장면. 동작 시연이 아님

[② selfCare — 셀프 케어] ★망고 참고 사진 1순위
- blog.selfCare와 **동일한** 동작·자세. 전신 또는 명확한 옆·대각. 잘리지 않게

[③ explanation — 원리 설명]
- 정렬·디테일·차분한 스튜디오/클리닉 컷 (도표·문자 금지)

${MANGO_DETAIL_BRIEF_RULE}`;

const DEFAULT_EXPERT_COURSE_IMAGE_PROMPT = `전문가 과정·강연 공유용 — **3장** (블로그·망고보드 참고 사진).

[공통] 영문 prompt, 사진만, 글자·로고 금지. documentary seminar/clinic. 1:1 또는 4:5.
망고보드 참고 사진은 **② 시연 컷**을 최우선.

[① problem/hook] 강의·영상 맥락 (blog.hook)
[② selfCare/demo] blog.outline·draft 시연과 **일치** ★참고 사진 1순위
[③ explanation] 손 위치·정렬 클로즈업

${MANGO_DETAIL_BRIEF_RULE}`;

const DEFAULT_HEILJAGYAE_IMAGE_PROMPT = `힐자계 아파트너 — **2장**: ① 공감·표지 ② 본문 셀프 동작.

[①] 단지·일상 공감 컷. 글자·로고 없음.
[②] community.selfCare와 **동일** 동작. 옆집 이웃 생활감.
...
function migrateMangoImagePromptDefaults_(){
  ...
  Object.keys(state.prompts.categories).forEach(function(catId){
    ...
    // 이미 망고보드 정렬이 있으면 재실행하지 않음 (매 로드마다 append 방지)
    if(/망고보드|mangoBrief|상세페이지 사진 슬롯|MANGO_DETAIL/i.test(cur)) return;
    var def = getDefaultCatPrompt_(catId, 'image');
    if(!def) return;
    // 구형 썸네일 밴드 2장만 기본값으로 교체. 커스텀 문구에는 임의 append 하지 않음
    var looksOldTwoSlot =
      /정확히 2장|상단 20%|단색 밴드|overlayFixedLine/.test(cur) &&
      cur.length < 1600;
    if(looksOldTwoSlot){
      cat.image = def;
      changed = true;
...
function buildSheetTabsHTML(tab){
  ...
  var imgN = getImageSlotCount(cid) || 2;
  if(isHeiljagyaeCategory(cid)){
    ...
    <button type="button" class="sheet-tab${tab==='images'?' active':''}" onclick="switchTab('images')">이미지 (${imgN})</button>
  </div></div>`;
  }
  return `<div class="sheet-tabs-scroll"><div class="sheet-tabs">
    <button type="button" class="sheet-tab${tab==='images'?' active':''}" onclick="switchTab('images')">망고보드 (${imgN})</button>
...
function normalizeMangoBrief_(brief){
  ...
  return {
    productName: clampMangoTitle_(brief.productName || ''),
    intro: intro,
...
function buildImagePromptGuide(topic, angle, customGuide, catId){
  ...
  return `
[이미지 3장 — 블로그 흐름 × 망고보드 상세페이지 사진 슬롯]
이번 초안 주제: "${t}"
작성 각도: "${a}"
${pillar ? '프로그램 톤: ' + pillar : ''}

**정확히 3개** gptVisuals + **mangoBrief** 1개.
각 prompt는 **영문 한 덩어리** (90~200 단어). 불릿·한 줄 요약 금지.
**이미지 안에 한글·영문 글자·로고·워터마크·UI 금지** (흑·백 타이포·로고는 망고보드 후처리).

블로그 흐름과 1:1로 맞출 것:
■ ① role:problem — ${isExpert ? 'blog.hook 맥락' : 'blog.problem 일상 공감'} (표지·도입). 동작 시연 컷 아님.
■ ② role:selfCare — ${isExpert ? 'blog.outline·draft 시연과 일치' : 'blog.selfCare와 동일 동작·자세·초·회·분'}
■ ③ role:explanation — ${isExpert ? '원리·손 위치 디테일' : 'blog.explanation을 돕는 정렬·디테일·차분한 공간'} (도표·문자 금지)

공통 비주얼:
- 1:1 또는 4:5, photorealistic editorial, natural light, visible skin texture
- 후처리에서 흑·백 배너·로고를 올릴 수 있게 피사체 중앙·가장자리 여유
- **금지**: 크리미 인포그래픽, sage medical illustration, 3D 해부, AI 매끈 피부, readable text, logos

${MANGO_DETAIL_BRIEF_RULE}

mangoBrief (망고보드 상세페이지 UI에 맞춤 — **빈 칸 금지**):
- productName: 「제품명 혹은 주제」 **최대 ${MANGO_DETAIL_TITLE_MAX}자**, blog.title 축약. **필수**
- intro / mangoInputPaste: 「소개」 **최대 ${MANGO_DETAIL_INTRO_MAX}자**, **동일 한글 본문**. 7장 카피(훅→문제→대상→셀프케어→원리→주의→CTA). **필수**
- intro에 designTone·[디자인]·로고 후처리 문구만 넣지 말 것 (그건 designTone 필드)
- photoTip: 참고 사진(최소 1장) — 서비스·시연 장면, ② selfCare 전신 컷 권장
- slidePlan: 정확히 7개
- designTone: 흑·백·미니멀·전문가 (intro와 분리)
...
**정확히 2개** gptVisuals: ① problem(공감·표지) ② selfCare(셀프 동작).
각 prompt는 **영문 한 덩어리** (90~200 단어). 이미지 안 글자·로고 금지.

■ ① role:problem — 단지·일상 공감. photorealistic, 광고톤 금지
■ ② role:selfCare — community.selfCare와 **동일** 동작·자세·초·회·분. 한국 아파트·요가 매트
...
function formatMangoBriefPaste_(brief){
  if(!brief) return '';
  // 소개란 = intro/mangoInputPaste만. designTone·features 폴백 금지
  return clampMangoIntro_(brief.intro || brief.mangoInputPaste || '');
}

function formatMangoTitlePaste_(brief){
  if(!brief) return '';
  return clampMangoTitle_(brief.productName || '');
}
...
function emptyMangoBrief_(){
  return {
    ...
    slidePlan: [],
    designTone: '흑·백·미니멀·전문가. 만화·코믹 금지. 로고는 후처리.',
    photoTip: '서비스·시연 장면 사진 최소 1장. ② 셀프 케어 전신 컷 권장.',
    mangoInputPaste: ''
  };
}
...
function isWeakMangoIntro_(s){
  ...
  // "[디자인] 흑·백…" 형태 · designTone만 들어온 경우
  if(/디자인/i.test(t) && /흑[·.\s]*백|미니멀|로고는 후처리/i.test(t) && t.length < 200) return true;
  if(/흑[·.\s]*백/.test(t) && /미니멀/.test(t) && /로고는 후처리|만화/.test(t) &&
    !/1\.\s*표지|문제 공감|셀프 케어|이런 분께/.test(t)) return true;
  return false;
}
...
function buildMangoBriefFromBlog_(content, topic){
  ...
  var productName = clampMangoTitle_(title || topic || '');
  var problem = '';
  var selfCare = '';
  var explanation = '';
  var cta = '';
  ...
  var lines = [];
  lines.push('1. 표지·훅');
  lines.push(trimMangoSection_(title || topic || hook || '오늘의 움직임 가이드', 120));
  lines.push('');
  lines.push('2. 문제 공감');
  lines.push(trimMangoSection_(problem || '일상에서 반복되는 불편을 가볍게 풀어 보는 내용입니다.', 320));
  lines.push('');
  lines.push('3. 이런 분께');
  lines.push(trimMangoSection_(
    (topic ? ('「' + topic + '」에 관심 있는 분, ') : '') + '오래 앉아 있거나 같은 자세가 반복될 때 몸이 굳는 느낌이 있는 분',
    160
  ));
  lines.push('');
  lines.push('4. 셀프 케어');
  lines.push(trimMangoSection_(selfCare || '무리하지 않는 범위에서 천천히 따라 해 보세요. 통증이 커지면 즉시 멈추세요.', 420));
  lines.push('');
  lines.push('5. 원리');
  lines.push(trimMangoSection_(explanation || '움직임과 정렬을 이해하면 일상에서 더 편하게 관리할 수 있습니다.', 420));
  lines.push('');
  lines.push('6. 주의');
  lines.push('통증이 갑자기 심해지거나 마비·저림이 커지면 셀프 케어 대신 병원 진료를 먼저 받으세요.');
  lines.push('');
  lines.push('7. FAQ·다음 행동');
  lines.push(trimMangoSection_(cta || '궁금한 점이 있으면 프로필·상담으로 문의해 주세요. 치료·진단이 아닌 일반 정보입니다.', 280));
  var intro = clampMangoIntro_(lines.join('\n').replace(/\n{3,}/g, '\n\n').trim());
  return {
...
function fillMangoBriefFromContent_(content, opts){
  ...
  if(!brief.designTone) brief.designTone = emptyMangoBrief_().designTone;
...
function renderMangoDetailInputCard_(brief){
  ...
  html += '<p style="font-size:12px;color:#6B7280;margin:0 0 10px;line-height:1.55;">망고보드 「상세페이지」의 <strong>제품명(30자)</strong>·<strong>소개(2000자)</strong>에 그대로 붙여넣습니다. 참고 사진은 <strong>최소 1장</strong>(② 셀프케어 시연 컷 권장).</p>';
  ...
  html += '<label style="display:block;font-size:11px;color:#374151;margin:0 0 4px;">소개 (7장 상세페이지용) <span id="sheet-mango-intro-count" style="color:#9CA3AF;">' + introVal.length + '/' + MANGO_DETAIL_INTRO_MAX + '</span></label>';
  ...
  html += '<button type="button" class="img-tool-main-btn mango" onclick="copyMangoBriefAndOpen_()">망고보드 열기 · 소개 복사</button>';
  html += '<button type="button" class="img-tool-copy-btn" onclick="copyMangoTitleOnly_()">제품명 복사</button>';
...
function buildImageTabBody(content){
  ...
  if(!isHj) ensureMangoBriefOnContent_(content);
  const brief = im.mangoBrief || null;
  ...
  if(isHj){
    html += '<div class="img-section-title">참고·배치 메모 (2장 · 아파트너)</div>';
    html += '<p style="font-size:12px;color:#6B7280;margin:0 0 14px;line-height:1.55;">① 공감·표지 · ② 셀프 동작. 게시글에 넣을 참고용입니다.</p>';
  } else if(gpt.length){
    html += '<div class="img-section-title">블로그 배치 참고 메모</div>';
    html += '<p style="font-size:12px;color:#6B7280;margin:0 0 14px;line-height:1.55;">문제→셀프케어→원리 배치용입니다. 망고보드에는 위 <strong>제품명·소개</strong>만 넣으면 됩니다.</p>';
  }
  if(gpt.length === 0){
    if(isHj) html += '<p class="empty-note" style="padding:12px 0;">메모가 없습니다. 재생성해 주세요.</p>';
  } else {
    gpt.forEach(function(item, i){
      ...
      var metaBits = [];
      if(item.blogSlot) metaBits.push('블로그: ' + item.blogSlot);
      if(item.mangoUse) metaBits.push('망고: ' + item.mangoUse);
      if(item.placementNote) metaBits.push(item.placementNote);
      if(item.aspect) metaBits.push(item.aspect);
      if(metaBits.length){
        html += '<div style="font-size:11px;color:#6B7280;margin:0 0 8px;line-height:1.5;">' + escapeHtml(metaBits.join(' · ')) + '</div>';
      }
      html += '<textarea class="sheet-edit" id="sheet-image-prompt-' + i + '" data-image-index="' + i + '" rows="5">' + escapeHtml(p) + '</textarea>';
      html += '<div class="img-tool-actions">';
      html += '<button type="button" class="img-tool-copy-btn" onclick="copyImagePromptField(' + i + ')">메모 복사</button>';
      html += '</div></div>';
    });
  ...
  html += '<p class="empty-note" style="padding:8px 0 0;font-size:11px;color:#9CA3AF;line-height:1.55;">수정 후 <strong>발행완료</strong>를 누르면 저장돼요.' + (isHj ? '' : ' 로고·흑백 타이포는 망고보드에서 올립니다.') + '</p>';
  if(!isHj){
    html += '<div class="img-tool-hint"><strong>망고보드 열기 · 소개 복사</strong>를 누르면 소개가 복사되고 <a href="' + escapeHtml(DEFAULT_MANGO_DESIGNER_URL) + '" target="_blank" rel="noopener noreferrer">AI 디자이너</a>가 열립니다. 상세페이지에서 제품명·소개·참고 사진을 넣어 주세요.</div>';
  }
...
window.copyMangoTitleOnly_ = function(){
  var el = document.getElementById('sheet-mango-title');
  var text = el ? clampMangoTitle_(el.value) : '';
  if(!text){
    var content = state.published[state.selectedId]?.content || state.generatedOnly[state.selectedId];
    text = formatMangoTitlePaste_(content && content.images && content.images.mangoBrief);
  }
...
window.copyMangoBriefOnly_ = function(){
  var el = document.getElementById('sheet-mango-brief-paste');
  var text = el ? clampMangoIntro_(el.value) : '';
  if(!text){
    var content = state.published[state.selectedId]?.content || state.generatedOnly[state.selectedId];
    text = formatMangoBriefPaste_(content && content.images && content.images.mangoBrief);
  }
...
window.copyMangoBriefAndOpen_ = function(){
  ...
  navigator.clipboard.writeText(text).then(function(){
    ...
    setAppToast('소개 복사됨 · 망고보드에서 상세페이지 → 소개란에 붙여넣으세요.', { duration: 3500, variant: 'ok' });
  }).catch(function(){
    window.open(getMangoDesignerOpenUrl_(), '_blank', 'noopener,noreferrer');
    window.prompt('소개를 복사해 주세요:', text);
  });
};
...
function renderSheetEmpty(draft, cat) {
  const genLine = isDailyShareCategory(cat.id)
    ...
    ? 'AI가 <strong>블로그(문제 제기·셀프 케어·원리)</strong>와 망고보드용 사진 프롬프트 <strong>3장</strong>·기획을 만들어드려요.<br>블로그 <strong>발행완료</strong> 후 인스타·쓰레드 초안이 <strong>동시에</strong> 만들어져요'
    : isExpertCourseCategory(cat.id)
    ? '교육·강의 <strong>영상 링크</strong> 또는 <strong>실습 사진</strong>을 먼저 올리면, AI가 그에 맞춰<br><strong>영상·사진 맥락 → 시연 포인트 → 원리 설명</strong> 글과 망고보드용 사진 프롬프트 <strong>3장</strong>을 만들어요.<br>블로그 <strong>발행완료</strong> 후 인스타·쓰레드 초안이 <strong>동시에</strong> 만들어져요'
    : 'AI가 <strong>블로그</strong>와 망고보드용 사진 프롬프트 <strong>3장</strong>을 만들어드려요.<br>블로그 <strong>발행완료</strong> 후 인스타·쓰레드 초안이 <strong>동시에</strong> 만들어져요';
...
window.getFullCopy = function(){
  ...
  if(im){
    ...
    if(imgText) t += '\n\n[망고보드 기획·사진 프롬프트 · ' + getDisplayGptVisuals_(im.gptVisuals, imgCatId).length + '장]\n' + imgText;
  }
  return t;
};
...
function getTabCopyText(tab, content){
  ...
  if(tab === 'images'){
    ...
    return imgCopy ? '[망고보드 기획·사진 프롬프트]\n' + imgCopy : '';
  }
  return '';
}
...
function applySheetImageEdits_(content){
  ...
  if(titleEl || briefEl){
    ...
    if(titleEl) content.images.mangoBrief.productName = clampMangoTitle_(titleEl.value);
    if(briefEl){
...
function getImagePromptTextForData_(content, catId){
  ...
  if(list.length){
    parts.push(list.map(function(x, i){
      var lines = [(i + 1) + '. ' + (x.title || '이미지')];
      if(x.blogSlot) lines.push('블로그 슬롯: ' + x.blogSlot);
      if(x.mangoUse) lines.push('망고보드: ' + x.mangoUse);
      if(x.placementNote) lines.push('배치: ' + x.placementNote);
      if(isHeiljagyaeCategory(catId)){
...
<div class="prompt-section">
  ...
  <div class="prompt-hint">망고보드 상세페이지: 제품명 30자 · 소개 2000자(7장 흐름). GPT 없이 제품명·소개·실사 참고사진만으로도 됩니다.</div>
</div>` : ''}
...
이 카테고리는 **blog(3단 구조)**와 **images(gptVisuals 3장 + mangoBrief)** 만 만듭니다. insta, notebookLM, community, thread 키는 JSON에 넣지 마세요.
...
반드시 images.gptVisuals는 **정확히 3개**(problem·selfCare·explanation). mangoBrief.productName·intro·mangoInputPaste는 **비우지 말 것**(intro=망고보드 소개란용 한글 7장 카피, designTone만 넣지 말 것). ②번 동작은 blog.selfCare와 일치. 사진에 글자·로고 금지.`;
...
이 카테고리는 **blog**와 **images(gptVisuals 3장 + mangoBrief)** 만 만듭니다. insta, notebookLM, community, thread 키는 JSON에 넣지 마세요.
...
반드시 images.gptVisuals는 **정확히 3개**. mangoBrief.productName·intro·mangoInputPaste **필수**(intro=망고보드 소개란용 한글, designTone만 금지). ②번은 blog.outline·draft의 시연·테크닉과 일치. 사진에 글자·로고 금지.`;
} else {
