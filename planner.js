...
function isGptVisualThumbnail_(item, idx, list){
  ...
  var isThumb = item.role === 'thumbnail' || item.role === 'problem' || item.role === 'cover' ||
    /썸네일|thumbnail|표지|공감|문제 제기|①/i.test(String(item.title || '')) ||
    (/①/.test(String(item.title || '')) && !/②|③|동작|본문|시연|셀프|원리/i.test(String(item.title || '')));
  if(!isThumb && idx === 0 && list && list.length >= 2){
    var second = list[1];
    if(second && (/동작|본문|시연|셀프|②|selfCare/i.test(String(second.title || '')) || second.role === 'body' || second.role === 'selfCare')){
      isThumb = true;
    }
  }
  return isThumb;
...
function syncThumbnailOverlayHook_(content, catId){
  ...
  if(!isHeiljagyaeCategory(catId)) return content;
...
function enrichImageOverlays_(content, catId){
  if(!content || !content.images || !content.images.gptVisuals) return content;
  // 망고보드·블로그 3슬롯(problem/selfCare/explanation)은 역할·오버레이를 덮어쓰지 않음
  if(!isHeiljagyaeCategory(catId)) return content;
...
function buildImageGptVisualsJsonExample_(catId, opts){
  ...
  var id = parseInt(catId, 10);
  if(id === 7){
    return `"images": { "gptVisuals": [], "mangoBrief": null }`;
  }
  var slidePlan7 =
    '[{"slide":1,"role":"표지","copyHint":"훅 한 줄(큰 글씨)"},{"slide":2,"role":"공감","copyHint":"일상 불편 헤드+불릿"},{"slide":3,"role":"대상","copyHint":"이런 분 한 줄"},{"slide":4,"role":"따라하기","copyHint":"STEP 동작·초·회"},{"slide":5,"role":"원리","copyHint":"왜-쉬운 비유"},{"slide":6,"role":"주의","copyHint":"병원 신호·무리 금지"},{"slide":7,"role":"다음","copyHint":"FAQ 또는 CTA"}]';
  var mangoBriefEx =
    '"mangoBrief": {\n' +
    '"productName":"주제 최대30자",\n' +
    '"intro":"【1. 표지】헤드/비주얼 … 7장. 이미지용 짧은 카피만. 블로그 원문·… 금지. 최대2000자",\n' +
    '"features":["특장점1","특장점2","특장점3"],\n' +
    '"slidePlan":' + slidePlan7 + ',\n' +
    '"designTone":"' + getMangoDesignToneLine_(id).replace(/"/g, '\\"') + '",\n' +
...
function applyInitialProgramTopicsToCat_(catId){
  ...
  // 사용자가 「초안 없는 주제 삭제」한 뒤에는 시드로 다시 채우지 않음
  try {
    var blockedAt = state._blockAutoSeedTopics && state._blockAutoSeedTopics[String(catId)];
    if(blockedAt && (Date.now() - blockedAt) < 7 * 24 * 60 * 60 * 1000) return 0;
  } catch(eBlock){}
  var applied = 0;
...
function getEffectiveSubGoalPlan_(catId){
  if(usePendingProgramPreview_(catId)){
    var p = state.pendingSubGoalPlan;
    if(p && p.plan) dedupeSubGoalPlanStepsInPlace_(p.plan);
    return {
...
function applyTopicToStepSlot_(catId, stepId, slotNum, topicData){
  ...
  if(state.deletedDraftIds && state.deletedDraftIds[id]){
    // 삭제된 시드 id 재사용 금지
    id = makeExtraDraftId_(catId, Date.now() % 100000 + slot * 17);
    if(state.deletedDraftIds[id]) return false;
  }
  var draft = {
    id: id,
...
/** 단계 id 정규화: "1" / "s1" → "s1" */
function canonicalSubGoalStepId_(id, fallbackIndex){
  var raw = String(id == null ? '' : id).trim();
  var m = raw.match(/^s?(\d+)$/i);
  if(m) return 's' + m[1];
  if(raw) return raw;
  return 's' + (Math.max(0, parseInt(fallbackIndex, 10) || 0) + 1);
}
function stepTitleDedupeKey_(title){
  return String(title || '')
    .replace(/^[①②③④⑤⑥⑦⑧⑨⑩⑪⑫]\s*/u, '')
    .replace(/^\d+\s*단계\s*[·—\-:.]?\s*/u, '')
    .replace(/\s+/g, ' ')
    .trim()
    .toLowerCase();
}
function stepStageNumFromTitle_(title){
  var m = String(title || '').match(/(\d+)\s*단계/);
  if(!m) return 0;
  var n = parseInt(m[1], 10);
  return (n >= 1 && n <= 12) ? n : 0;
}
/**
 * 동기화/ID 마이그레이션으로 같은 단계가 여러 줄 쌓인 경우 정리.
 * (④~⑧이 모두 「4단계 · …」처럼 같은 제목이 반복되는 고스트 블록)
 * @returns {boolean|Object} changed 여부. opts.collectIdMap 이면 { changed, idMap }
 */
function dedupeSubGoalPlanStepsInPlace_(plan, opts){
  opts = opts || {};
  if(!plan || !Array.isArray(plan.steps)) return opts.collectIdMap ? { changed: false, idMap: {} } : false;
  var seenId = {};
  var seenTitle = {};
  var seenStage = {};
  var keepByTitle = {};
  ...
  plan.steps.forEach(function(step, i){
    if(!step || typeof step !== 'object'){ changed = true; return; }
    var oldId = String(step.id != null ? step.id : '');
    var nid = canonicalSubGoalStepId_(step.id, i);
    var tkey = stepTitleDedupeKey_(step.title);
    var stage = stepStageNumFromTitle_(step.title);
    var dropTo = null;
    if(seenId[nid]) dropTo = seenId[nid];
    else if(tkey && seenTitle[tkey]) dropTo = keepByTitle[tkey];
    else if(stage && seenStage[stage]) dropTo = keepByStage[stage];
    if(dropTo){
      if(oldId && oldId !== dropTo) idMap[oldId] = dropTo;
      if(nid && nid !== dropTo) idMap[nid] = dropTo;
      changed = true;
      return;
    }
    seenId[nid] = nid;
    if(tkey){ seenTitle[tkey] = true; keepByTitle[tkey] = nid; }
    if(stage){ seenStage[stage] = true; keepByStage[stage] = nid; }
    if(oldId && oldId !== nid) idMap[oldId] = nid;
    if(String(step.id) !== nid){
      step.id = nid;
      changed = true;
    }
    out.push(step);
  ...
  out.forEach(function(step, i){
    var want = 's' + (i + 1);
    var cur = String(step.id);
    if(cur !== want){
      idMap[cur] = want;
      Object.keys(idMap).forEach(function(k){
        if(idMap[k] === cur) idMap[k] = want;
      });
      step.id = want;
      changed = true;
    }
  });
  if(changed) plan.steps = out;
  if(opts.collectIdMap) return { changed: changed, idMap: idMap };
  return changed;
}
function dedupeAllSubGoalPlanSteps_(){
  if(!state.branding || !state.branding.subGoalPlans) return false;
  var changed = false;
  Object.keys(state.branding.subGoalPlans).forEach(function(k){
    var res = dedupeSubGoalPlanStepsInPlace_(state.branding.subGoalPlans[k], { collectIdMap: true });
    if(res.changed){
      changed = true;
      if(res.idMap && Object.keys(res.idMap).length){
        try { remapDraftStepIdsForCat_({ draftBrandOverrides: state.draftBrandOverrides }, k, res.idMap); } catch(eMap){}
      }
    }
  });
  if(state.pendingSubGoalPlan && state.pendingSubGoalPlan.plan){
    var pend = dedupeSubGoalPlanStepsInPlace_(state.pendingSubGoalPlan.plan, { collectIdMap: true });
    if(pend.changed){
      changed = true;
      if(pend.idMap && state.pendingSubGoalPlan.assignments){
        state.pendingSubGoalPlan.assignments.forEach(function(a){
          if(!a) return;
          var sid = String(a.stepId || '');
          if(pend.idMap[sid]) a.stepId = pend.idMap[sid];
        });
      }
    }
  }
  if(changed){
    try { pruneOrphanSubGoalStepOutbox_(); } catch(ePrune){}
  }
  return changed;
}
function getSubGoalPlan_(catId){
  ...
  if(res.changed){
    if(res.idMap && Object.keys(res.idMap).length){
      try { remapDraftStepIdsForCat_({ draftBrandOverrides: state.draftBrandOverrides }, catId, res.idMap); } catch(eMap){}
    }
    plan.updatedAt = new Date().toISOString();
    if(!state._dedupeStepsSaveScheduled){
      state._dedupeStepsSaveScheduled = true;
      setTimeout(function(){
        state._dedupeStepsSaveScheduled = false;
        try {
          pruneOrphanSubGoalStepOutbox_();
          // IndexedDB 가 있으면 초안 삭제 복구 경로를 쓰지 않음
...
function repairMislabeledSubGoalSteps_(catId){
  ...
  if(dedupeSubGoalPlanStepsInPlace_(plan)) changed = true;
  var seed = getInitialProgramPlanDraft_(catId);
  var maxKeep = (seed && seed.steps && seed.steps.length) ? seed.steps.length : 5;
  if(plan.steps.length > maxKeep){
    plan.steps = plan.steps.slice(0, maxKeep);
    changed = true;
  }
  var seenTitles = {};
  var idMap = {};
  plan.steps.forEach(function(step, i){
    ...
    if((dup || mismatch) && seed && seed.steps[i]){
      ...
      changed = true;
      title = String(step.title || '').trim();
    } else if(dup && !(seed && seed.steps[i])){
      // 시드 밖 중복은 위에서 slice 했지만, 안전망으로 빈 제목 방지
      changed = true;
    }
  ...
  if(Object.keys(idMap).length){
    remapDraftStepIdsForCat_({ draftBrandOverrides: state.draftBrandOverrides }, catId, idMap);
    var pendingMap = state.pendingSubGoalPlan;
    if(pendingMap && sameCatId_(pendingMap.catId, catId) && pendingMap.assignments){
      pendingMap.assignments.forEach(function(a){
        if(!a) return;
        var sid = String(a.stepId || '');
  ...
  var pending = state.pendingSubGoalPlan;
  if(pending && sameCatId_(pending.catId, catId) && pending.plan){
    if(dedupeSubGoalPlanStepsInPlace_(pending.plan)) changed = true;
  }
...
function closeTruncatedJsonSlice_(slice){
  ...
  if(inString) s += '"';
  s = s.replace(/,\s*$/, '');
  var openBrackets = (s.match(/\[/g) || []).length - (s.match(/\]/g) || []).length;
  var openBraces = (s.match(/\{/g) || []).length - (s.match(/\}/g) || []).length;
...
function parsePlannerAiJsonValue_(text){
  var obj = parsePlannerAiJsonObject_(text);
...
const SK = 'ht_v2';
/** IndexedDB — localStorage(5~10MB) 한도를 넘는 플래너 본문 보관 */
const PLANNER_IDB_NAME = 'ht_planner_idb_v1';
const PLANNER_IDB_STORE = 'kv';
let plannerIdbDb_ = null;
let plannerIdbWriteChain_ = Promise.resolve();
let plannerLastDiskPayloadStr_ = null;
let plannerUsingIdbPrimary_ = false;
let plannerIdbWriteFailed_ = false;

function openPlannerIdb_(){
  return new Promise(function(resolve, reject){
    if(plannerIdbDb_){ resolve(plannerIdbDb_); return; }
    if(typeof indexedDB === 'undefined'){ reject(new Error('IndexedDB unavailable')); return; }
    var req = indexedDB.open(PLANNER_IDB_NAME, 1);
    req.onupgradeneeded = function(){
      var db = req.result;
      if(!db.objectStoreNames.contains(PLANNER_IDB_STORE)){
        db.createObjectStore(PLANNER_IDB_STORE);
      }
    };
    req.onsuccess = function(){
      plannerIdbDb_ = req.result;
      resolve(plannerIdbDb_);
    };
    req.onerror = function(){ reject(req.error || new Error('idb open failed')); };
  });
}
function schedulePlannerIdbWrite_(payloadStr){
  // 이전 실패가 체인을 끊지 않도록 항상 복구
  plannerIdbWriteChain_ = plannerIdbWriteChain_.catch(function(){ return null; }).then(function(){
    return openPlannerIdb_().then(function(db){
      return new Promise(function(resolve, reject){
        var tx = db.transaction(PLANNER_IDB_STORE, 'readwrite');
        tx.oncomplete = function(){ resolve(true); };
        tx.onerror = function(){ reject(tx.error || new Error('idb write failed')); };
        tx.objectStore(PLANNER_IDB_STORE).put({
          v: 1,
          savedAt: Date.now(),
          len: String(payloadStr || '').length,
          payload: String(payloadStr || '')
        }, SK);
      });
    });
  }).then(function(){
    plannerUsingIdbPrimary_ = true;
    plannerIdbWriteFailed_ = false;
    try {
      if(navigator.storage && navigator.storage.persist && !state._storagePersistAsked){
        state._storagePersistAsked = true;
        navigator.storage.persist().catch(function(){});
      }
    } catch(ePersist){}
  }).catch(function(err){
    plannerIdbWriteFailed_ = true;
    console.warn('[planner IndexedDB]', err);
  });
  return plannerIdbWriteChain_;
}
function flushPlannerIdbNow_(){
  if(!plannerLastDiskPayloadStr_) return plannerIdbWriteChain_;
  return schedulePlannerIdbWrite_(plannerLastDiskPayloadStr_);
}
function readPlannerIdbPayload_(){
  return openPlannerIdb_().then(function(db){
    return new Promise(function(resolve, reject){
      var tx = db.transaction(PLANNER_IDB_STORE, 'readonly');
      var req = tx.objectStore(PLANNER_IDB_STORE).get(SK);
      req.onsuccess = function(){
        var row = req.result;
        if(row == null){ resolve(''); return; }
        if(typeof row === 'string'){ resolve(row); return; }
        resolve(String((row && row.payload) || ''));
      };
      req.onerror = function(){ reject(req.error || new Error('idb read failed')); };
    });
  }).catch(function(){ return ''; });
}
function isPlannerLsIdbStubRaw_(raw){
  if(!raw || raw.length > 800) return false;
  try {
    var j = JSON.parse(raw);
    return !!(j && j._idbPrimary);
  } catch(e){ return false; }
}
function buildPlannerLsMirrorStub_(payloadStr){
  var savedAt = '';
  var rev = 0;
  try {
    var m1 = String(payloadStr || '').match(/"localSavedAt"\s*:\s*"([^"]*)"/);
    var m2 = String(payloadStr || '').match(/"syncRevision"\s*:\s*(\d+)/);
    if(m1) savedAt = m1[1];
    if(m2) rev = parseInt(m2[1], 10) || 0;
  } catch(e){}
  return JSON.stringify({
    _idbPrimary: true,
    localSavedAt: savedAt || (state && state.localSavedAt) || '',
    syncRevision: rev || (state && state.syncRevision) || 0,
    _payloadChars: String(payloadStr || '').length
  });
}
function readPreviousDiskPayloadSync_(){
  if(plannerLastDiskPayloadStr_){
    try { return JSON.parse(plannerLastDiskPayloadStr_); } catch(e0){}
  }
  try {
    var raw = localStorage.getItem(SK) || '';
    if(!raw || isPlannerLsIdbStubRaw_(raw)) return null;
...
function getPromptTypeLabelKr_(type){
  var labels = { blog: '블로그', insta: '인스타', threads: '쓰레드', image: '망고보드', community: '아파트너 게시판', thread: '일상 공유' };
...
const DEFAULT_CHATGPT_IMAGE_PROJECT_URL = 'https://www.mangoboard.net/aichat/designer';
const DEFAULT_MANGO_DESIGNER_URL = 'https://www.mangoboard.net/aichat/designer';
...
/** 블로그·인스타 기본 말투 (존댓말). 저장본 마이그레이션 마커로도 사용 */
const BLOG_INSTA_HONORIFIC_SPEECH_RULE = `[말투 — 블로그·인스타 존댓말]
- 본문·캡션·CTA는 **존댓말(해요체)**로 씁니다. 예: "~해요", "~이에요", "~하세요", "~습니다".
- **한다체·반말 금지**: "~한다", "~이다", "~된다", "~눕는다", "~야" 같은 평서·반말 어미를 쓰지 마세요.
- 독자에게 말 거는 따뜻한 존댓말을 끝까지 유지하세요.`;

/** 쓰레드 기본 말투 (반말). 저장본 마이그레이션 마커로도 사용 */
const THREADS_BANMAL_SPEECH_RULE = `[말투 — 쓰레드 반말]
- **반말(구어체)**이 기본입니다. 친구·동료에게 짧게 말하듯. 예: "~해", "~야", "~거든", "~지", "~더라".
- 해요체·합니다체 존댓말은 쓰지 마세요.
...
/** 0 도수·3 CMT=남색 / 1 무브·5 Re:Al움직임=데본크림 / 2 페이스·4 IFC=페이스 / 6 일상·7 힐자계=녹색 */
function getMangoPaletteKeyForCat_(catId){
  var id = parseInt(catId, 10);
  if(id === 0 || id === 3) return 'navy';
  if(id === 1 || id === 5) return 'devon';
  if(id === 2 || id === 4) return 'face';
  if(id === 6 || id === 7) return 'green';
  return 'face';
}
function getMangoPaletteForCat_(catId){
  return MANGO_COLOR_PALETTES[getMangoPaletteKeyForCat_(catId)] || MANGO_COLOR_PALETTES.face;
}
...
function getMangoModelGuideForCat_(catId){
  ...
  if(id === 2 || id === 4){
    return {
      gender: 'female',
      label: '여성',
      pasteLine: '인물=여성 실사 우선. 남성 모델 반복 지양. 전/후 환영. 추상 한발서기만 반복 금지.',
      copyBullet: '인물 사진이 필요하면 **여성 모델·실사** 우선(일상·클리닉/홈). **남성 모델 반복 지양**.',
      photoTip: '여성·전신·일상/시연 동작 컷. 남성 모델·한발서기만 반복 컷 지양. 잘린 제품샷·글자 캡처 금지.',
      photoTipShort: '여성·전신·일상/시연 동작 컷',
      visualCover: '큰 타이포 한 줄 + 여성 실사(선택)',
      visualStep: '픽토그램 STEP 또는 여성 시연',
      briefHint: '여성 모델',
      uiHint: '여성 모델·일상 전/후'
    };
  }
  if(id === 0 || id === 1 || id === 3 || id === 5){
    return {
      gender: 'male',
      label: '남성',
      pasteLine: '인물=남성 실사 우선. 여성 모델 반복 지양. 전/후 환영. 추상 한발서기만 반복 금지.',
      copyBullet: '인물 사진이 필요하면 **남성 모델·실사** 우선(일상·클리닉/홈·시연). **여성 모델 반복 지양**.',
      photoTip: '남성·전신·일상/시연 동작 컷. 여성 모델·한발서기만 반복 컷 지양. 잘린 제품샷·글자 캡처 금지.',
      photoTipShort: '남성·전신·일상/시연 동작 컷',
      visualCover: '큰 타이포 한 줄 + 남성 실사(선택)',
      visualStep: '픽토그램 STEP 또는 남성 시연',
      briefHint: '남성 모델',
      uiHint: '남성 모델·일상 전/후'
    };
  }
  return {
    gender: 'neutral',
    label: '실사',
    pasteLine: '인물=실사 시연 컷. 전/후 환영. 추상 한발서기만 반복 금지.',
    copyBullet: '인물 사진이 필요하면 **실사** 우선(일상·시연).',
    photoTip: '실사·전신·일상/시연 동작 컷. 한발서기만 반복 컷 지양. 잘린 제품샷·글자 캡처 금지.',
    photoTipShort: '실사·전신·일상/시연 동작 컷',
    visualCover: '큰 타이포 한 줄 + 실사(선택)',
    visualStep: '픽토그램 STEP 또는 시연',
    briefHint: '실사',
    uiHint: '실사·일상 전/후'
  };
}
function buildMangoDesignToneLineFromPalette_(p){
  p = p || MANGO_COLOR_PALETTES.face;
...
- **로고·서명(필수)**
  - 위치 **우하단 고정**. 슬라이드 높이 **약 6~8% 이하**.
  - 헤드·불릿·얼굴·손과 **겹치지 않게**. 여백 확보.
  - 생성 중 넣기 어려우면 후처리로 동일 규칙 적용.
- 용어는 일반 회원용. 「가동성·안정성·ROM·고유수용」 대신 「잘 늘어나는 느낌 / 버티는 힘 / 조절 / 삐끗」처럼 쉬운 말.
- ${m.copyBullet}
- 픽토그램·간단한 아이콘 STEP은 유지 (과한 인포그래픽·표 UI 금지).
...
function getMangoDetailBriefRule_(catId){
  ...
  var layoutPaste = getMangoLayoutPasteBlock_(catId);
  var layoutCopy = getMangoLayoutCopyRule_(catId);
  var p = getMangoPaletteForCat_(catId);
  var m = getMangoModelGuideForCat_(catId);
  return `[망고보드 AI 상세페이지 입력 — 필수 형식]
망고보드 「상세페이지」 화면의 **제품명·소개 입력란에 그대로 붙여넣는 한글**만 작성합니다.
1) productName (=「제품명 혹은 주제」): **반드시 채움**, **최대 ${MANGO_DETAIL_TITLE_MAX}자**. 블로그 제목이 길면 **핵심 포인트가 남는 짧은 제목**으로. 이모지·과장 금지.
2) intro (=「소개」): **반드시 채움**, **최대 ${MANGO_DETAIL_INTRO_MAX}자**. **이미지로 그릴 수 있는 짧은 카피**만. 블로그 원문 붙여넣기·말줄임(…) 금지.
3) mangoInputPaste = intro와 **동일**.
4) intro 본문은 아래 7장(이미지용) + **맨 끝 [디자인]·[레이아웃] 블록**.
...
… (3~7장 동일 패턴, 장마다 역할·메시지 다르게)

레이아웃·톤도 함께 반영:
${layoutCopy}

intro 맨 끝 필수 (짧게만 — 망고 붙여넣기용):
[디자인]
${tone}
...
5) photoTip: 참고 사진은 **${m.photoTipShort}** 권장 (잘린 제품샷·글자 캡처 금지). 인물 모델은 **${m.label}**.
6) slidePlan: 7개 — 각 slide의 role·copyHint는 위 장 역할과 맞추고, copyHint는 **짧은 헤드 문장**만.
7) designTone: 위 [디자인] 한 줄과 동일 (${p.label}).`;
}

/** @deprecated 호환용 — 페이스 팔레트 기본값. 새 코드는 getMangoDesignToneLine_(catId) 사용 */
const MANGO_DESIGN_TONE_LINE = buildMangoDesignToneLineFromPalette_(MANGO_COLOR_PALETTES.face);
const MANGO_LAYOUT_PASTE_BLOCK = getMangoLayoutPasteBlock_(2);
const MANGO_LAYOUT_COPY_RULE = getMangoLayoutCopyRule_(2);

/** 망고보드 소개란: 블로그 붙여넣기 대신 장별 이미지용 핵심 카피 */
const MANGO_SLIDE_VISUAL_COPY_RULE = `[이미지용 카피 — 소개란 본문 필수]
블로그 문단을 잘라 붙이지 마세요. 말줄임표(…) 금지.
각 장은 **역할이 다르고**, 앞 장과 **같은 문장·같은 메시지 반복 금지**.
장마다 형식:
- 헤드: (슬라이드 큰 글씨, **40자 내외 · 문장 완성**, 무리한 중간 절단 금지)
- • 불릿 1~3개 (각 **50자 내외 · 의미 완결**) 또는 STEP1·2·3 — **표지(1장)는 불릿 최소화**
- 비주얼: (한 줄 — 사진/픽토그램 힌트. 장면은 주제마다 다르게, 구도 고정 금지)
...
1. 표지 — 질문·대전제 **한 줄만**. 설명·불릿 최소.
2. 공감 — 일상에서 겪는 불편 장면. 표지 문장 재사용 금지.
3. 대상 — 「이런 분」한 줄 + 짧은 신호 1~2개. 공감 장 문장 반복 금지.
4. 따라하기 — **동작·초·회만** STEP. 원리 설명 넣지 말 것.
5. 원리 — 「왜」만 쉬운 비유 1헤드+불릿. 동작 STEP 반복 금지.
...
제품명·소개·[디자인][레이아웃] 색상은 **생성 시 카테고리별 팔레트 지침**을 따릅니다.`;

const DEFAULT_HEILJAGYAE_IMAGE_PROMPT = `힐자계 아파트너 — **망고보드·GPT 이미지 프롬프트 사용 안 함**. gptVisuals는 []. mangoBrief는 null.
게시글용 사진은 **실사·시연 컷만** 직접 첨부합니다.`;
...
function buildExpertCourseInstaPrompt_(opts){
  ...
  return '전문가 과정 **강연·영상·사진 공유**용 인스타 캡션입니다. 블로그보다 짧고 밀도 있게.\n\n' +
    ...
    '- caption: hook 제외 **200~400자**. 수강·등록 유도 금지. **존댓말**\n' +
...
인스타: @dr.park_dc.pt` + DEFAULT_PSP_CLINICAL_FRAMEWORK,
  categories: {
    0: { // 도수치료
      ...
      insta: `${BLOG_INSTA_HONORIFIC_SPEECH_RULE}\n\n캐러셀(여러 장 슬라이드) 없이 **한 포스트 캡션**으로 끝내세요. 첫 줄 후킹에 이어, 본문 캡션 안에 통증→원인→해결 흐름의 **핵심 포인트**를 번호(1. 2.) 또는 • 불릿으로 넣어 한 번에 읽히게 하세요.`,
...
function migrateSpeechStylePromptDefaults_(){
  if(!state.prompts || !state.prompts.categories) return false;
  var changed = false;
  var blogMarker = '[말투 — 블로그·인스타 존댓말]';
  var threadsMarker = '[말투 — 쓰레드 반말]';
  var oldThreadsTone =
    '[말투]\n' +
  ...
  Object.keys(state.prompts.categories).forEach(function(catId){
    var cat = state.prompts.categories[catId];
    if(!cat || typeof cat !== 'object') return;
    if(cat.blog && String(cat.blog).indexOf(blogMarker) < 0){
      cat.blog = String(cat.blog).trim() + '\n\n' + BLOG_INSTA_HONORIFIC_SPEECH_RULE;
      changed = true;
    }
    if(cat.insta && String(cat.insta).indexOf(blogMarker) < 0){
      cat.insta = String(cat.insta).trim() + '\n\n' + BLOG_INSTA_HONORIFIC_SPEECH_RULE;
      changed = true;
    }
    if(cat.threads){
      var t = String(cat.threads);
      if(t.indexOf(threadsMarker) >= 0) return;
      if(t.indexOf(oldThreadsTone) >= 0){
        cat.threads = t.split(oldThreadsTone).join(THREADS_BANMAL_SPEECH_RULE);
        changed = true;
      } else {
        cat.threads = t.trim() + '\n\n' + THREADS_BANMAL_SPEECH_RULE;
        changed = true;
...
/** 구형 2장(썸네일 밴드) 이미지 지침 → 망고보드·블로그 3장 슬롯 */
function migrateMangoImagePromptDefaults_(){
  if(!state.prompts || !state.prompts.categories) return false;
  var changed = false;
  var badHjAppend = /\[망고보드·블로그 정렬\][\s\S]*$/;
  Object.keys(state.prompts.categories).forEach(function(catId){
    var id = parseInt(catId, 10);
    var cat = state.prompts.categories[catId];
    if(!cat || !cat.image) return;
    var cur = String(cat.image);

    // 이전 버전이 힐자계 프롬프트에 잘못 append한 경우 복구
    if(isHeiljagyaeCategory(id) && badHjAppend.test(cur)){
      var cleaned = cur.replace(badHjAppend, '').trim();
      cat.image = cleaned || getDefaultCatPrompt_(catId, 'image') || cleaned;
      changed = true;
      return;
    }

    // 힐자계(2장)·일일공유는 망고보드 마이그레이션 대상 아님
    if(isHeiljagyaeCategory(id) || isThreadCategory(id)) return;
    ...
    var looksOldGptSlots =
      /정확히 2장|정확히 3장|상단 20%|단색 밴드|overlayFixedLine|영문 한 덩어리|photorealistic editorial/i.test(cur);
    if(looksOldGptSlots){
      cat.image = def;
      changed = true;
    }
...
function syncValueFingerprint_(value){
  ...
  try {
    ...
    // 대형 초안/발행본 전체 문자열 비교는 부트스트랩에서 메인 스레드를 멈춤
    if(s.length > 2500) return 'h:' + syncStringHash_(s);
    return s;
...
function syncPayloadContentFingerprint_(payload){
  ...
  try {
    ...
    if(s.length > 8000) return 'h:' + syncStringHash_(s);
    return s;
  } catch(e){ return ''; }
}
...
function collectSyncEntities_(payload, opts){
  ...
  if(!(opts && opts.skipEnsureIds)) ensurePlanRowStableIdsInPayload_(payload);
  var out = {};
...
function ensureSyncEntityTimes_(payload){
  ...
  // times 보강만 할 때는 id 마이그레이션을 다시 돌리지 않음
...
function upsertPlanRowByStableKey_(rows, keyPart, wrapped, missing, touchId){
  ...
  function sameStepId_(a, b){
    a = String(a || '');
    b = String(b || '');
    if(a === b) return true;
    // "1" ↔ "s1" 동일 단계로 취급 (동기화 중복 삽입 방지)
    var na = a.match(/^s?(\d+)$/i);
    var nb = b.match(/^s?(\d+)$/i);
    return !!(na && nb && na[1] === nb[1]);
  }
  var hasIdMatch = rows.some(function(row){ return row && sameStepId_(row.id, key); });
  ...
  for(i = 0; i < rows.length; i++){
    if(rows[i] && sameStepId_(rows[i].id, key)){ found = i; break; }
  }
  // 제목이 같고 id만 다른 중복(동기화 잔재)도 갱신 대상으로
  if(found < 0 && wrapped && wrapped.value && wrapped.value.title){
    var title = String(wrapped.value.title || '').trim();
    var tkey = typeof stepTitleDedupeKey_ === 'function' ? stepTitleDedupeKey_(title) : title.toLowerCase();
    var stage = typeof stepStageNumFromTitle_ === 'function' ? stepStageNumFromTitle_(title) : 0;
    for(i = 0; i < rows.length; i++){
      if(!rows[i]) continue;
      var rowTitle = String(rows[i].title || '').trim();
      if(rowTitle === title){ found = i; break; }
      if(tkey && typeof stepTitleDedupeKey_ === 'function' && stepTitleDedupeKey_(rowTitle) === tkey){ found = i; break; }
      if(stage && typeof stepStageNumFromTitle_ === 'function' && stepStageNumFromTitle_(rowTitle) === stage){ found = i; break; }
    }
  }
  if(missing){
...
function mergeTrackedSyncEntities_(out, local, remote, preferRemote){
  ...
  var mergedTimes = Object.assign({}, out.syncEntityUpdatedAt || {});
  var keys = {};
  ...
  Object.keys(keys).sort(compareSyncEntityKeysForApply_).forEach(function(key){
    ...
    // published/generated/draft 본문은 위에서 merge*Maps_ 로 이미 합침.
    // 여기서 setSyncEntity_ 하면 초안마다 JSON clone → 대용량에서 페이지 정지.
    if(key.indexOf('generated:') === 0 || key.indexOf('published:') === 0 || key.indexOf('draft:') === 0){
      if((key.indexOf('generated:') === 0 || key.indexOf('published:') === 0) && !pickRemote && lVal !== undefined){
        setSyncEntity_(out, key, lVal);
      }
      return;
    }
    setSyncEntity_(out, key, pickRemote ? rVal : lVal);
  });
  out.syncEntityUpdatedAt = mergedTimes;
  // 엔티티 LWW 가 step 키마다 밀어 넣어 같은 제목이 여러 줄이 될 수 있음 → 즉시 정리
  try {
    Object.keys((out.branding && out.branding.subGoalPlans) || {}).forEach(function(catKey){
      dedupeSubGoalPlanStepsInPlace_(out.branding.subGoalPlans[catKey]);
    });
  } catch(eDedupeMerge){}
  return out;
}
...
function getPersistPayload(){
  ...
  if(state._blockAutoSeedTopics && typeof state._blockAutoSeedTopics === 'object'){
    payload._blockAutoSeedTopics = state._blockAutoSeedTopics;
  }
  return payload;
}
...
function normalizeSyncOutbox_(raw, opts){
  ...
  (Array.isArray(raw) ? raw : []).forEach(function(item){
    ...
    var normalized = {
      ...
      key: key,
      value: item.deleted ? null : (skipClone ? item.value : cloneSyncValue_(item.value)),
...
function overlayOutboxOnPayload_(payload){
  ...
  // outbox 없으면 전체 payload deep clone 생략 (대용량 초안에서 부트스트랩 정지 원인)
  if(!ops.length) return payload || {};
  // 초안 본문 전체 JSON clone 금지 — 건드릴 컨테이너만 얕게 복사
  var src = payload || {};
  var out = Object.assign({}, src);
  if(src.published) out.published = Object.assign({}, src.published);
  if(src.generatedOnly) out.generatedOnly = Object.assign({}, src.generatedOnly);
  if(src.extraDraftsByCat){
    out.extraDraftsByCat = {};
    Object.keys(src.extraDraftsByCat).forEach(function(k){
      out.extraDraftsByCat[k] = (src.extraDraftsByCat[k] || []).slice();
    });
  }
  if(src.draftBrandOverrides) out.draftBrandOverrides = Object.assign({}, src.draftBrandOverrides);
  if(src.deletedDraftIds) out.deletedDraftIds = Object.assign({}, src.deletedDraftIds);
  if(src.pinnedDraftIds) out.pinnedDraftIds = Object.assign({}, src.pinnedDraftIds);
  if(src.prompts) out.prompts = cloneSyncValue_(src.prompts);
  if(src.branding) out.branding = cloneSyncValue_(src.branding);
  if(src.opsManual) out.opsManual = cloneSyncValue_(src.opsManual);
  out.syncEntityUpdatedAt = Object.assign({}, src.syncEntityUpdatedAt || {});
...
/** 중복 제거 후 사라진 단계 키의 outbox 잔재 삭제 — 안 지우면 overlay 가 다시 붙임 */
function pruneOrphanSubGoalStepOutbox_(){
  var plans = (state.branding && state.branding.subGoalPlans) || {};
  var keep = {};
  Object.keys(plans).forEach(function(catId){
    ((plans[catId] && plans[catId].steps) || []).forEach(function(s){
      if(s && s.id != null) keep['plan:subgoal:' + catId + ':step:' + String(s.id)] = true;
    });
    keep['plan:subgoal:' + catId + ':meta'] = true;
  });
  var before = (state.syncOutbox || []).length;
  state.syncOutbox = (state.syncOutbox || []).filter(function(op){
    if(!op || !op.key) return false;
    var k = String(op.key);
    if(k.indexOf('plan:subgoal:') !== 0 || k.indexOf(':step:') < 0) return true;
    return !!keep[k];
  });
...
async function plannerGasPushNow_(){
  ...
  try {
    ...
    if(data.result === 'success' && data.payload){
      ...
      // apply 중 마이그레이션이 새로 생긴 경우에만 dirty 유지 (기존 snapshot 플래그로 무한 push 금지)
      var migratedNow = !!state._applyMigrated || !!state._postAdoptMigration;
      state._applyMigrated = false;
      state._postAdoptMigration = false;
...
function captureSyncSettingsDrafts_(){
  var emp = document.getElementById('sync-settings-employee');
  var draft = {
    employee: emp ? String(emp.value || '') : '',
    activeId: (document.activeElement && document.activeElement.id) || '',
    selStart: null,
    selEnd: null
  };
  if(emp && document.activeElement === emp){
    try {
      draft.selStart = emp.selectionStart;
      draft.selEnd = emp.selectionEnd;
    } catch(e0){}
  }
  return draft;
}
function restoreSyncSettingsDrafts_(draft){
  if(!draft) return;
  var emp = document.getElementById('sync-settings-employee');
  if(!emp) return;
  if(draft.employee != null) emp.value = draft.employee;
  if(draft.activeId === 'sync-settings-employee'){
    try {
      emp.focus({ preventScroll: true });
      if(draft.selStart != null && draft.selEnd != null){
        emp.setSelectionRange(draft.selStart, draft.selEnd);
      }
    } catch(e1){
      try { emp.focus(); } catch(e2){}
    }
  }
}
function rememberSyncEmployeeDraft_(value){
  try { sessionStorage.setItem('ht_sync_employee_draft', String(value == null ? '' : value)); } catch(e){}
}
function readSyncEmployeeDraft_(){
  try { return sessionStorage.getItem('ht_sync_employee_draft') || ''; } catch(e){ return ''; }
}
function bindSyncSettingsEmployeeDraftOnce_(){
  var emp = document.getElementById('sync-settings-employee');
  if(!emp || emp.getAttribute('data-draft-bound') === '1') return;
  emp.setAttribute('data-draft-bound', '1');
  emp.addEventListener('input', function(){
    rememberSyncEmployeeDraft_(emp.value);
  });
  emp.addEventListener('blur', function(){
    var next = String(emp.value || '').trim();
    rememberSyncEmployeeDraft_(next);
    if(next && isPlannerEmployeeEmailValid_(next) && next !== getPlannerEmployeeId_()){
      setPlannerEmployeeId_(next);
    }
  });
}
function refreshSyncStatusBodyHtml_(info){
  var body = document.getElementById('sync-status-body');
  var overlay = document.getElementById('sync-status-overlay');
  if(!body || !overlay || !overlay.classList.contains('open')) return;
  var draft = captureSyncSettingsDrafts_();
  // 모바일: 이메일 입력 중 innerHTML 교체 → 키보드 내려감 + 입력 초기화
  if(draft.activeId === 'sync-settings-employee'){
    rememberSyncEmployeeDraft_(draft.employee);
    var sum = body.querySelector('.sync-status-summary');
    if(sum && info && info.summary) sum.textContent = info.summary;
    return;
  }
  if(draft.employee) rememberSyncEmployeeDraft_(draft.employee);
  body.innerHTML = renderSyncStatusBodyHTML_(info || getSyncStatusInfo_());
  var prefer = draft.employee || readSyncEmployeeDraft_();
  if(prefer){
    restoreSyncSettingsDrafts_({
      employee: prefer,
...
function renderSyncStatusBodyHTML_(info){
  ...
  try {
    var draftEmail = readSyncEmployeeDraft_();
    if(draftEmail && draftEmail !== email) email = draftEmail;
  } catch(eDraft){}
  var emailOk = isPlannerEmployeeEmailValid_(email);
...
window.openSyncStatusModal_ = async function(){
  ...
  waitNote.className = 'sync-status-row-meta sync-status-wait-note';
  waitNote.style.padding = '4px 2px';
  waitNote.textContent = '서버 상태 확인 중…';
...
function setPlannerBootstrapUi_(mode, message){
  ...
  var titleEl = document.getElementById('sync-bootstrap-title');
  var appRoot = document.getElementById('planner-app-root');
  if(!overlay) return;
  var locked = mode !== 'done';
  var wasFailed = overlay.classList.contains('failed');
  ...
  // 입력 중인 값은 모드 전환 전에 보존 (display:none 되어도 유지)
  var liveEmployee = employeeInput ? String(employeeInput.value || '') : '';
  var liveToken = tokenInput ? String(tokenInput.value || '') : '';
  var editingBootstrap = !!(document.activeElement === employeeInput || document.activeElement === tokenInput);
  if(liveEmployee){
    try { sessionStorage.setItem('ht_bootstrap_employee_draft', liveEmployee); } catch(eD0){}
  }

  if(appRoot){
    // 입력 중이면 inert 토글로 키보드가 내려가지 않게 함
    if(!editingBootstrap){
  ...
  if(titleEl){
    if(mode === 'failed') titleEl.textContent = '서버 동기화 필요';
    else if(mode === 'done') titleEl.textContent = '서버 데이터 확인 중';
    else titleEl.textContent = '서버 데이터 확인 중';
  }
  if(msg) msg.textContent = message || '';
  var contBtn = document.getElementById('sync-bootstrap-continue-local');
  if(contBtn && mode !== 'done') contBtn.style.display = 'block';
  if(mode === 'failed'){
    var draftEmp = '';
    try { draftEmp = sessionStorage.getItem('ht_bootstrap_employee_draft') || ''; } catch(eD1){}
    var preferEmp = liveEmployee || draftEmp || getPlannerEmployeeId_();
    var preferTok = liveToken || getPlannerSyncToken_();
    // 포커스 중·이미 입력 중이면 value를 덮어쓰지 않음
    if(employeeInput && document.activeElement !== employeeInput){
      if(!wasFailed || !String(employeeInput.value || '').trim()){
        employeeInput.value = preferEmp;
      }
    }
    if(tokenInput && document.activeElement !== tokenInput){
      if(!wasFailed || !String(tokenInput.value || '').trim()){
        tokenInput.value = preferTok;
      }
    }
    if(!wasFailed && !editingBootstrap){
      setTimeout(function(){
        if(!employeeInput) return;
        var active = document.activeElement;
        if(active === employeeInput || active === tokenInput) return;
        try { employeeInput.focus({ preventScroll: true }); } catch(eF){
          try { employeeInput.focus(); } catch(eF2){}
        }
      }, 0);
    }
    bindBootstrapEmployeeDraftOnce_();
  }
}
function bindBootstrapEmployeeDraftOnce_(){
  var employeeInput = document.getElementById('sync-bootstrap-employee');
  if(!employeeInput || employeeInput.getAttribute('data-draft-bound') === '1') return;
  employeeInput.setAttribute('data-draft-bound', '1');
  employeeInput.addEventListener('input', function(){
    try { sessionStorage.setItem('ht_bootstrap_employee_draft', employeeInput.value || ''); } catch(e){}
  });
  employeeInput.addEventListener('blur', function(){
    var next = String(employeeInput.value || '').trim();
    try { sessionStorage.setItem('ht_bootstrap_employee_draft', next); } catch(e){}
    if(next && isPlannerEmployeeEmailValid_(next)) setPlannerEmployeeId_(next);
  });
}
function isPlannerPayloadHeavy_(payload){
  if(!payload || typeof payload !== 'object') return false;
  var gen = Object.keys(payload.generatedOnly || {}).length;
  var pub = Object.keys(payload.published || {}).length;
  var drafts = 0;
  Object.keys(payload.extraDraftsByCat || {}).forEach(function(k){
    drafts += (payload.extraDraftsByCat[k] || []).length;
  });
  // 임계값을 낮춤: 리:얼 무브먼트 규모면 즉시 가벼운 경로
  return (gen + pub) >= 8 || drafts >= 40 || gen >= 6 || pub >= 6;
}
/** 로컬 저장 문자열 크기 기준 — 파싱 전에 대용량 판별 */
function isPlannerLocalStoreHeavy_(){
  try {
    if(plannerLastDiskPayloadStr_ && plannerLastDiskPayloadStr_.length > 80000) return true;
    var raw = localStorage.getItem(SK) || '';
    if(isPlannerLsIdbStubRaw_(raw)){
      try {
        var j = JSON.parse(raw);
        return !!(j && (parseInt(j._payloadChars, 10) || 0) > 80000);
      } catch(e0){ return true; }
    }
    return raw.length > 80000;
  } catch(e){ return false; }
}
function unlockPlannerBootstrapLocal_(opts){
  opts = opts || {};
  plannerSyncBootstrapReady_ = true;
  plannerSyncBootstrapRunning_ = false;
  state._bootstrapDeferHeavy = false;
  setPlannerBootstrapUi_('done', '');
  if(opts.toast && typeof setAppToast === 'function'){
    setAppToast(opts.toast, { duration: opts.toastMs || 5500, variant: 'ok' });
  }
  // 부팅 직후 이중 render 로 멈추지 않게 — 이미 onload에서 그렸으면 가볍게만
  if(!opts.skipRender){
    try { updateSyncStatusUI_(); } catch(eUi0){}
    if(!state._bootFirstPaintDone){
      try { renderTabs(); renderMain(); } catch(eUi){}
    }
  } else {
    try { updateSyncStatusUI_(); } catch(eUi2){}
  }
  setTimeout(function(){
    try { runDeferredHeavyPlanRepairs_(); } catch(eDef){}
  }, 80);
}
/** 부트스트랩용 가벼운 맞춤 — 대용량 초안 통째 adopt/merge 로 UI를 잠그지 않음 */
function reconcileBootstrapLight_(localPayload, remotePayload, remoteRevision){
  remoteRevision = parseInt(remoteRevision, 10) || getPayloadRevision_(remotePayload);
  ensureOutboxFromLegacyDirty_(localPayload || getPersistPayload());
  var heavy = isPlannerPayloadHeavy_(remotePayload) || isPlannerPayloadHeavy_(localPayload || getPersistPayload());
  if(heavy){
    // 로컬로 편집을 열고, 서버 revision만 기록한 뒤 백그라운드 푸시/수동 동기화에 맡김
    if(remoteRevision > (parseInt(state.syncRevision, 10) || 0)){
      state.syncRevision = remoteRevision;
    }
    if(!(state.syncBaseEntityUpdatedAt && Object.keys(state.syncBaseEntityUpdatedAt).length)){
    ...
    return 'deferred-heavy';
  }
  return reconcileLocalWithServerPayload_(localPayload, remotePayload, remoteRevision);
}
...
function yieldPlannerBootstrapUi_(message){
  if(message) setPlannerBootstrapUi_('loading', message);
  return new Promise(function(resolve){ setTimeout(resolve, 0); });
}
async function runPlannerBootstrapInBackground_(){
  try {
    if(!isPlannerEmployeeEmailValid_(getPlannerEmployeeId_())) return;
    // 대용량 로컬: 서버 전체 pull + JSON.parse 자체가 수십 초~1분 UI 정지 → 생략
    if(isPlannerLocalStoreHeavy_() || isPlannerPayloadHeavy_(getPersistPayload())){
      if(hasPendingLocalSyncChanges_()){
        state.syncNeedsSnapshot = true;
        state.syncDirty = true;
        schedulePlannerGasPush_(true);
    ...
    if(bootstrapData.result === 'success' && bootstrapData.payload){
      ...
      if(isPlannerPayloadHeavy_(remotePayload)){
        if(remoteRevision > (parseInt(state.syncRevision, 10) || 0)){
          state.syncRevision = remoteRevision;
        }
        if(!(state.syncBaseEntityUpdatedAt && Object.keys(state.syncBaseEntityUpdatedAt).length)){
        ...
        reconcileBootstrapLight_(getPersistPayload(), remotePayload, remoteRevision);
        save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true, forceWrite: true });
      }
  ...
  // (대용량 응답 파싱 중에는 setTimeout 도 밀려 「확인 중」이 1분씩 남음)
    unlockPlannerBootstrapLocal_({
      toast: isPlannerLocalStoreHeavy_()
        ? '이 기기 데이터로 바로 열었어요. 서버 동기화는 백그라운드에서 진행합니다.'
        : '편집을 먼저 열었어요. 서버 확인은 백그라운드에서 이어갑니다.',
      toastMs: 4200,
      skipRender: true
    });
  plannerSyncBootstrapRunning_ = false;
  state._bootstrapDeferHeavy = false;
  setTimeout(function(){ runPlannerBootstrapInBackground_(); }, 120);
}
/** 서버 병합이 너무 길 때 — 이 기기 저장본으로 편집을 먼저 연다 */
window.continuePlannerWithLocalData_ = function(){
  try {
    state._bootstrapUnlockedEarly = true;
    unlockPlannerBootstrapLocal_({
...
function keepLocalConflictsWithoutMerge_(remote, serverRevision, conflicts){
  try {
    ...
    if(remote && remote.syncEntityUpdatedAt){
      state.syncBaseEntityUpdatedAt = Object.assign({}, remote.syncEntityUpdatedAt);
...
/** @deprecated keepLocalConflictsWithoutMerge_ 사용 — 호환용 래퍼 */
function autoResolveConflictsKeepLocal_(local, remote, serverRevision, conflicts){
  keepLocalConflictsWithoutMerge_(remote, serverRevision, conflicts);
}
function renderPlannerConflictUi_(pending){
  // 다른 오버레이가 클릭을 가로채지 못하게 전부 닫음
  ['sync-bootstrap-overlay', 'api-modal', 'drive-oauth-busy', 'sync-status-overlay'].forEach(function(id){
  ...
  if(list){
    ...
    var conflicts = (pending.conflicts || []).slice(0, 12);
    var hidden = Math.max(0, (pending.conflicts || []).length - conflicts.length);
    var remoteBy = remote.lastModifiedBy ? ' · ' + remote.lastModifiedBy : '';
    var localBy = getPlannerEmployeeId_() ? ' · ' + getPlannerEmployeeId_() : '';
    var html = conflicts.map(function(conflict, i){
      var key = String(conflict.key || '');
      ...
      var hint = preferLocal
        ? '<div class="sync-conflict-name-sub" style="font-size:11px;color:#B45309;margin-top:4px;">이 기기 초안 본문이 있어요. 「이 기기」 권장.</div>'
        : '';
    ...
    if(hidden > 0){
      html += '<div style="font-size:12px;color:#92400E;padding:8px 4px;">외 ' + hidden +
        '건은 목록에서 생략됐어요. 「전부 이 기기로 적용」을 누르면 함께 맞춥니다.</div>';
    }
    list.innerHTML = html;
  }
  bindPlannerConflictButtons_();
  var applyBtnReset = document.getElementById('sync-conflict-apply');
  if(applyBtnReset){
    applyBtnReset.disabled = false;
    applyBtnReset.textContent = '선택 내용 적용 후 동기화';
  }
...
function bindPlannerConflictButtons_(){
  ...
  if(applyBtn && !applyBtn._boundConflictApply){
    applyBtn._boundConflictApply = true;
    applyBtn.addEventListener('click', function(ev){
      ev.preventDefault();
      ev.stopPropagation();
      applyPlannerConflictChoices_();
    }, true);
  }
  var keepBtn = document.getElementById('sync-conflict-keep-local');
  if(keepBtn && !keepBtn._boundConflictKeep){
    keepBtn._boundConflictKeep = true;
    keepBtn.addEventListener('click', function(ev){
      ev.preventDefault();
      ev.stopPropagation();
      applyAllConflictsKeepLocal_();
    }, true);
  }
}
window.applyAllConflictsKeepLocal_ = function(){
  var pending = plannerSyncConflictPending_;
  if(!pending){
    if(typeof setAppToast === 'function') setAppToast('적용할 충돌이 없어요.', { duration: 3500, variant: 'err' });
    return;
  }
  var keepBtn = document.getElementById('sync-conflict-keep-local');
  var applyBtn = document.getElementById('sync-conflict-apply');
...
window.retryPlannerBootstrapSync_ = function(){
  ...
  var employeeInput = document.getElementById('sync-bootstrap-employee');
  var email = employeeInput ? String(employeeInput.value || '').trim() : getPlannerEmployeeId_();
  if(!isPlannerEmployeeEmailValid_(email)){
    setPlannerBootstrapUi_('failed',
      '올바른 직원 이메일을 입력해 주세요.\n예: name@company.com\n\n이메일이 없으면 「이 기기 데이터로 열기」로 편집할 수 있어요.');
    if(typeof setAppToast === 'function'){
      setAppToast('이메일 형식을 확인해 주세요. (@와 도메인이 필요합니다)', { duration: 4500, variant: 'err' });
    }
    if(employeeInput) try { employeeInput.focus(); } catch(e){}
    return;
  }
  setPlannerEmployeeId_(email);
  var localRaw = '';
  try { localRaw = plannerLastDiskPayloadStr_ || localStorage.getItem(SK) || ''; } catch(e){}
  if(isPlannerLsIdbStubRaw_(localRaw)) localRaw = plannerLastDiskPayloadStr_ || '';
  if(localRaw.length > 200000){
    unlockPlannerBootstrapLocal_({
      toast: '이메일을 저장했고, 데이터가 많아 이 기기 기준으로 바로 열었어요.'
    });
    setTimeout(function(){ runPlannerBootstrapInBackground_(); }, 100);
    return;
  }
...
function openPlannerOutboxConflict_(serverConflicts, remote, serverRevision){
  ...
  var localEntities = collectSyncEntities_(localPayload, { skipEnsureIds: true });
  var outboxByKey = {};
  ...
  var conflicts = (serverConflicts || []).map(function(conflict){
    ...
    return {
      key: conflict.key,
      // deep clone 생략 — 대용량에서 모달 오픈 자체가 멈춤
      local: op ? (op.deleted ? undefined : op.value) : localEntities[conflict.key],
      remote: conflict.serverDeleted ? undefined : conflict.serverValue,
...
window.applyPlannerConflictChoices_ = async function(){
  var pending = plannerSyncConflictPending_;
  if(!pending){
    if(typeof setAppToast === 'function') setAppToast('적용할 충돌 항목이 없어요. 화면을 새로고침해 주세요.', { duration: 4000, variant: 'err' });
    return;
  }
  var applyBtn = document.getElementById('sync-conflict-apply');
  if(applyBtn){
  ...
  var choices = (pending.conflicts || []).map(function(conflict, i){
    var picked = document.querySelector('input[name="sync-conflict-' + i + '"]:checked');
    var useLocal = pending._forceAllLocal || !picked || picked.value === 'local';
    var key = String(conflict.key || '');
    ...
    return { conflict: conflict, useLocal: useLocal };
  });
  var snap = pending;
  plannerSyncConflictPending_ = null;
  ...
  try {
    var heavy = isPlannerLocalStoreHeavy_() || isPlannerPayloadHeavy_(snap.local) || isPlannerPayloadHeavy_(snap.remote);
    ...
    choices.forEach(function(choice){
      ...
      setSyncEntity_(getPersistPayload(), key, conflict.remote);
      if(!state.syncEntityUpdatedAt) state.syncEntityUpdatedAt = {};
      state.syncEntityUpdatedAt[key] = conflict.remoteAt || new Date().toISOString();
    ...
    var rev = parseInt(snap.serverRevision, 10) || state.syncRevision || 0;
    if(rev > (parseInt(state.syncRevision, 10) || 0)) state.syncRevision = rev;
    if(snap.remote && snap.remote.syncEntityUpdatedAt){
      state.syncBaseEntityUpdatedAt = Object.assign({}, snap.remote.syncEntityUpdatedAt);
    }
    state.syncDirty = Object.keys(state.syncDirtyEntityKeys || {}).length > 0;
    state.syncNeedsSnapshot = true;
    if(!plannerSyncBootstrapReady_) plannerSyncBootstrapReady_ = true;
    save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true });
    if(typeof setAppToast === 'function'){
      setAppToast('선택 내용을 이 기기에 반영했어요. 서버 저장은 백그라운드에서 이어갑니다.', { duration: 4500, variant: 'ok' });
    }
    schedulePlannerGasPush_(true);
  } catch(err){
    ...
    if(applyBtn && document.body.contains(applyBtn)){
      applyBtn.disabled = false;
      applyBtn.textContent = '선택 내용 적용 후 동기화';
    }
  }
};
...
function applyPersistPayload(s, opts){
  ...
  if(Array.isArray(s._syncOutbox)){
    state.syncOutbox = normalizeSyncOutbox_(s._syncOutbox, { skipClone: fastBoot });
  }
  ...
  if(s._blockAutoSeedTopics && typeof s._blockAutoSeedTopics === 'object'){
    state._blockAutoSeedTopics = Object.assign({}, state._blockAutoSeedTopics || {}, s._blockAutoSeedTopics);
  }
  ...
  purgeDeletedDraftsFromCatalog_();
  if(!fastBoot) applyBuiltinDraftBrandSeeds_();
  if(!(opts && opts.skipRestorePending) && !fastBoot){
    restorePendingSubGoalPlan_();
    restorePendingYearPlan_();
  }
  // 빠른 부팅: 마이그레이션·id 재작성·save 전부 미룸 (응답 없는 페이지 방지)
  if(fastBoot){
    state._pendingHeavyRepair = true;
    state._pendingBootMigrations = true;
    return;
  }
  ...
  if(migrated){
    ...
    state._applyMigrated = true;
    save(migOpts);
  }
}
function runDeferredHeavyPlanRepairs_(){
  if(!state._pendingHeavyRepair) return false;
  state._pendingHeavyRepair = false;
  var changed = false;
  try {
    if(repairAllIncompleteSubGoalPlans_()) changed = true;
    if(repairAllSubGoalDraftAssignments_()) changed = true;
    CATEGORIES.forEach(function(cat, i){ ensureUserAddedDraftsInMisc_(i); });
  } catch(eRep){ console.warn('[plan repair]', eRep); }
  if(changed){
    state.syncDirty = true;
    state.syncNeedsSnapshot = true;
    save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true, forceWrite: true });
...
function pruneGeneratedDraftsForQuota_(opts){
  ...
  var onlyIds = opts.onlyDraftIds || null; // { id: true } 이면 해당 id만
  var onlyCatId = opts.onlyCatId;
  if(onlyCatId != null && !onlyIds){
    onlyIds = {};
    var cat = CATEGORIES[onlyCatId];
    (cat && cat.drafts || []).forEach(function(d){
      if(d && d.id) onlyIds[d.id] = true;
    });
  }
  var removed = 0;
  var ids = Object.keys(state.generatedOnly || {});
  // 큰 본문부터 제거
  ids.sort(function(a, b){
    var sa = 0, sb = 0;
    try { sa = JSON.stringify(state.generatedOnly[a] || '').length; } catch(e1){}
    try { sb = JSON.stringify(state.generatedOnly[b] || '').length; } catch(e2){}
    return sb - sa;
  });
  ids.forEach(function(id){
    if(onlyIds && !onlyIds[id]) return;
    if(keepPublished && state.published && state.published[id] && state.published[id].content) return;
    if(state.generatedOnly[id]){
      delete state.generatedOnly[id];
      removed++;
    }
  });
  // sync outbox의 generated: 항목도 줄임
  if(removed){
    state.syncOutbox = (state.syncOutbox || []).filter(function(op){
      if(!op || !op.key) return true;
      var k = String(op.key || '');
      if(k.indexOf('generated:') !== 0) return true;
      var gid = k.slice('generated:'.length);
      if(onlyIds && !onlyIds[gid]) return true;
      return false;
    });
    rebuildDirtyStateFromOutbox_();
    state.syncNeedsSnapshot = true;
...
/**
 * 한도 초과 상황에서 반드시 성공하도록 가볍게 저장.
 * IndexedDB 가 있으면 초안을 지우지 않고 먼저 저장 시도.
 * opts.onlyCatId 가 있으면 우선 그 탭 초안만 비움 (다른 탭 보호).
 */
function persistPlannerUnderQuota_(opts){
  opts = opts || {};
  var stripped = 0;
  var scopeCat = opts.onlyCatId;
  var canIdb = typeof indexedDB !== 'undefined';
  ...
  function tryWrite_(){
    return !!save({
      ...
      skipGasPush: true,
      skipMarkDirty: true,
      skipEntityStamp: true,
      forceWrite: true,
      _quotaRetried: true
    });
  }

  // 0차: IndexedDB 가능하면 삭제 없이 저장 (한도 복구 버튼이 초안을 날리던 문제 방지)
  if(canIdb && !opts.forcePrune){
    var ok0 = false;
    try { ok0 = tryWrite_(); } catch(e0w){ ok0 = false; }
    if(ok0){
      plannerStorageWriteFailed_ = false;
      removeQuotaRecoveryFab_();
      if(!opts.skipPush) schedulePlannerGasPush_(false);
      return { ok: true, stripped: 0 };
    }
  }

  // 1차: 지정 탭만 (또는 전체)
  try {
    stripped += pruneGeneratedDraftsForQuota_({
      keepPublished: true,
      onlyCatId: scopeCat
    }) || 0;
  } catch(e0){}
  // outbox 에 초안 본문 clone 이 남아 있으면 한도를 계속 넘김
  try {
    var beforeOut = (state.syncOutbox || []).length;
    var allowIds = null;
    if(scopeCat != null){
      allowIds = {};
      var cat0 = CATEGORIES[scopeCat];
      (cat0 && cat0.drafts || []).forEach(function(d){ if(d && d.id) allowIds[d.id] = true; });
    }
    state.syncOutbox = (state.syncOutbox || []).filter(function(op){
      if(!op || !op.key) return false;
      var k = String(op.key);
      if(k.indexOf('generated:') === 0){
        var gid = k.slice('generated:'.length);
        if(allowIds && !allowIds[gid]) return true; // 다른 탭 outbox 유지
        return false;
      }
      if(k.indexOf('published:') === 0){
        var pid = k.slice('published:'.length);
        if(allowIds && !allowIds[pid]) return true;
        return false;
      }
      return true;
    });
    if((state.syncOutbox || []).length !== beforeOut){
      stripped += Math.max(1, beforeOut - (state.syncOutbox || []).length);
    }
    state.syncNeedsSnapshot = true;
    state.syncDirty = true;
    rebuildDirtyStateFromOutbox_();
  } catch(e1){}
  // deletedDraftIds 가 수천 개면 문자열만으로도 커짐 → 최근 400개만 유지
  try {
    var del = state.deletedDraftIds || {};
    var dkeys = Object.keys(del);
    if(dkeys.length > 400){
      dkeys.sort(function(a, b){ return String(del[b] || '').localeCompare(String(del[a] || '')); });
      var keepDel = {};
      dkeys.slice(0, 400).forEach(function(k){ keepDel[k] = del[k]; });
      state.deletedDraftIds = keepDel;
      stripped++;
    }
  } catch(e2){}

  var ok = false;
  try { ok = tryWrite_(); } catch(e3){ ok = false; }
  if(!ok && scopeCat != null){
    // 2차(탭 한정): 그 탭 generatedOnly 잔여분 강제 삭제
    try {
      var cat1 = CATEGORIES[scopeCat];
      (cat1 && cat1.drafts || []).forEach(function(d){
        if(d && d.id && state.generatedOnly[d.id]){
          delete state.generatedOnly[d.id];
          stripped++;
        }
      });
    } catch(e4a){}
    try { ok = tryWrite_(); } catch(e4b){ ok = false; }
  }
  if(!ok && opts.allowAllTabs !== false){
    // 3차: 그래도 실패하면 전체 미발행 초안 정리 (최후 수단)
    try {
      stripped += pruneGeneratedDraftsForQuota_({ keepPublished: true }) || 0;
      state.syncOutbox = (state.syncOutbox || []).filter(function(op){
        if(!op || !op.key) return false;
        var k = String(op.key);
        return k.indexOf('generated:') !== 0 && k.indexOf('published:') !== 0;
      });
      state.syncNeedsSnapshot = true;
      state.syncDirty = true;
      rebuildDirtyStateFromOutbox_();
    } catch(e5){}
    try { ok = tryWrite_(); } catch(e6){ ok = false; }
  }
  if(!ok){
    // 4차: generatedOnly 전부 삭제
    try {
      var nGen = Object.keys(state.generatedOnly || {}).length;
      state.generatedOnly = {};
      stripped += nGen;
      state.syncOutbox = [];
      state.syncNeedsSnapshot = true;
    ...
    try { ok = tryWrite_(); } catch(e8){ ok = false; }
  }
  if(!ok){
    // 5차: 미발행 published.content 도 비움 (발행 완료 기록은 유지)
    try {
      Object.keys(state.published || {}).forEach(function(id){
        var p = state.published[id];
        if(!p || typeof p !== 'object') return;
        if(p.date || (p.tabPublished && Object.keys(p.tabPublished).length)) return;
        if(p.content){ delete p.content; stripped++; }
      });
    } catch(e9){}
    try { ok = tryWrite_(); } catch(e10){ ok = false; }
  }
  if(ok){
    plannerStorageWriteFailed_ = false;
    removeQuotaRecoveryFab_();
  ...
  return { ok: ok, stripped: stripped };
}

function writeLocalStoragePayloadString_(payloadStr){
  payloadStr = String(payloadStr || '');
  plannerLastDiskPayloadStr_ = payloadStr;

  function failUi_(bytesHint){
    plannerStorageWriteFailed_ = true;
    var mb = bytesHint ? (bytesHint / (1024 * 1024)).toFixed(1) : '?';
    var tip =
      '브라우저 저장에 실패했어요.\n' +
      '현재 플래너 데이터 약 ' + mb + 'MB.\n' +
      '미발행 AI 초안 본문을 비우면 다시 저장할 수 있어요.';
    if (typeof setAppToast === 'function') {
      setAppToast(tip, { duration: 9000, variant: 'err' });
    } else {
      alert(tip);
    }
    if(plannerSyncBootstrapReady_){
      ensureQuotaRecoveryFab_();
      return;
    }
    setPlannerBootstrapUi_('failed',
      '브라우저 저장에 실패했어요.\n' +
      '「이 기기 데이터로 열기」 후, 안 쓰는 생성 초안을 지워 주세요.\n' +
      '또는 아래 「초안 본문 비우고 저장」을 누르세요.');
    var cont = document.getElementById('sync-bootstrap-continue-local');
    if(cont) cont.style.display = 'block';
    ensureQuotaRecoveryButton_();
  }

  // 1) IndexedDB에 전체 본문 저장 (한도 큼)
  var idbOkLikely = typeof indexedDB !== 'undefined';
  if(idbOkLikely){
    schedulePlannerIdbWrite_(payloadStr).then(function(){
      if(!plannerIdbWriteFailed_){
        plannerStorageWriteFailed_ = false;
        try { removeQuotaRecoveryFab_(); } catch(eFab){}
      } else if(plannerStorageWriteFailed_){
        try { ensureQuotaRecoveryFab_(); } catch(eFabFail){}
      }
    });
  }

  // 2) localStorage 미러 — 작으면 전체, 크거나 한도면 스텁만
  var preferFull = payloadStr.length < (1.6 * 1024 * 1024);
  try {
    if(preferFull){
      localStorage.setItem(SK, payloadStr);
    } else {
      localStorage.setItem(SK, buildPlannerLsMirrorStub_(payloadStr));
      plannerUsingIdbPrimary_ = true;
      if(typeof setAppToast === 'function' && !state._idbMirrorToast){
        state._idbMirrorToast = true;
        setAppToast('데이터가 커서 IndexedDB에 보관합니다. (브라우저 사이트 한도 우회)', { duration: 6500, variant: 'ok' });
      }
    }
    plannerStorageWriteFailed_ = false;
    return true;
  } catch(e0){
    // 한도 초과 → 스텁만 남기고 IDB에 위임
    try {
      localStorage.setItem(SK, buildPlannerLsMirrorStub_(payloadStr));
      plannerUsingIdbPrimary_ = true;
      plannerStorageWriteFailed_ = false;
      if(typeof setAppToast === 'function' && !state._idbQuotaToast){
        state._idbQuotaToast = true;
        setAppToast('localStorage 한도를 넘어 IndexedDB에 저장했어요. 초안을 지울 필요 없이 계속 쓸 수 있어요.', { duration: 7500, variant: 'ok' });
      }
      try { removeQuotaRecoveryFab_(); } catch(eFab2){}
      return true;
    } catch(e1){
      // 스텁조차 실패 — IndexedDB만으로 진행
      if(idbOkLikely){
        plannerUsingIdbPrimary_ = true;
        plannerStorageWriteFailed_ = false;
        try { removeQuotaRecoveryFab_(); } catch(eFab3){}
        return true;
      }
      // IDB도 없으면 예전처럼 초안 비우고 재시도
      var pruned = 0;
      try { pruned = pruneGeneratedDraftsForQuota_({ keepPublished: true }); } catch(ePr){}
      if(pruned > 0){
        try {
          var retryStr = JSON.stringify(getLocalStoragePayload());
          plannerLastDiskPayloadStr_ = retryStr;
          localStorage.setItem(SK, retryStr);
          plannerStorageWriteFailed_ = false;
          if(typeof setAppToast === 'function'){
            setAppToast('저장 한도 때문에 미발행 AI 초안 본문 ' + pruned + '건을 비우고 저장했어요.', { duration: 8000, variant: 'ok' });
          }
          return true;
        } catch(e2){}
      }
      failUi_(payloadStr.length * 2);
      return false;
    }
  }
}
function ensureQuotaRecoveryButton_(){
  var box = document.querySelector('#sync-bootstrap-overlay .sync-bootstrap-box');
  if(!box || document.getElementById('sync-bootstrap-prune-quota')) return;
  var btn = document.createElement('button');
  btn.type = 'button';
  btn.id = 'sync-bootstrap-prune-quota';
  btn.className = 'modal-btn';
  btn.style.marginTop = '8px';
  btn.style.background = '#FEF3C7';
  btn.style.color = '#92400E';
  btn.textContent = '미발행 초안 본문 비우고 저장';
  btn.onclick = function(){ pruneQuotaAndContinue_(); };
  box.appendChild(btn);
}
function removeQuotaRecoveryFab_(){
  var wrap = document.getElementById('planner-quota-prune-fab');
  if(wrap) wrap.remove();
  try { document.body.classList.remove('planner-quota-fab-visible'); } catch(eCls){}
}
function setQuotaFabBusy_(busy){
  var wrap = document.getElementById('planner-quota-prune-fab');
  if(!wrap) return;
  var action = wrap.querySelector('[data-quota-action]');
  if(action){
...
/** 편집 중 한도 초과 시 — 부트스트랩을 다시 잠그지 않고 하단 복구 바만 표시 */
function ensureQuotaRecoveryFab_(){
  // IndexedDB 로 정상 저장 중이면 한도 바를 띄우지 않음
  if(plannerUsingIdbPrimary_ && !plannerIdbWriteFailed_ && !plannerStorageWriteFailed_){
    removeQuotaRecoveryFab_();
    return;
  }
  var existing = document.getElementById('planner-quota-prune-fab');
  // 예전: button#planner-quota-prune-fab (disabled 로 죽은 경우) → 새 구조로 교체
  if(existing && (!existing.classList.contains('planner-quota-fab') || !existing.querySelector('[data-quota-action]'))){
    try { existing.remove(); } catch(eOld){}
    existing = null;
  }
  if(existing){
    // 주제 삭제 등으로 busy 만 남은 경우 — 다시 살림
    if(!state._quotaPruneBusy) setQuotaFabBusy_(false);
    try { document.body.classList.add('planner-quota-fab-visible'); } catch(e0){}
    return;
  }
  var wrap = document.createElement('div');
  wrap.id = 'planner-quota-prune-fab';
  wrap.className = 'planner-quota-fab';
  wrap.setAttribute('role', 'status');

  var action = document.createElement('button');
  action.type = 'button';
  action.className = 'planner-quota-fab-action';
  action.setAttribute('data-quota-action', '1');
  action.setAttribute('aria-label', '저장 복구');
  action.textContent = '저장 복구 · 초안 정리 후 다시 저장';
  action.onclick = function(ev){
    if(ev){ ev.preventDefault(); ev.stopPropagation(); }
    pruneQuotaAndContinue_();
  };

  var dismiss = document.createElement('button');
  dismiss.type = 'button';
  dismiss.className = 'planner-quota-fab-dismiss';
  ...
  dismiss.onclick = function(ev){
    ...
    removeQuotaRecoveryFab_();
    if(typeof setAppToast === 'function'){
      setAppToast('안내를 닫았어요. 저장이 안 되면 새로고침 후 다시 정리해 주세요.', { duration: 4200, variant: 'ok' });
    }
  };
  ...
  try { document.body.classList.add('planner-quota-fab-visible'); } catch(e1){}
}
window.pruneQuotaAndContinue_ = function(){
  if(state._quotaPruneBusy) return;
  state._quotaPruneBusy = true;
  setQuotaFabBusy_(true);
  if(typeof setAppToast === 'function'){
    setAppToast('저장 복구 중…', { duration: 5000, variant: 'ok' });
  }
  // 메인 스레드 블로킹 전에 UI가 먼저 그리도록
  setTimeout(function(){
    var result = { ok: false, stripped: 0 };
    try {
      // IndexedDB 우선 — forcePrune 없이 먼저 시도
      result = persistPlannerUnderQuota_({
        skipPush: false,
        onlyCatId: state.currentCat,
        allowAllTabs: true
      }) || result;
    } catch(err){
      console.warn('[한도 복구]', err);
      result = { ok: false, stripped: 0 };
    }
    state._quotaPruneBusy = false;
    if(result.ok){
      removeQuotaRecoveryFab_();
      try {
        unlockPlannerBootstrapLocal_({
          toast: result.stripped
            ? ('저장을 복구했습니다 (정리 ' + result.stripped + '건).')
            : '저장을 복구했어요. 초안은 그대로 두었습니다.'
        });
      } catch(eUnlock){
        if(typeof setAppToast === 'function'){
          setAppToast(
      ...
      setQuotaFabBusy_(false);
      ensureQuotaRecoveryFab_();
      if(typeof setAppToast === 'function'){
        setAppToast('아직도 저장에 실패했어요. 「초안 없는 주제 삭제」 후 다시 누르거나, 브라우저 사이트 데이터를 정리해 주세요.', { duration: 7500, variant: 'err' });
      }
    }
  }, 80);
};
...
function persistSyncMetaIfNeeded_(){
  ...
  if(plannerLastDiskPayloadStr_ && plannerLastDiskPayloadStr_ === payloadStr) return true;
  var prevStr = plannerLastDiskPayloadStr_;
  if(!prevStr){
    try {
      var ls = localStorage.getItem(SK);
      if(ls && !isPlannerLsIdbStubRaw_(ls)) prevStr = ls;
    } catch(ePrev){}
  }
...
function save(opts) {
  ...
  if(o.skipEntityStamp && o.skipMarkDirty){
    var metaPayload = JSON.stringify(getLocalStoragePayload());
    var metaOk = writeLocalStoragePayloadString_(metaPayload);
    if(metaOk) scheduleSyncStatusUiRefresh_();
    return metaOk;
  }
  // 이전 저장 실패 직후: fingerprint 생략하고 바로 기록 (한도 FAB·복구 경로)
  if(plannerStorageWriteFailed_ && !o._quotaRetried){
    ...
    var emergencyOk = writeLocalStoragePayloadString_(emergencyStr);
    if(!emergencyOk){
      try {
        var pr = persistPlannerUnderQuota_({ skipPush: !!o.skipGasPush, allowAllTabs: true });
        emergencyOk = !!(pr && pr.ok);
      } catch(eQ){ emergencyOk = false; }
    }
    if(emergencyOk && plannerSyncBootstrapReady_ && !plannerSyncConflictPending_){
      if(!o.skipDriveUpload) scheduleDriveUpload(!!o.driveImmediate);
      if(!o.skipGasPush) schedulePlannerGasPush_(!!o.gasImmediate);
    }
    if(emergencyOk) scheduleSyncStatusUiRefresh_();
    return emergencyOk;
  }
  var previous = readPreviousDiskPayloadSync_() || {};
  ...
  if(!ok && !o._quotaRetried){
    try {
      var pr2 = persistPlannerUnderQuota_({
        skipPush: !!o.skipGasPush,
        allowAllTabs: true
      });
      ok = !!(pr2 && pr2.ok);
    } catch(eQuota){}
...
function scheduleDriveUpload(immediate){
  ...
  if(immediate && !forceDelay){
    driveUploadNow().catch(function(err){
      console.warn('[Drive 즉시 백업]', err);
    });
    return;
  }
  driveUploadTimer = setTimeout(run, forceDelay ? Math.max(2000, quietUntil - Date.now()) : 2000);
}
...
function replaceJsonTopLevelObjectProp_(raw, propName, replacement){
  raw = String(raw || '');
  var needle = '"' + propName + '"';
  var idx = raw.indexOf(needle);
  if(idx < 0) return raw;
  var colon = raw.indexOf(':', idx + needle.length);
  if(colon < 0) return raw;
  var i = colon + 1;
  while(i < raw.length && /\s/.test(raw.charAt(i))) i++;
  var start = i;
  var ch0 = raw.charAt(i);
  if(ch0 === '{'){
    var depth = 0;
    for(; i < raw.length; i++){
      var ch = raw.charAt(i);
      ...
      if(ch === '{') depth++;
        ...
        depth--;
        if(depth === 0){
          return raw.slice(0, start) + replacement + raw.slice(i + 1);
        }
      }
    }
  } else if(ch0 === '['){
...
/** 미발행 생성 초안 본문만 비움 — 주제 카드·발행본 유지 (부팅 멈춤·용량 한도 완화) */
function stripGeneratedContentBodiesInPayload_(payload){
  if(!payload || !payload.generatedOnly) return 0;
  var n = 0;
  Object.keys(payload.generatedOnly).forEach(function(id){
    var g = payload.generatedOnly[id];
    if(!g || typeof g !== 'object') return;
    if(g.content != null){
      g.content = { _prunedForBoot: true };
      n++;
      return;
    }
    if(g.blog || g.thread || g.community || g.insta || g.threads || g.images){
      payload.generatedOnly[id] = { _prunedForBoot: true, topic: g.topic || '' };
      n++;
    }
  });
  return n;
}
function runDeferredBootMigrations_(){
  if(!state._pendingBootMigrations) return;
  state._pendingBootMigrations = false;
  try {
  ...
  try { runDeferredHeavyPlanRepairs_(); } catch(eRep){}
}
// Load persisted — 스크립트 평가를 막지 않도록 첫 페인트 이후로 미룸
state._bootQuietUntil = Date.now() + 20000;
state._bootStrippedGenerated = 0;
state._bootLocalDone = false;
state._bootLocalStarted = false;
function applyBootPlannerRaw_(raw){
  if(!raw){
    try { applyBuiltinDraftBrandSeeds_(); } catch(e0){}
    return;
  }
  plannerLastDiskPayloadStr_ = raw;
  // IndexedDB 사용 가능하면 부팅 때 AI 초안을 지우지 않음 (예전 localStorage 한도 우회가
  // 새로고침마다 초안을 날리는 원인이었음).
  var canKeepFull = typeof indexedDB !== 'undefined';
  var s = null;
  try {
    s = JSON.parse(raw);
  } catch(eParse){
    // 파싱 실패 시에만 초안을 비운 문자열로 재시도
    if(canKeepFull){
      try {
        var slim = replaceJsonTopLevelObjectProp_(raw, 'generatedOnly', '{}');
        s = JSON.parse(slim);
        state._bootStrippedGenerated = Math.max(1, Math.floor((raw.length - slim.length) / 4000));
      } catch(eParse2){
        try { applyBuiltinDraftBrandSeeds_(); } catch(e1){}
        return;
      }
    } else {
      try { applyBuiltinDraftBrandSeeds_(); } catch(e1b){}
      return;
    }
  }
  if(!s || typeof s !== 'object'){
    try { applyBuiltinDraftBrandSeeds_(); } catch(e2){}
    return;
  }
  if(s._idbPrimary){
    try { applyBuiltinDraftBrandSeeds_(); } catch(eStub){}
    return;
  }
  try {
    ...
    console.warn('[boot apply]', eApply);
    try { applyBuiltinDraftBrandSeeds_(); } catch(e3){}
    return;
  }
  try {
    if(state.syncDirty || Object.keys(state.syncDirtyEntityKeys || {}).length){
      state.syncNeedsSnapshot = true;
      state.syncDirty = true;
    }
  } catch(eDirty){}
  // 파싱 실패로만 초안을 비운 경우에만 디스크에 반영 (정상 부팅에서는 유지)
  if(state._bootStrippedGenerated > 0){
    state.syncDirty = true;
    state.syncNeedsSnapshot = true;
    setTimeout(function(){
      try {
        save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true, forceWrite: true });
        if(typeof setAppToast === 'function'){
          setAppToast('손상된 저장본을 복구하느라 일부 미발행 초안 본문을 비웠어요.\n주제 카드는 남아 있습니다.', { duration: 9000, variant: 'ok' });
        }
      } catch(eSave){}
    }, 2000);
  }
}
function bootPlannerLocalState_(){
  if(state._bootLocalDone) return Promise.resolve();
  if(state._bootLocalPromise) return state._bootLocalPromise;
  state._bootLocalStarted = true;
  try {
    var ov = document.getElementById('sync-bootstrap-overlay');
    if(ov) ov.classList.remove('open');
    var appRoot = document.getElementById('planner-app-root');
    if(appRoot){
      appRoot.inert = false;
  ...
  state._bootLocalPromise = readPlannerIdbPayload_().then(function(idbRaw){
    ...
    if(idbRaw && idbRaw.length > 2 && !isPlannerLsIdbStubRaw_(idbRaw)){
      if(lsStub || !lsRaw || idbRaw.length >= lsRaw.length){
        raw = idbRaw;
        fromIdb = true;
        plannerUsingIdbPrimary_ = true;
      } else {
    ...
    } else if(lsRaw && !lsStub){
      raw = lsRaw;
    } else if(idbRaw && !isPlannerLsIdbStubRaw_(idbRaw)){
      raw = idbRaw;
      fromIdb = true;
      plannerUsingIdbPrimary_ = true;
    }
    applyBootPlannerRaw_(raw);
    // localStorage → IndexedDB 마이그레이션
    if(raw && !fromIdb && raw.length > 20000){
      schedulePlannerIdbWrite_(raw);
      plannerUsingIdbPrimary_ = true;
      try {
        if(raw.length > 1.6 * 1024 * 1024){
          localStorage.setItem(SK, buildPlannerLsMirrorStub_(raw));
        }
      } catch(eMigLs){
        try { localStorage.setItem(SK, buildPlannerLsMirrorStub_(raw)); } catch(e2){}
      }
    }
    if(fromIdb || plannerUsingIdbPrimary_){
      try { removeQuotaRecoveryFab_(); } catch(eFabBoot){}
    }
    state._bootLocalDone = true;
  }).catch(function(err){
    console.warn('[boot idb]', err);
    try { applyBootPlannerRaw_(lsStub ? '' : lsRaw); } catch(eBoot){}
    state._bootLocalDone = true;
  });
  return state._bootLocalPromise;
}
...
function setupPlannerServiceWorker_(){
  ...
  if(typeof requestIdleCallback === 'function'){
    requestIdleCallback(function(){ setTimeout(registerLater_, 1500); }, { timeout: 4000 });
  } else {
    setTimeout(registerLater_, 2500);
  }
  if(!window.__plannerSwControllerBound){
    window.__plannerSwControllerBound = true;
    navigator.serviceWorker.addEventListener('controllerchange', function(){
      suggestPlannerSwRefresh_('controllerchange', { preferReload: false });
    });
  }
}
...
window.onload = () => {
  ...
  // 스플래시가 먼저 그려진 뒤 데이터 로드·렌더 (체감 대기 단축)
  var startUi_ = function(){
    Promise.resolve()
      .then(function(){ return bootPlannerLocalState_(); })
      .catch(function(eBoot){ console.warn('[boot]', eBoot); })
      .then(function(){
        try { updateApiBadge(); } catch(e0){}
        try { refreshPlannerServerCaps_(); } catch(e1){}
        try { updateDriveButtonState(); } catch(e2){}
        try { updateSyncStatusUI_(); } catch(e3){}
        bindPlannerMainClickDelegation_();
        bindPlannerConflictButtons_();
        try { renderTabs(); } catch(e4){}
        try { updateAddButtonVisibility_(); } catch(e5){}
        try { renderMain(); } catch(e6){}
        state._bootFirstPaintDone = true;
        try { document.documentElement.classList.add('planner-ready'); } catch(e7){}
        setTimeout(function(){ try { runDeferredBootMigrations_(); } catch(e){} }, 600);
        setTimeout(function(){ try { runPlannerBootstrapSync_(); } catch(e2){} }, 1200);
        if(oauthReturned){
  ...
  if(typeof requestAnimationFrame === 'function'){
    requestAnimationFrame(function(){ setTimeout(startUi_, 0); });
  } else {
...
function normalizeChatgptOpenUrl(raw){
  ...
  try {
    ...
    if(h !== 'chatgpt.com' && h !== 'chat.openai.com' && h !== 'mangoboard.net') return '__invalid__';
...
function getMangoDesignerOpenUrl_(){
  ...
  // 예전 ChatGPT 주소가 저장돼 있어도 이미지 작업은 망고보드로 연다
  if(n && n !== '__invalid__' && /mangoboard\.net/i.test(n)) return n;
  return DEFAULT_MANGO_DESIGNER_URL;
}
...
function saveKey() {
  ...
  if(nu === '__invalid__'){
    errEl.textContent = '주소는 mangoboard.net 또는 chatgpt.com 만 허용됩니다';
    return;
  }
...
function openApiModal() {
  ...
  if(uEl){
    var storedUrl = String(state.chatgptOpenUrl || '');
    uEl.value = (/mangoboard\.net/i.test(storedUrl) ? storedUrl : '') || DEFAULT_MANGO_DESIGNER_URL;
  }
  var syncTokEl = document.getElementById('planner-sync-token');
...
function draftHasContent(d) {
  if(!d || !d.id) return false;
  var pub = state.published[d.id];
  if(pub && pub.content && !pub.content._prunedForBoot){
    if(hasUsableAiDraftContent_(pub.content) || hasUsableAiDraftContent_(pub)) return true;
    // 발행 기록이 있으면 초안 있음으로 봄
    if(draftIsPublished_(d.id)) return true;
  }
  var g = state.generatedOnly[d.id];
  if(!g || g._prunedForBoot) return false;
  if(g.content && g.content._prunedForBoot) return false;
 ...
 * @returns {{ removed: number, kept: number, saved?: boolean, stripped?: number }}
 */
function pruneTopicsWithoutDraftCore_(catId, opts){
  opts = opts || {};
  var cat = CATEGORIES[catId];
  if(!cat) return { removed: 0, kept: 0, saved: true, stripped: 0 };
  var removedIds = [];
  var kept = [];
  (cat.drafts || []).forEach(function(d){
    if(!d || !d.id) return;
  ...
  removedIds.forEach(function(id){
    state.deletedDraftIds[id] = now;
    if(state.pinnedDraftIds) delete state.pinnedDraftIds[id];
    if(state.draftBrandOverrides) delete state.draftBrandOverrides[id];
    delete state.published[id];
    delete state.generatedOnly[id];
    if(typeof instaBgByDraft !== 'undefined') delete instaBgByDraft[id];
    if(typeof threadsBgByDraft !== 'undefined') delete threadsBgByDraft[id];
    if(state.selectedId === id){
      try { closeSheetUiOnly_(); } catch(eClose){}
    }
  });
  // 빈 슬롯에 시드 주제가 다시 채워지지 않게
  if(!state._blockAutoSeedTopics) state._blockAutoSeedTopics = {};
  state._blockAutoSeedTopics[String(catId)] = Date.now();

  if(state.pendingSubGoalPlan && sameCatId_(state.pendingSubGoalPlan.catId, catId)){
    var drop = {};
    removedIds.forEach(function(id){ drop[id] = true; });
    state.pendingSubGoalPlan.assignments = (state.pendingSubGoalPlan.assignments || []).filter(function(a){
      return a && a.draftId && !drop[a.draftId];
    });
  ...
  var saveOk = true;
  if(!opts.skipSave){
    // 무거운 fingerprint/한도 prune 경로 금지 — 삭제 직후 UI가 안 넘어가던 원인
    try {
      state.syncDirty = true;
      state.syncNeedsSnapshot = true;
      state.localSavedAt = new Date().toISOString();
      var payloadStr = JSON.stringify(getLocalStoragePayload());
      saveOk = !!writeLocalStoragePayloadString_(payloadStr);
      if(saveOk){
        try { scheduleSyncStatusUiRefresh_(); } catch(eUi){}
        if(!opts.skipUpload){
          try { schedulePlannerGasPush_(false); } catch(ePush){}
        }
      }
    } catch(eSave){
      console.warn('[주제 삭제 저장]', eSave);
      saveOk = false;
    }
  }
  if(!opts.skipRender){
    try { renderMain(); } catch(eR){}
    if(state.planWorkshopMode === 'topic'){
      try { refreshTopicWorkshop_(); } catch(eW){}
    }
  }
  return {
...
window.forceDedupeSubGoalSteps_ = function(catId){
  catId = parseInt(catId, 10);
  if(isNaN(catId)) catId = state.currentCat;
  var before = 0;
  try {
    var p0 = (state.branding && state.branding.subGoalPlans && state.branding.subGoalPlans[String(catId)]) || null;
    before = (p0 && p0.steps && p0.steps.length) || 0;
  } catch(e0){}
  var changed = false;
  try {
  ...
  var after = 0;
  try {
    var p1 = getSubGoalPlan_(catId);
    after = (p1 && p1.steps && p1.steps.length) || 0;
  } catch(e4){}
  if(typeof setAppToast === 'function'){
    setAppToast(
      changed || before !== after
        ? ('중복 단계를 정리했어요. ' + before + '줄 → ' + after + '줄.')
        : '정리할 중복 단계가 없어요.',
      { duration: 5000, variant: 'ok' }
    );
  }
};
...
async function rewriteOpsReviewProposal_(itemId, proposalId, branchId){
  ...
  try {
    ...
    var obj = parsePlannerAiJsonObject_(raw);
...
function buildSheetTabsHTML(tab){
  ...
  if(isHeiljagyaeCategory(cid)){
    ...
    <button type="button" class="sheet-tab${tab==='images'?' active':''}" onclick="switchTab('images')">이미지</button>
  </div></div>`;
  }
  return `<div class="sheet-tabs-scroll"><div class="sheet-tabs">
    <button type="button" class="sheet-tab${tab==='images'?' active':''}" onclick="switchTab('images')">망고보드</button>
...
function clampMangoTitle_(s){
  return String(s || '').trim().slice(0, MANGO_DETAIL_TITLE_MAX);
}
function clampMangoIntro_(s){
  return String(s || '').trim().slice(0, MANGO_DETAIL_INTRO_MAX);
}

/** 블로그 제목이 길면 포인트를 살린 채 30자 이내로 줄임 */
function shortenMangoProductName_(raw){
  var s = String(raw || '').replace(/\s+/g, ' ').trim();
  s = s.replace(/^["'「『\s]+|["'」』\s]+$/g, '').trim();
  if(!s) return '';
  if(s.length <= MANGO_DETAIL_TITLE_MAX) return s;

  var parts = s.split(/\s*[—–―－|·|:：]\s*/).map(function(p){ return p.trim(); }).filter(Boolean);
  if(parts.length >= 2){
    var after = parts[parts.length - 1];
    var before = parts[0];
    // 대시 뒤가 핵심 포인트인 경우가 많음
    if(after.length >= 6 && after.length <= MANGO_DETAIL_TITLE_MAX) return after;
    if(before.length <= MANGO_DETAIL_TITLE_MAX) return before;
    // 앞·뒤를 짧게 합치기
    var room = MANGO_DETAIL_TITLE_MAX - 1;
    var aLen = Math.min(after.length, Math.floor(room * 0.45));
    var bLen = room - aLen;
    var combo = before.slice(0, bLen).replace(/[\s,，、]+$/,'') + ' ' + after.slice(0, aLen).replace(/^[\s,，、]+/,'');
    if(combo.length >= 8 && combo.length <= MANGO_DETAIL_TITLE_MAX) return combo;
  }

  // 물음표 질문형: 질문 문장만 남기기
  var q = s.match(/^(.+?[?？])/);
  if(q && q[1].length <= MANGO_DETAIL_TITLE_MAX) return q[1].trim();

  var cut = s.slice(0, MANGO_DETAIL_TITLE_MAX);
  var lastBreak = -1;
  for(var i = cut.length - 1; i >= 10; i--){
    if(/[\s,，、·・]/.test(cut.charAt(i))){ lastBreak = i; break; }
  }
  if(lastBreak > 0) return cut.slice(0, lastBreak).trim();
  var trimmed = cut.replace(/\s+\S*$/, '').trim();
  return (trimmed.length >= 8 ? trimmed : cut).slice(0, MANGO_DETAIL_TITLE_MAX);
}

function normalizeMangoBrief_(brief){
  if(!brief || typeof brief !== 'object') return null;
  var intro = clampMangoIntro_(brief.intro || brief.mangoInputPaste || '');
  var paste = clampMangoIntro_(brief.mangoInputPaste || intro);
  if(!paste) paste = intro;
  if(!intro) intro = paste;
  return {
    productName: shortenMangoProductName_(brief.productName || '') || clampMangoTitle_(brief.productName || ''),
    intro: intro,
    features: Array.isArray(brief.features) ? brief.features.map(function(f){ return String(f || '').trim(); }).filter(Boolean).slice(0, 8) : [],
    slidePlan: Array.isArray(brief.slidePlan) ? brief.slidePlan.slice(0, 7) : [],
    designTone: brief.designTone != null ? String(brief.designTone) : '',
    photoTip: brief.photoTip != null ? String(brief.photoTip) : '',
    mangoInputPaste: paste
  };
}
...
function normalizeImagesBlock(raw){
  ...
  return {
    ...
    mangoBrief: normalizeMangoBrief_(raw.mangoBrief)
  };
}

/** 구형 2장(썸네일+본문) 초안 호환 — 힐자계·레거시 */
function trimBlogInstaImages_(gptVisuals){
  var list = (gptVisuals || []).filter(function(x){ return x && x.prompt; });
  if(list.length <= 2) return list;
  function isThumb(x){
    return x.role === 'thumbnail' || x.role === 'problem' || x.role === 'cover' ||
      (/썸네일|thumbnail|①|표지|공감|문제/i.test(String(x.title || '')) &&
        !/②|③|본문|동작|시연|셀프|원리|body|selfCare|explanation/i.test(String(x.title || '')));
  }
  function isBody(x){
    return x.role === 'body' || x.role === 'selfCare' ||
  ...
  var scene = list.find(function(x){
    return /썸네일|thumbnail|공감|일상|클로즈|close|cover|문제/i.test(String(x.title || ''));
  });
...
function sortMangoBlogVisuals_(list){
  var rank = {
    problem: 0, cover: 0, thumbnail: 0, hook: 0,
    selfCare: 1, body: 1, demo: 1, outline: 1,
    explanation: 2, principle: 2, draft: 2
  };
  return (list || []).slice().sort(function(a, b){
    var ra = a && a.role != null && rank[a.role] != null ? rank[a.role] : 9;
    var rb = b && b.role != null && rank[b.role] != null ? rank[b.role] : 9;
    if(ra !== rb) return ra - rb;
    return 0;
  });
}

/** 이미지 탭·저장·복사에 쓰는 정규화 목록 */
function getDisplayGptVisuals_(gptVisuals, catId){
  ...
  if(isHeiljagyaeCategory(catId)) return trimBlogInstaImages_(raw).slice(0, 2);
  if(raw.length <= cap) return sortMangoBlogVisuals_(raw);
  // 슬롯이 많으면 역할 순으로 cap장만
  var sorted = sortMangoBlogVisuals_(raw);
  var picked = [];
  var want = ['problem', 'selfCare', 'explanation'];
  want.forEach(function(role){
    if(picked.length >= cap) return;
    var hit = sorted.find(function(x){ return x.role === role && picked.indexOf(x) < 0; });
...
[힐자계 이미지]
주제: "${t}" / 각도: "${a}"
images.gptVisuals는 빈 배열 [] (영문 GPT 프롬프트 작성 금지). mangoBrief는 null.
`;
}

function formatMangoBriefPaste_(brief, catId){
  if(!brief) return '';
  // 소개란 = intro/mangoInputPaste + 디자인 블록(망고보드가 색을 읽도록)
  return ensureMangoIntroHasDesignBlock_(brief.intro || brief.mangoInputPaste || '', catId);
}

function formatMangoTitlePaste_(brief){
  if(!brief) return '';
  return shortenMangoProductName_(brief.productName || '') || clampMangoTitle_(brief.productName || '');
}
...
function ensureMangoIntroHasDesignBlock_(intro, catId){
  ...
  var layoutSec = '';
  var layoutMatch = body.match(/\n*\[레이아웃\]([\s\S]*)$/i);
  if(layoutMatch) layoutSec = layoutMatch[1] || '';
  // 레이아웃 블록 안에 로고·타이포 규칙이 있어야 최신으로 간주 (디자인 톤의 '우하단'만으로는 통과시키지 않음)
  ...
  if(stripped.length > room){
    var cut = stripped.slice(0, room);
    var br = Math.max(cut.lastIndexOf('\n\n'), cut.lastIndexOf('\n'));
    stripped = (br >= Math.floor(room * 0.55) ? cut.slice(0, br) : cut).trim();
  }
  var combined = stripped + '\n\n' + designBlock;
  if(combined.length > MANGO_DETAIL_INTRO_MAX){
    // 최후: 본문을 더 줄여 디자인 블록 전체 보존
    var keepDesign = designBlock.length + 2;
    var bodyRoom = MANGO_DETAIL_INTRO_MAX - keepDesign;
    if(bodyRoom < 80) bodyRoom = 80;
    stripped = stripped.slice(0, bodyRoom).trim();
    combined = stripped + '\n\n' + designBlock;
    if(combined.length > MANGO_DETAIL_INTRO_MAX){
      combined = combined.slice(0, MANGO_DETAIL_INTRO_MAX);
    }
  }
  return combined;
}

/** 망고보드: 소개란에 한 번에 붙인 뒤, 첫 줄(제목)만 잘라 제품명란으로 옮기기용 */
function formatMangoTitleAndIntroPaste_(title, intro, catId){
  var t = shortenMangoProductName_(title) || clampMangoTitle_(title);
  var body = ensureMangoIntroHasDesignBlock_(intro, catId);
  if(t && body) return t + '\n\n' + body;
  return t || body || '';
}

function readMangoTitleAndIntroFromSheet_(){
  var titleEl = document.getElementById('sheet-mango-title');
  var introEl = document.getElementById('sheet-mango-brief-paste');
  var content = state.published[state.selectedId]?.content || state.generatedOnly[state.selectedId];
  var brief = content && content.images && content.images.mangoBrief;
  var catId = state.selectedCatId;
  var title = titleEl ? String(titleEl.value || '').trim() : '';
  var intro = introEl ? String(introEl.value || '').trim() : '';
  if(!title) title = formatMangoTitlePaste_(brief);
  if(!intro) intro = formatMangoBriefPaste_(brief, catId);
  title = shortenMangoProductName_(title) || clampMangoTitle_(title);
  intro = ensureMangoIntroHasDesignBlock_(intro, catId);
  // 화면에 디자인 블록이 없으면 반영
  if(introEl && String(introEl.value || '').trim() !== intro){
    introEl.value = intro;
    if(typeof window.updateMangoFieldCounts_ === 'function') window.updateMangoFieldCounts_();
  }
  return { title: title, intro: intro, paste: formatMangoTitleAndIntroPaste_(title, intro, catId) };
}

function emptyMangoBrief_(catId){
  return {
    productName: '',
    intro: '',
    features: [],
    slidePlan: [],
    designTone: getMangoDesignToneLine_(catId),
    photoTip: getMangoModelGuideForCat_(catId).photoTip,
    mangoInputPaste: ''
  };
}

/** intro가 비었거나 디자인 톤만 있는 경우(붙여넣기 부적합) */
function isWeakMangoIntro_(s){
  var t = String(s || '').trim();
  if(!t) return true;
  if(t.length < 80) return true;
  // "[디자인] …" 만 있거나 디자인 톤만인 경우
  var withoutDesign = t.replace(/\n*\[디자인\][\s\S]*$/i, '').replace(/\n*\[레이아웃\][\s\S]*$/i, '').trim();
  if(!withoutDesign || withoutDesign.length < 60) return true;
  if(/디자인/i.test(t) && /흑[·.\s]*백|미니멀|로고는 후처리/i.test(t) && withoutDesign.length < 60) return true;
  if(/흑[·.\s]*백/.test(t) && /미니멀/.test(t) && /로고는 후처리|만화/.test(t) &&
    !/【\s*\d|헤드:|1\.\s*표지|문제 공감|셀프 케어|이런 분께/.test(t)) return true;
  return false;
}

/** 블로그 원문 붙여넣기·말줄임 등 이미지용으로 부적합한 구형 소개 */
function isLegacyBlogPasteMangoIntro_(s){
  var body = String(s || '')
    .replace(/\n*\[디자인\][\s\S]*$/i, '')
    .replace(/\n*\[레이아웃\][\s\S]*$/i, '')
    .trim();
  if(!body) return false;
  if(/헤드:\s*\S/.test(body) && /【\s*\d/.test(body)) return false;
  if(/…|\.\.\./.test(body)) return true;
  if(/1\.\s*표지/.test(body) && !/헤드:/.test(body)) return true;
  // 긴 산문만 있고 장 구분·불릿이 거의 없으면 덤프
  if(body.length > 900 && !/【\s*\d|STEP\s*\d/i.test(body) && (body.match(/•/g) || []).length < 3) return true;
  return false;
}

function trimMangoSection_(s, maxLen){
  var t = String(s || '').replace(/\s+/g, ' ').trim();
  if(!t) return '';
  if(t.length <= maxLen) return t;
  // 말줄임 대신 문장 경계에서 자름 (이미지용 — … 표시 안 함)
  var cut = t.slice(0, maxLen);
  var sp = cut.lastIndexOf(' ');
  var punct = Math.max(
    cut.lastIndexOf('.'), cut.lastIndexOf('。'),
    cut.lastIndexOf('요'), cut.lastIndexOf('다'),
    cut.lastIndexOf('까'), cut.lastIndexOf('죠'),
    cut.lastIndexOf('!'), cut.lastIndexOf('?'), cut.lastIndexOf('？')
  );
  if(punct >= Math.floor(maxLen * 0.4)) return cut.slice(0, punct + 1).trim();
  if(sp >= Math.floor(maxLen * 0.45)) return cut.slice(0, sp).trim();
  return cut.trim();
}

/** 산문에서 앞쪽 문장만 (이미지 헤드·불릿용 — 2000자 여유 반영) */
function mangoPickSentences_(text, maxCount, maxEach){
  maxCount = maxCount || 2;
  maxEach = maxEach || 52;
  var raw = String(text || '').replace(/\r\n/g, '\n').replace(/👉/g, '\n').trim();
  if(!raw) return [];
  var parts = [];
  if(typeof splitProseSentences_ === 'function'){
    parts = splitProseSentences_(raw.replace(/\n+/g, ' '));
  }
  if(!parts.length){
    parts = raw.split(/[.!?。？！]+/).map(function(x){ return x.trim(); }).filter(Boolean);
  }
  if(!parts.length) parts = [raw];
  var out = [];
  var seen = {};
  for(var i = 0; i < parts.length && out.length < maxCount; i++){
    var s = trimMangoSection_(parts[i].replace(/^[-•*]\s*/, '').replace(/^\d+[.)]\s*/, ''), maxEach);
...
function mangoStripStepPrefix_(s){
  return String(s || '')
...
/** 셀프케어 → STEP 줄 (동작·초·회 우선) */
function mangoExtractSteps_(selfCare, maxSteps){
  maxSteps = maxSteps || 3;
  var raw = String(selfCare || '').trim();
  if(!raw) return [];
  var steps = (typeof parseSelfCareSteps_ === 'function') ? parseSelfCareSteps_(raw) : [raw];
  var out = [];
  var seen = {};
  for(var i = 0; i < steps.length && out.length < maxSteps; i++){
    var line = trimMangoSection_(mangoStripStepPrefix_(steps[i]), 56);
    if(!line || line.length < 4) continue;
    // 원리·주의 문장은 STEP에서 제외
    if(/병원|원리|때문에|왜냐하면|진단|치료/.test(line) && !/\d+\s*(초|회|분)/.test(line)) continue;
    var key = line.slice(0, 14);
    if(seen[key]) continue;
    seen[key] = 1;
    out.push(line);
  }
...
function getDraftTopicForMango_(draftId){
  draftId = draftId != null ? draftId : state.selectedId;
  if(!draftId) return '';
  var pub = state.published && state.published[draftId];
  if(pub && pub.topic) return String(pub.topic).trim();
  var catId = state.selectedCatId;
  var cat = CATEGORIES[catId];
  if(cat && Array.isArray(cat.drafts)){
    var hit = cat.drafts.find(function(d){ return d && d.id === draftId; });
    if(hit && hit.topic) return String(hit.topic).trim();
  }
  return '';
}

/** 블로그 → 망고보드용 장별 이미지 핵심 카피 (원문 붙여넣기·… 없음) */
function buildMangoBriefFromBlog_(content, topic, catId){
  catId = catId != null ? catId : state.selectedCatId;
  var palette = getMangoPaletteForCat_(catId);
  var blog = content && content.blog ? content.blog : null;
  var title = blog && String(blog.title || '').trim();
  var productName = shortenMangoProductName_(title || topic || '');
  var problem = '';
  var selfCare = '';
  var explanation = '';
  var cta = '';
  var hook = '';
  if(blog){
    problem = getGeneralBlogProblemText_(blog) || String(blog.problem || '').trim();
    selfCare = String(blog.selfCare || '').trim();
    explanation = String(blog.explanation || '').trim();
    cta = String(blog.cta || '').trim();
    hook = String(blog.hook || '').trim();
    if(!selfCare && !explanation && blog.draft){
      var draftBody = String(blog.draft || '').trim();
      if(draftBody) explanation = draftBody;
    }
    if(!problem && hook) problem = hook;
    if(!problem && Array.isArray(blog.outline) && blog.outline.length){
      problem = blog.outline.map(function(x){ return String(x || '').trim(); }).filter(Boolean).join(' ');
    }
  }

  var coverHead = trimMangoSection_(title || topic || hook || '유연한데 왜 자꾸 삐끗할까요?', 42);
  var problemSents = mangoPickSentences_(problem, 3, 52);
  var empathyHead = problemSents[0] || '스트레칭은 되는데 자꾸 삐끗해요';
  var empathyBullets = problemSents.slice(1, 3);
  if(!empathyBullets.length){
    empathyBullets = ['특정 동작에서 힘이 빠지는 느낌', '늘어나는 것과 버티는 힘은 다름'];
  }
  // 표지와 공감 헤드가 같으면 공감은 다른 각도
  if(empathyHead.slice(0, 10) === coverHead.slice(0, 10) && problemSents[1]){
    empathyHead = problemSents[1];
    empathyBullets = problemSents.slice(2, 3).concat(['일상에서 반복되는 흔들림']);
  }

  var audienceHead = '이런 분께 맞춰 정리했어요';
  var audienceBullets = [];
  if(/삐끗|접질|불안정|흔들/.test(problem + coverHead)){
    audienceBullets.push('잘 늘어나는데도 자주 삐끗하는 분');
  }
  if(/스트레칭|유연|가동/.test(problem + coverHead + selfCare)){
    audienceBullets.push('스트레칭만 열심히 해 온 분');
  }
  if(!audienceBullets.length){
    audienceBullets = ['움직임이 불안정하게 느껴지는 분', '무리하게 범위만 키워 온 분'];
  }
  audienceBullets = audienceBullets.slice(0, 2);

  var steps = mangoExtractSteps_(selfCare, 3);
  if(!steps.length){
    steps = [
      '지금 가능한 범위에서 10초만 정확히 버티기',
      '흔들리면 멈추고 범위 줄이기',
      '같은 동작 3회만 반복'
    ];
  }

  var whySents = mangoPickSentences_(explanation, 3, 52);
  var whyHead = whySents[0] || '범위보다 버티는 힘이 먼저예요';
  var whyBullets = whySents.slice(1, 3);
  if(!whyBullets.length){
    whyBullets = ['조절이 따라와야 움직임이 안전해요', '먼저 버티고, 그다음 범위를 넓혀요'];
  }
  // 원리 헤드가 공감과 겹치면 교체
  if(whyHead.slice(0, 12) === empathyHead.slice(0, 12)){
    whyHead = '왜 그런지, 한 줄로';
  }

  var cautionBullets = [
    '갑자기 아파지거나 저림·마비가 커지면 중단',
  ...
  var intro = ensureMangoIntroHasDesignBlock_(blocks.join('\n\n'), catId);
  return {
    productName: productName,
    intro: intro,
    mangoInputPaste: intro,
    designTone: getMangoDesignToneLine_(catId)
  };
}

function fillMangoBriefFromContent_(content, opts){
  opts = opts || {};
  if(!content || !content.images) return content;
  var catId = opts.catId != null ? opts.catId : state.selectedCatId;
  if(isHeiljagyaeCategory(catId)) return content;
  var brief = content.images.mangoBrief;
  if(!brief || typeof brief !== 'object'){
    brief = emptyMangoBrief_(catId);
    content.images.mangoBrief = brief;
  } else {
    brief = normalizeMangoBrief_(brief) || emptyMangoBrief_(catId);
    content.images.mangoBrief = brief;
  }
  var topic = opts.topic || getDraftTopicForMango_(opts.draftId);
  // 시트 제목(블로그 주제)도 보조로 사용
  if(!topic){
    var titleEl = document.getElementById('sheet-title');
    if(titleEl) topic = String(titleEl.textContent || '').trim();
  }
  var combinedIntro = String(brief.intro || brief.mangoInputPaste || '').trim();
  // designTone이 intro로 잘못 들어온 경우도 감지
  if(!combinedIntro && brief.designTone && isWeakMangoIntro_(brief.designTone)){
    combinedIntro = '';
  }
  var needTitle = !String(brief.productName || '').trim();
  var fullTitle = (content.blog && String(content.blog.title || '').trim()) || topic || '';
  // 긴 제목을 그냥 앞에서 자른 상태면 포인트 살려 다시 줄임
  if(!needTitle && fullTitle.length > MANGO_DETAIL_TITLE_MAX){
    var cur = String(brief.productName || '').trim();
    if(cur === fullTitle.slice(0, MANGO_DETAIL_TITLE_MAX) || cur.length > MANGO_DETAIL_TITLE_MAX){
      needTitle = true;
    }
  }
  var needIntro = isWeakMangoIntro_(combinedIntro) ||
    isWeakMangoIntro_(brief.intro) ||
    isWeakMangoIntro_(brief.mangoInputPaste) ||
    isLegacyBlogPasteMangoIntro_(combinedIntro) ||
    isLegacyBlogPasteMangoIntro_(brief.intro) ||
    isLegacyBlogPasteMangoIntro_(brief.mangoInputPaste);
  if(needTitle || needIntro){
    var built = buildMangoBriefFromBlog_(content, topic, catId);
    if(needTitle && built.productName) brief.productName = built.productName;
    else if(brief.productName) brief.productName = shortenMangoProductName_(brief.productName);
    if(needIntro && built.intro){
      brief.intro = built.intro;
      brief.mangoInputPaste = built.intro;
    }
  } else {
    // intro·paste 동기화 + 제품명 30자 정리
    var intro = clampMangoIntro_(brief.intro || brief.mangoInputPaste || '');
    brief.intro = intro;
    brief.mangoInputPaste = intro;
    if(brief.productName) brief.productName = shortenMangoProductName_(brief.productName);
  }
  // 복사·망고보드용: 소개 끝에 카테고리 팔레트 디자인 블록 보장
  brief.intro = ensureMangoIntroHasDesignBlock_(brief.intro || brief.mangoInputPaste || '', catId);
  brief.mangoInputPaste = brief.intro;
  brief.designTone = getMangoDesignToneLine_(catId);
  var modelGuide = getMangoModelGuideForCat_(catId);
  var tip = String(brief.photoTip || '');
  var tipMismatch = !tip ||
    (modelGuide.gender === 'female' && /남성/.test(tip) && !/여성/.test(tip)) ||
    (modelGuide.gender === 'male' && /여성/.test(tip) && !/남성/.test(tip)) ||
    (modelGuide.gender === 'female' && !/여성/.test(tip)) ||
    (modelGuide.gender === 'male' && !/남성/.test(tip));
  if(tipMismatch) brief.photoTip = modelGuide.photoTip;
  content.images.mangoBrief = normalizeMangoBrief_(brief) || brief;
  return content;
}

function ensureMangoBriefOnContent_(content){
  if(!content || !content.images) return content;
  fillMangoBriefFromContent_(content, {
    catId: state.selectedCatId,
    draftId: state.selectedId,
    topic: getDraftTopicForMango_(state.selectedId)
  });
  return content;
}

function renderMangoDetailInputCard_(brief){
  var catId = state.selectedCatId;
  var p = getMangoPaletteForCat_(catId);
  brief = brief || emptyMangoBrief_(catId);
  ...
  html += '<label style="display:block;font-size:11px;color:#374151;margin:0 0 4px;">소개 (7장 · 이미지용 핵심 카피) <span id="sheet-mango-intro-count" style="color:#9CA3AF;">' + introVal.length + '/' + MANGO_DETAIL_INTRO_MAX + '</span></label>';
  html += '<textarea class="sheet-edit" id="sheet-mango-brief-paste" rows="10" maxlength="' + MANGO_DETAIL_INTRO_MAX + '" oninput="updateMangoFieldCounts_()">' + escapeHtml(introVal) + '</textarea>';
  if(brief.photoTip){
    html += '<div style="margin-top:10px;padding:10px 12px;background:#F5F3FF;border-radius:8px;font-size:12px;color:#5B21B6;line-height:1.55;"><strong>TIP · 참고 사진</strong><br>' + escapeHtml(brief.photoTip) + '</div>';
  } else {
    html += '<div style="margin-top:10px;padding:10px 12px;background:#F5F3FF;border-radius:8px;font-size:12px;color:#5B21B6;line-height:1.55;"><strong>TIP · 참고 사진</strong><br>서비스·시연 장면 사진을 올리세요. ② 셀프 케어 전신 컷(실사)을 권장합니다. 제품이 잘린 사진·글자만 있는 캡처는 피하세요.</div>';
  }
  html += '<div class="img-tool-actions" style="margin-top:10px;">';
...
function buildImageTabBody(content){
  const isHj = isHeiljagyaeCategory(state.selectedCatId);

  // 힐자계: GPT 영문 프롬프트 UI 없음
  if(isHj){
    return '<p class="empty-note" style="padding:0;line-height:1.55;">게시판 글용 참고 사진은 실사·시연 컷을 직접 첨부하면 됩니다. (GPT 이미지 프롬프트는 사용하지 않습니다.)</p>';
  }
  ...
  html += '<p class="empty-note" style="padding:8px 0 0;font-size:11px;color:#9CA3AF;line-height:1.55;">수정 후 <strong>발행완료</strong>를 누르면 저장돼요. 참고 사진은 실사·시연 컷을 올리면 되고, 로고·타이포는 망고보드에서 올립니다.</p>';
...
window.updateMangoFieldCounts_ = function(){
  var titleEl = document.getElementById('sheet-mango-title');
  var introEl = document.getElementById('sheet-mango-brief-paste');
  var titleCount = document.getElementById('sheet-mango-title-count');
  var introCount = document.getElementById('sheet-mango-intro-count');
  if(titleEl && titleCount) titleCount.textContent = String(titleEl.value || '').length + '/' + MANGO_DETAIL_TITLE_MAX;
  if(introEl && introCount) introCount.textContent = String(introEl.value || '').length + '/' + MANGO_DETAIL_INTRO_MAX;
};

window.copyMangoTitleOnly_ = function(){
  var pack = readMangoTitleAndIntroFromSheet_();
  var text = pack.title;
  if(!text){ setAppToast('복사할 제품명이 없어요.', { duration: 2500, variant: 'err' }); return; }
  navigator.clipboard.writeText(text).then(function(){
    setAppToast('제품명을 복사했어요. (' + text.length + '/' + MANGO_DETAIL_TITLE_MAX + ')', { duration: 2000, variant: 'ok' });
  }).catch(function(){ window.prompt('복사할 제품명:', text); });
};

window.copyMangoBriefOnly_ = function(){
  var pack = readMangoTitleAndIntroFromSheet_();
  var text = pack.intro;
  if(!text){ setAppToast('복사할 소개가 없어요.', { duration: 2500, variant: 'err' }); return; }
  navigator.clipboard.writeText(text).then(function(){
    setAppToast('소개를 복사했어요. (' + text.length + '/' + MANGO_DETAIL_INTRO_MAX + ')', { duration: 2000, variant: 'ok' });
  }).catch(function(){ window.prompt('복사할 소개:', text); });
};

window.copyMangoBriefAndOpen_ = function(){
  var pack = readMangoTitleAndIntroFromSheet_();
  var text = pack.paste;
  if(!text){ setAppToast('복사할 제목·소개가 없어요.', { duration: 2500, variant: 'err' }); return; }
  // 입력란에도 30자 정리본 반영
  var titleEl = document.getElementById('sheet-mango-title');
  if(titleEl && pack.title) titleEl.value = pack.title;
  if(typeof window.updateMangoFieldCounts_ === 'function') window.updateMangoFieldCounts_();
  navigator.clipboard.writeText(text).then(function(){
    window.open(getMangoDesignerOpenUrl_(), '_blank', 'noopener,noreferrer');
    setAppToast('제목·소개 복사됨 · 소개란에 붙여넣은 뒤, 첫 줄만 잘라 제품명란으로 옮기세요.', { duration: 4500, variant: 'ok' });
  }).catch(function(){
    window.open(getMangoDesignerOpenUrl_(), '_blank', 'noopener,noreferrer');
...
const MANGO_BG_ESTIMATE_MS = 40000;
const MANGO_BG_TIMEOUT_MS = 90000;
...
function enqueueInstaFromBlog_(draftId, catId, topic, blog, opts){
  ...
  genActiveJob = { topic: label, endMs: endMs, kind: 'insta', draftId: draftId, catId: catId };
  updateGenIndicator();
...
var SHEET_FIELD_META_ = {
  'blog.title':       { block: 'blog', label: '제목', instr: '증상명과 지역/브랜드 키워드가 자연스럽게 들어간, 검색·AI 인용에 강한 제목 한 줄. 존댓말·질문형 권장.' },
  'blog.problem':     { block: 'blog', label: '문제 제기', instr: '공감 질문으로 시작해 "일상에서 바로 풀 수 있다"는 한 줄까지. 독자의 상황을 구체적으로. **존댓말(해요체)**. 한다체·반말 금지.' },
  'blog.selfCare':    { block: 'blog', label: '셀프 케어', step: true, instr: '집에서 따라 할 수 있는 셀프 케어를 단계별로. 각 단계는 새 줄에서 "👉"로 시작하고 동작·초/회/세트를 앞쪽에. 단계 사이 빈 줄. **존댓말(해요체)**.' },
  'blog.explanation': { block: 'blog', label: '원리 설명', instr: '왜 그런지 해부학적·기능적 원리를 쉬운 말로. **존댓말(해요체)**. 한다체(~한다/~이다) 금지.' },
  'blog.cta':         { block: 'blog', label: '마무리 CTA', instr: '전문가 상담·프로필 페이지로 부드럽게 안내하는 마무리 2~3문장. **존댓말**.' },
  'blog.hashtags':    { block: 'blog', label: '해시태그', array: true, instr: '증상·지역·브랜드와 관련된 해시태그 5~8개.' },
  'blog.hook':        { block: 'blog', label: '영상·사진 맥락', instr: '이번 영상·사진에서 다룬 장면·상황을 짧게. **존댓말**.' },
  'blog.outline':     { block: 'blog', label: '시연·핵심 포인트', arrayLines: true, instr: '손 위치·동작·주의사항 등 핵심 포인트를 한 줄에 하나씩. **존댓말**.' },
  'blog.draft':       { block: 'blog', label: '원리 설명(본문)', instr: '본문의 중심. 왜 이렇게 하는지 짧은 메커니즘을 포함. **존댓말(해요체)**.' },
  ...
  'insta.hook':     { block: 'insta', label: '첫 줄 후킹', instr: '스크롤을 멈추게 하는 인스타 첫 줄 후킹 한 문장. **존댓말(해요체)**.' },
...
window.regenSheetField_ = async function(key, btn){
  ...
  if(btn){
    startButtonCountdown_(btn, { estimateSec: SHEET_FIELD_REGEN_ESTIMATE_SEC, busyLabel: (meta.label || '필드') + ' 재생성 중', idleText: '재생성' });
  }
  try {
...
function applySheetImageEdits_(content){
  ...
  if(titleEl || briefEl){
    if(!content.images.mangoBrief || typeof content.images.mangoBrief !== 'object'){
      content.images.mangoBrief = {};
    }
    if(titleEl) content.images.mangoBrief.productName = shortenMangoProductName_(titleEl.value) || clampMangoTitle_(titleEl.value);
    if(briefEl){
      var intro = ensureMangoIntroHasDesignBlock_(String(briefEl.value || '').trim(), catId);
      content.images.mangoBrief.intro = intro;
      content.images.mangoBrief.mangoInputPaste = intro;
      if(String(briefEl.value || '') !== intro) briefEl.value = intro;
    }
    content.images.mangoBrief.designTone = getMangoDesignToneLine_(catId);
    content.images.mangoBrief = normalizeMangoBrief_(content.images.mangoBrief);
  }
  return content;
}
...
async function generateMangoBriefFromBlog_(catId, blog, topic, draft){
  var imageGuide = getCatPromptForGeneration_(catId, 'image');
  var brandBlock = buildBrandContextForPrompt_(catId, draft);
  var tone = getMangoDesignToneLine_(catId);
  var prompt =
    getBasePrompt() + '\n\n' +
    (brandBlock ? brandBlock + '\n\n' : '') +
    '주제: "' + (topic || '') + '"\n\n' +
    getMangoDetailBriefRule_(catId) + '\n\n' +
    (imageGuide ? '[카테고리 망고보드 지침]\n' + imageGuide + '\n\n' : '') +
    '아래 **확정·수정된 블로그**를 바탕으로 **mangoBrief만** 다시 만드세요.\n' +
    '블로그·인스타 등 다른 키는 출력하지 마세요.\n' +
    '블로그 문단 붙여넣기·말줄임(…) 금지. 【1~7장】헤드+불릿+비주얼로 변환.\n' +
    'JSON 규칙(필수):\n' +
    '- 유효한 JSON **객체 하나만**. 설명·코드펜스 금지. 첫 글자 {\n' +
    '- intro·mangoInputPaste 안의 줄바꿈은 \\n, 따옴표는 \\" 로 이스케이프\n' +
    '- intro는 최대 ' + MANGO_DETAIL_INTRO_MAX + '자(디자인·레이아웃 포함). 잘리지 않게 간결히\n' +
    '- mangoInputPaste는 intro와 동일 문자열\n\n' +
    'JSON만:\n' +
    '{"mangoBrief":{"productName":"최대30자","intro":"【1. 표지】…7장+[디자인]+[레이아웃]","mangoInputPaste":"intro와 동일","photoTip":"' +
    getMangoModelGuideForCat_(catId).photoTipShort.replace(/"/g, '\\"') + '","slidePlan":[{"slide":1,"role":"표지","copyHint":"..."}],"designTone":"' +
    tone.replace(/"/g, '\\"') + '"}}\n\n' +
    '[블로그 원문]\n' + buildBlogSourceText_(blog, catId);
  var text = await callClaudePlanner_(prompt, { maxTokens: 4500, timeoutMs: MANGO_BG_TIMEOUT_MS });
  var obj = null;
  try {
    obj = parsePlannerAiJsonObject_(text);
  } catch(eParse){
    console.warn('[망고보드 JSON 파싱 실패 → 로컬 변환]', eParse);
    return buildMangoBriefFromBlog_({ blog: blog }, topic, catId);
  }
  var brief = normalizeMangoBrief_(obj && (obj.mangoBrief || obj));
  if(!brief || isWeakMangoIntro_(brief.intro || brief.mangoInputPaste)){
    return buildMangoBriefFromBlog_({ blog: blog }, topic, catId);
  }
  if(brief.productName) brief.productName = shortenMangoProductName_(brief.productName);
  brief.intro = ensureMangoIntroHasDesignBlock_(brief.intro || brief.mangoInputPaste || '', catId);
  brief.mangoInputPaste = brief.intro;
  brief.designTone = tone;
  var modelGuide = getMangoModelGuideForCat_(catId);
  var tip = String(brief.photoTip || '');
  var tipMismatch = !tip ||
    (modelGuide.gender === 'female' && /남성/.test(tip) && !/여성/.test(tip)) ||
...
function enqueueMangoFromBlog_(draftId, catId, topic, blog, draft){
  if(mangoBgByDraft[draftId]) return;
  if(!state.apiKey) return;
  var startedMs = Date.now();
  var endMs = startedMs + MANGO_BG_ESTIMATE_MS;
  mangoBgByDraft[draftId] = { startedMs: startedMs, endMs: endMs, topic: topic || '', catId: catId };
  beginGenIndicator();
  var blogSnapshot = JSON.parse(JSON.stringify(blog || {}));
  var label = String(topic || '망고보드');
  if(label.length > 34) label = label.slice(0, 34) + '…';
  genActiveJob = { topic: label, endMs: endMs, kind: 'mango', draftId: draftId, catId: catId };
  updateGenIndicator();

  (async function(){
    try {
      var brief = await generateMangoBriefFromBlog_(catId, blogSnapshot, topic, draft);
      var content = getDraftContent_(draftId);
      if(content){
        if(!content.images) content.images = { gptVisuals: [], mangoBrief: null };
        content.images.mangoBrief = brief;
      ...
      setAppToast('망고보드 소개를 다시 만들었어요.', { duration: 4500, variant: 'ok' });
    } catch(err){
      console.warn('[망고보드 재생성]', err);
      var msg = (err && err.message) ? err.message : String(err);
      setAppToast('망고보드 재생성에 실패했어요.\n' + msg, { duration: 9000, variant: 'err' });
    } finally {
      delete mangoBgByDraft[draftId];
      genActiveJob = null;
      endGenIndicator();
      var regenBtn = document.querySelector('.sheet-actions .btn-regen');
      if(regenBtn){ regenBtn.disabled = false; regenBtn.textContent = '재생성'; }
      stopButtonCountdown_(regenBtn);
    }
  })();
}
...
function buildCombinedFinalText_(finalTexts){
  var labels = { blog: '블로그', insta: '인스타', threads: '쓰레드', image: '망고보드·이미지 프롬프트', community: '게시판', thread: '일상 공유' };
...
function parseCombinedFinalText_(text){
  ...
  if(!raw) return out;
  var labels = { '블로그': 'blog', '인스타': 'insta', '쓰레드': 'threads', '망고보드·이미지 프롬프트': 'image', '이미지 생성 프롬프트': 'image', '게시판': 'community', '일상 공유': 'thread' };
...
']';
  ...
  var arr = parsePlannerAiJsonValue_(text);
...
<div class="prompt-section">
  ...
  <textarea class="prompt-textarea" id="pt-threads" placeholder="Threads 톤, 반말·구어체, 줄바꿈 리듬, 금지 표현 등을 입력하세요.">${escapeHtml(threadsVal)}</textarea>
  <div class="prompt-hint">기본 말투는 <strong>반말(구어체)</strong>. 블로그 발행 후 자동 생성 · 본문=훅, 댓글=해설.</div>
</div>` : ''}
...
<div class="prompt-section">
  ...
  <textarea class="prompt-textarea" id="pt-image" style="min-height:200px;" placeholder="망고보드·블로그용 사진 슬롯, 구도, 금지 요소, 3장 역할 등을 입력하세요...">${escapeHtml(imageVal)}</textarea>
  <div class="prompt-hint">망고보드 상세페이지: 제품명 30자 · 소개 2000자(7장 · 이미지용 헤드+불릿, 블로그 원문 붙여넣기 X). GPT 영문 이미지 프롬프트는 사용하지 않습니다.</div>
</div>` : ''}
...
function updateGenIndicator() {
  ...
  var prefix = genStatusPrefixForJob_(genActiveJob);
  ...
  if(regenBtn){
    ...
    if(genActiveJob && genActiveJob.draftId === selId){
      ...
      regenBtn.textContent = buttonCountdownText_(genStatusBusyLabel_(genActiveJob), leftR);
  ...
  document.querySelectorAll('.btn-gen-big, .sheet-gen-cta button').forEach(function(genBtn){
    ...
    if(genActiveJob && genActiveJob.draftId === selId){
      ...
      genBtn.textContent = buttonCountdownText_(genStatusBusyLabel_(genActiveJob), leftG);
    } else if(!busy){
...
/** 상단 상태바: 무엇이 생성·재생성 중인지 */
function genStatusPrefixForJob_(job){
  var kind = job && job.kind;
  if(kind === 'mango' || kind === 'images' || kind === 'image') return '망고보드 작성 중';
  if(kind === 'insta') return '인스타 작성 중';
  if(kind === 'threads') return '쓰레드 작성 중';
  if(kind === 'blog') return '블로그·망고보드 작성 중';
  if(kind === 'community') return '게시판 작성 중';
  if(kind === 'thread') return '일상 공유 작성 중';
  return '초안 작성 중';
}
function genStatusBusyLabel_(job){
  var kind = job && job.kind;
  if(kind === 'mango' || kind === 'images' || kind === 'image') return '망고보드 재생성 중';
  if(kind === 'insta') return '인스타 재생성 중';
  if(kind === 'threads') return '쓰레드 재생성 중';
  if(kind === 'blog') return '블로그 재생성 중';
  if(kind === 'community') return '게시판 재생성 중';
  if(kind === 'thread') return '일상 공유 재생성 중';
  return '재생성 중';
}
...
if(opts.userInitiated){
  var toastLabel = '초안';
  if(opts.statusKind === 'blog') toastLabel = '블로그·망고보드 초안';
  else if(opts.statusKind === 'community') toastLabel = '게시판 초안';
  else if(opts.statusKind === 'thread') toastLabel = '일상 공유 초안';
  setAppToast('백그라운드에서 ' + toastLabel + '을 만들고 있어요.\n다른 카드나 브라우저 탭에서도 순서대로 처리되며, 완료되면 알려드릴게요.', { duration: 3000 });
  closeSheet();
}
beginGenIndicator();
...
genActiveJob = {
  topic: draft.topic || '',
  endMs: Date.now() + estimateDraftMs(catId),
  draftId: jobDraftId,
  catId: catId,
  kind: opts.statusKind || opts.kind || (isDailyShareCategory(catId) ? 'thread' : (isHeiljagyaeCategory(catId) ? 'community' : 'blog'))
};
updateGenIndicator();
try {
...
이 카테고리는 **아파트너 게시판용 community**만 만듭니다. images는 {"gptVisuals":[],"mangoBrief":null}. blog, insta, thread 키는 JSON에 넣지 마세요.
...
images.gptVisuals는 **빈 배열 []**. 영문 GPT 프롬프트 작성 금지.`;
...
이 카테고리는 **blog(3단 구조)**와 **images(mangoBrief만, gptVisuals는 [])** 만 만듭니다. insta, notebookLM, community, thread 키는 JSON에 넣지 마세요.
...
반드시 images.gptVisuals는 **빈 배열 []**. 영문 GPT 이미지 프롬프트 작성 금지. mangoBrief.productName·intro·mangoInputPaste는 **비우지 말 것**(intro=망고보드 소개란용 한글 7장 카피).`;
...
이 카테고리는 **blog**와 **images(mangoBrief만, gptVisuals는 [])** 만 만듭니다. insta, notebookLM, community, thread 키는 JSON에 넣지 마세요.
...
반드시 images.gptVisuals는 **빈 배열 []**. 영문 GPT 프롬프트 금지. mangoBrief.productName·intro·mangoInputPaste **필수**(intro=망고보드 소개란용 한글).`;
} else {
...
window.genContent = async function(ev){
  ...
  if(clickBtn && !clickBtn.classList.contains('btn-regen')){
    var busyLabel = clickBtn.classList.contains('btn-gen-big') ? genStatusBusyLabel_({ kind: statusKind }) : '생성 중';
