  ...
  if(byId === 'ops-g-1'){
    ...
    proposalItems = [
      ...
      { id:'basic', title:'전화·주소·업종·지도핀',
        ...
        text: opsProposalWithReason_(buildOpsPlaceBasicInfo_(pk), '약수 전화 010-5967-4596 · 업종 퍼스널트레이닝 유지.') },
...
const OPS_MANUAL_SECTIONS = [
  ...
  { id: 'ops-y-online', branch: 'yaksu', phase: '1. 온라인 채널', items: [
    opsBranchItem_('ops-y-6', '네이버 스마트플레이스 — 기본정보·상세설명·키워드·사진'),
  ...
  { id: 'ops-y-open', branch: 'yaksu', phase: '2. 오픈·운영', items: [
  ...
  { id: 'ops-j-online', branch: 'jakjeon', phase: '1. 온라인 채널', items: [
    opsBranchItem_('ops-j-6', '네이버 스마트플레이스 — 기본정보·상세설명·키워드·사진'),
  ...
  { id: 'ops-j-open', branch: 'jakjeon', phase: '2. 오픈·운영', items: [
...
function renderTabs() {
  ...
  function opsGroupSelectorHTML(){
    ...
    var currentBranch = om.activeBranch;
    var branchProg = countOpsManualProgress_(currentBranch);
    ...
    var options = CAT_GROUP_OPS.map(function(opt){
      ...
      var selected = isActive && currentBranch === opt.branch;
      return '<option value="' + opt.branch + '"' + (selected ? ' selected' : '') + '>' +
        opt.label + ' · ' + count + '</option>';
    }).join('');
    return '<div class="cat-group-select-wrap cat-group-ops' + (isActive ? ' active' : '') + '">' +
      '<span class="cat-group-dot' + (isActive ? ' on ops-dot' : '') + '" aria-hidden="true"></span>' +
      '<label class="cat-group-label">지점브랜딩</label>' +
      '<select class="cat-group-select" onchange="selectOpsBranchFromNav_(this.value)" aria-label="지점브랜딩 선택">' +
      options +
      '</select>' +
      ...
      var proposalHtml = proposalItems.map(function(p){
        return '<div class="ops-review-proposal' + (p.done ? ' done' : '') + '" data-proposal-id="' + escapeHtml(p.id) + '">' +
          ...
          '<div class="ops-review-field">' +
            ...
            '<textarea class="ops-review-input ops-grow-textarea" rows="1" data-ops-text="' + it.id + '-' + p.id + '" oninput="autoGrowTextarea_(this)" onchange="setOpsReviewProposalText_(\'' + it.id + '\', \'' + p.id + '\', this.value)">' + escapeHtml(p.text || '') + '</textarea>' +
          '</div>' +
        '</div>';
      }).join('');
