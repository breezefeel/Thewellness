...
function pruneGeneratedDraftsForQuota_(opts){
  ...
  if(removed){
    state.syncOutbox = (state.syncOutbox || []).filter(function(op){
      return !(op && String(op.key || '').indexOf('generated:') === 0);
    });
    rebuildDirtyStateFromOutbox_();
    state.syncNeedsSnapshot = true;
    state.syncDirty = true;
...
window.pruneQuotaAndContinue_ = function(){
  var n = pruneGeneratedDraftsForQuota_({ keepPublished: true });
  var ok = false;
  try {
    ok = writeLocalStoragePayloadString_(JSON.stringify(getLocalStoragePayload()));
  } catch(e){ ok = false; }
  if(ok){
    var fab = document.getElementById('planner-quota-prune-fab');
    if(fab) fab.remove();
    unlockPlannerBootstrapLocal_({
      toast: n
        ? ('미발행 초안 본문 ' + n + '건을 비웠습니다. 이 기기 데이터로 열었어요.')
        : '저장에 성공했어요. 이 기기 데이터로 열었어요.'
    });
  } else if(typeof setAppToast === 'function'){
    setAppToast('아직도 한도가 부족해요. 주제 카드 자체를 더 삭제해야 할 수 있어요.', { duration: 7000, variant: 'err' });
  }
};
...
function setupPlannerServiceWorker_(){
  ...
  if(!window.__plannerSwControllerBound){
    ...
    navigator.serviceWorker.addEventListener('controllerchange', function(){
      // preferReload:true 는 1MB JS 를 다시 받게 만들어 접속이 수 분씩 늘 수 있음 → 안내만
...
function pruneTopicsWithoutDraft_(catId, opts){
  ...
  if(!opts.skipSave){
    save({ driveImmediate: !opts.skipUpload, gasImmediate: !opts.skipUpload });
  }
  if(!opts.skipRender){
  ...
  return { removed: removedIds.length, kept: kept.length };
}
...
window.pruneTopicsWithoutDraft_ = function(catId){
  ...
  if(!confirm(name + '에서 초안·발행 본문이 없는 주제 ' + n + '개를 삭제할까요?\n(초안 있는 주제·발행 완료 주제는 그대로 둡니다)')) return;
  var result = pruneTopicsWithoutDraft_(catId);
  if(typeof setAppToast === 'function'){
    setAppToast('초안 없는 주제 ' + result.removed + '개를 삭제했어요. 남은 주제 ' + result.kept + '개.', { duration: 5200, variant: 'ok' });
  }
};
