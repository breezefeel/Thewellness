...
function ensurePlanRowStableIdsInPayload_(payload){
  ...
  Object.keys(branding.subGoalPlans || {}).forEach(function(catId){
    ...
    steps.forEach(function(step, i){
      ...
      var next = takeId_(
        step.id,
        'st_mig_' + catId + '_' + i + '_' + String(step.title || '').slice(0, 12).replace(/\s+/g, '')
      );
...
function remapIndexedPlanEntityKey_(key, payload){
  ...
  m = key.match(/^plan:subgoal:([^:]+):step:(\d+)$/);
  if(m){
    var step = ((((branding.subGoalPlans || {})[m[1]] || {}).steps) || [])[parseInt(m[2], 10)];
    return step && step.id != null && String(step.id) !== ''
      ? ('plan:subgoal:' + m[1] + ':step:' + step.id)
      : key;
  }
  return key;
}
...
function getVisibleDraftsInMain_(catId){
  ...
  if(isDailyShareCategory(catId)){
    return (cat.drafts || []).filter(function(d){
      return d && d.id && !draftTombstoned_(state.deletedDraftIds, d);
    }).sort(function(a, b){
  ...
  function pushDraft(d){
    ...
    if(draftTombstoned_(state.deletedDraftIds, d)) return;
    seen[d.id] = true;
    out.push(d);
  }
...
function getDraftRoadmapStepId_(draft, catId, draftIndex){
  ...
  if(isUserAddedDraftId_(draft.id)) return SUBGOAL_MISC_ID;
...
function isValidSubGoalStepId_(plan, stepId){
  var sid = String(stepId || '');
  if(!sid || sid === SUBGOAL_MISC_ID || !plan || !plan.steps) return false;
  return plan.steps.some(function(s){ return String(s.id) === sid; });
}
...
function resolveUserAddedDraftStepAssignment_(draft, catId){
  ...
  var plan = getSubGoalPlan_(catId);
  if(!plan) return null;
  var sid = String(draft.roadmapStepId || '');
  var order = 0;
  var m = String(draft.step || '').match(/^(\d+)\s*\/\s*(\d+)/);
  if(m) order = parseInt(m[1], 10) || 0;
  if(isValidSubGoalStepId_(plan, sid)){
  ...
  if(state.pendingSubGoalPlan && Number(state.pendingSubGoalPlan.catId) === Number(catId)){
    ...
    if(pa && isValidSubGoalStepId_(plan, pa.stepId)){
      var po = parseInt(pa.order, 10) || order || 1;
      return { stepId: String(pa.stepId), order: (po >= 1 && po <= 5) ? po : 1, total: 5 };
    }
  }
  return null;
}
...
function parsePlannerAiJsonObject_(text){
  ...
  try {
    ...
    try {
      ...
      throw new Error((e1 && e1.message) || 'JSON 파싱 실패');
    }
  }
}
...
[대안 모드 — 긴 후기·성찰형 (강의·연수·경험 공유·가치관 주제일 때만)]
- 통념 뒤집기 주제·정보형이면 위 짧은 2단을 쓰고, **강의·연수 후기·경험·성찰** 주제이면 이 모드를 선택하세요(둘 중 하나만).
- 이 모드에서는 **body(본문)** 를 여러 문단의 **긴 단일 흐름**으로 씁니다.
  흐름: 통념 문장 인용·뒤집기 훅 → 현장 관찰·장면 → **핵심 메시지 하나** → (교사·기관 등) **역할의 경계**(진단·교정이 아니라 관찰·연결) → 자세·현상에서 **삶·태도로 한 뼘 확장**.
- 비유는 **1개만**, 가능하면 **처음과 끝에 배치**(수미상관).
- 개인 감정은 **절제된 1문장**까지만("솔직히 꽤 뭉클했어" 정도). 감성 과잉·눈물·시적 수사 금지.
- 이때 **comment(댓글)** 는 본문과 직접 연결된 **여운·질문 1개**(억지 공감 금지) 또는 짧은 보충 한 문단. 본문 요약·반복 금지.
...
[말투 · 채널 기준 — 중요]
- **쓰레드는 개인 채널**입니다. 대상이 교사·기관이어도 **자연스러운 구어체**가 기본입니다(예: "~있어", "~봐"). 정중체는 블로그·기관 공지처럼 격식이 필요한 채널에서만 쓰세요.
- 대상이 예민한 주제(학생·환자 등)일 때는 구어체를 유지하되 **가볍거나 무례하지 않게**, 존중하는 태도로 쓰세요.
...
const THREADS_REFLECTIVE_CHANNEL_ADDENDUM = `[말투 · 채널 기준 — 최우선]
- **쓰레드는 개인 채널**입니다. 대상이 교사·기관이어도 **자연스러운 구어체**가 기본입니다(예: "~있어", "~봐"). 정중체는 블로그·기관 공지처럼 격식이 필요한 채널에서만 쓰세요.
- 예민한 대상(학생·환자 등)은 구어체를 유지하되 가볍거나 무례하지 않게, 존중하는 태도로 쓰세요.

[대안 모드 — 긴 후기·성찰형 (강의·연수·경험 공유·가치관 주제일 때만)]
- 통념 뒤집기·정보형이면 짧은 2단(본문 훅+댓글), **강의·연수 후기·경험·성찰** 주제이면 이 모드를 선택하세요(둘 중 하나).
- 이 모드의 **body(본문)** 는 여러 문단의 **긴 단일 흐름**: 통념 인용·뒤집기 훅 → 현장 관찰·장면 → 핵심 메시지 하나 → 역할의 경계(진단·교정이 아니라 관찰·연결) → 삶·태도로 한 뼘 확장.
- 비유는 1개만, 가능하면 처음과 끝에 배치(수미상관). 개인 감정은 절제된 1문장까지만.
- **comment(댓글)** 는 본문과 직접 연결된 여운·질문 1개 또는 짧은 보충 한 문단. 본문 요약·반복 금지.`;
/** 저장된 쓰레드 프롬프트에 채널 기준 말투 + 긴 후기·성찰형 모드 반영 */
function migrateThreadsReflectiveChannelTone_(){
  if(!state.prompts || !state.prompts.categories) return false;
  var changed = false;
  Object.keys(state.prompts.categories).forEach(function(catId){
    var cat = state.prompts.categories[catId];
    if(!cat || typeof cat !== 'object' || !cat.threads) return;
    if(String(cat.threads).indexOf('[대안 모드 — 긴 후기·성찰형') >= 0) return;
    cat.threads = String(cat.threads).trim() + '\n\n' + THREADS_REFLECTIVE_CHANNEL_ADDENDUM;
    changed = true;
  });
  return changed;
}
...
/** 안정 id 키로 배열 행 upsert. 숫자-only 키는 레거시 index 경로(진행 중 outbox 호환). */
function upsertPlanRowByStableKey_(rows, keyPart, wrapped, missing, touchId){
  ...
  var legacyIndex = /^\d+$/.test(key) && !hasIdMatch;
  if(legacyIndex){
...
function mergeTrackedSyncEntities_(out, local, remote, preferRemote){
  ...
  Object.keys(keys).sort(compareSyncEntityKeysForApply_).forEach(function(key){
    ...
    var chosen = pickRemote ? rEntities[key] : lEntities[key];
...
function indexedPlanEntityForApply_(key){
  ...
  m = key.match(/^plan:subgoal:([^:]+):(?:step:(\d+)|(meta))$/);
  if(m) return { group: 'plan:subgoal:' + m[1], index: m[3] ? -1 : parseInt(m[2], 10) };
  return null;
}
...
function detectPlannerEntityConflicts_(local, remote){
  ...
  Object.keys(keys).forEach(function(key){
    ...
    if(localChanged && remoteChanged && syncValueFingerprint_(lEntities[key]) !== syncValueFingerprint_(rEntities[key])){
...
async function plannerGasPushNow_(){
  ...
  try {
    ...
    if(Array.isArray(data.conflicts) && data.conflicts.length && data.payload){
      ...
      openPlannerOutboxConflict_(data.conflicts, data.payload, data.serverRevision);
...
/** 삭제 tombstone(삭제 시각)이 이 초안의 편집 시각 이후일 때만 삭제로 인정한다.
 *  초안 편집이 삭제보다 더 최신이면 다른 기기에서 살아있는 최신 작업이므로 삭제를 무시(부활)한다.
 *  → "한 기기의 삭제가 다른 기기의 최신 편집을 지우는" 데이터 유실 방지. */
function draftTombstoned_(deleted, draft){
  if(!draft || !draft.id) return false;
  var delTs = deleted && deleted[draft.id];
  if(!delTs) return false;
  var delMs = parseIsoMs_(delTs);
  if(!delMs) return true; // 삭제 시각을 못 읽으면(구버전 tombstone 등) 삭제를 존중
  return draftExtraTimestamp_(draft) <= delMs;
}
/** payload 병합 시, 살아있는(더 최신 편집) 초안에 대한 tombstone을 union 맵에서 제거해 부활을 전파한다. */
function reconcileDeletedDraftIdsAgainstSources_(deletedUnion, sources){
  var out = Object.assign({}, deletedUnion || {});
  if(!Object.keys(out).length) return out;
  (sources || []).forEach(function(byCat){
    if(!byCat || typeof byCat !== 'object') return;
    Object.keys(byCat).forEach(function(k){
      (byCat[k] || []).forEach(function(d){
        if(!d || !d.id || !out[d.id]) return;
        var delMs = parseIsoMs_(out[d.id]);
        if(delMs && draftExtraTimestamp_(d) > delMs) delete out[d.id];
      });
    });
  });
  return out;
}
/** 현재 살아있는 초안(CATEGORIES)보다 오래된 tombstone을 state에서 제거(재삭제·재전파 방지). */
function pruneStaleDraftTombstones_(){
  var deleted = state.deletedDraftIds;
  if(!deleted || !Object.keys(deleted).length) return;
  CATEGORIES.forEach(function(cat){
    (cat.drafts || []).forEach(function(d){
      if(!d || !d.id || !deleted[d.id]) return;
      var delMs = parseIsoMs_(deleted[d.id]);
      if(delMs && draftExtraTimestamp_(d) > delMs) delete deleted[d.id];
    });
  });
}
...
function filterDeletedFromExtraDraftsByCat_(byCat, deleted){
  ...
  var out = {};
  Object.keys(byCat).forEach(function(k){
    out[k] = (byCat[k] || []).filter(function(d){ return d && d.id && !draftTombstoned_(deleted, d); });
  });
  return out;
}
  ...
  function addFrom(src){
    Object.keys(src || {}).forEach(function(k){
      ...
      (src[k] || []).forEach(function(d){
        if(!d || !d.id || draftTombstoned_(deleted, d)) return;
...
function mergePlannerPayloads_(local, remote){
  ...
  var reconciledDeleted = reconcileDeletedDraftIdsAgainstSources_(
    Object.assign({}, remote.deletedDraftIds || {}, local.deletedDraftIds || {}),
    [local.extraDraftsByCat, remote.extraDraftsByCat]
  );
  var out = Object.assign({}, preferRemote ? remote : local, {
    ...
    extraDraftsByCat: mergeExtraDraftsByCat_(local.extraDraftsByCat, remote.extraDraftsByCat, reconciledDeleted, preferRemote),
    deletedDraftIds: reconciledDeleted,
  ...
  if(list){
    ...
    list.innerHTML = conflicts.map(function(conflict, i){
      return '<div class="sync-conflict-item">' +
        '<div class="sync-conflict-name">' + escapeHtml(plannerConflictLabel_(conflict.key)) + '</div>' +
        '<div class="sync-conflict-options">' +
          '<label class="sync-conflict-option"><input type="radio" name="sync-conflict-' + i + '" value="remote" checked>' +
          ...
          '<label class="sync-conflict-option"><input type="radio" name="sync-conflict-' + i + '" value="local">' +
  ...
  if(pending.mode === 'outbox'){
    ...
    pending.conflicts.forEach(function(conflict, i){
      ...
      if(!useLocal){
        ...
        var remoteKey = String(conflict.key || '');
...
function purgeDeletedDraftsFromCatalog_(){
  ...
  if(!deleted || !Object.keys(deleted).length) return;
  CATEGORIES.forEach(function(cat){
    cat.drafts = (cat.drafts || []).filter(function(d){ return d && d.id && !draftTombstoned_(deleted, d); });
  });
}
function collectExtraDrafts(){
  ...
  CATEGORIES.forEach(function(cat, i){
    out[i] = (cat.drafts || []).filter(function(d){
      return d && isUserAddedDraftId_(d.id) && !draftTombstoned_(deleted, d);
    }).map(function(d){
      return {
...
function mergeExtraDrafts(byCat){
  ...
  CATEGORIES.forEach(function(cat, i){
    ...
    incoming.forEach(function(d){
      if(!d || !d.id || draftTombstoned_(state.deletedDraftIds, d)) return;
      cat.drafts.push({
        id: d.id, topic: d.topic || '', angle: d.angle || '',
        series: d.series || '', step: d.step || '', pillar: d.pillar || '', rationale: d.rationale || '',
        writingBrief: d.writingBrief || '',
...
function applyPersistPayload(s, opts){
  ...
  if(migrateThreadsReflectiveChannelTone_()){ save(migOpts); migrated = true; }
...
function renderSheetContent(content) {
  ...
  var undoPeek = hasRegenUndo_(state.selectedId) ? peekRegenUndo_(state.selectedId) : null;
  var redoPeek = hasRegenRedo_(state.selectedId) ? peekRegenRedo_(state.selectedId) : null;
  var undoBusy = !!(
    (state.selectedId && instaBgByDraft[state.selectedId]) ||
    (state.selectedId && threadsBgByDraft[state.selectedId]) ||
    (genActiveJob && genActiveJob.draftId === state.selectedId)
  );
  var undoBtn = (undoPeek && !undoBusy)
    ? '<button type="button" class="btn-undo" onclick="undoLastRegen_()" title="' +
      escapeHtml((undoPeek.label ? undoPeek.label + ' · ' : '') + '재생성 직전으로 되돌리기') + '">되돌리기</button>'
    : '';
  var redoBtn = (redoPeek && !undoBusy)
    ? '<button type="button" class="btn-redo" onclick="redoLastRegen_()" title="' +
      escapeHtml((redoPeek.label ? redoPeek.label + ' · ' : '') + '재생성본으로 앞돌리기') + '">앞돌리기</button>'
    : '';
  setSheetActionsHtml_(
...
async function generateThreadsFromBlog_(catId, blog, topic){
  ...
  var generalHint = isGeneralAudienceCategory(catId)
    ...
    : '';
  var threadsExamplesBlock = buildContentExamplePromptBlock_(catId, 'threads');
  var prompt = baseInfo + '\n\n' +
    '카테고리: ' + (CATEGORIES[catId] ? CATEGORIES[catId].name : '') + '\n' +
    '주제: "' + (topic || '') + '"\n\n' +
    '[쓰레드 작성 지침]\n' + threadsGuide + expertScope + generalHint + threadsExamplesBlock + '\n' +
    '아래 **블로그 글**을 인스타처럼 요약하지 말고 **새로** 작성하세요. 위 지침의 모드(짧은 2단 / 긴 후기·성찰형) 중 **주제에 맞는 하나**를 고르세요.\n\n' +
    'JSON만:\n{"threads":{"body":"본문(게시글) — 짧은 통념 뒤집기 훅 한 줄, 또는 긴 후기·성찰형 흐름","comment":"댓글(재게시용) — 해설·근거·과정, 또는 본문과 연결된 여운·질문 1개"}}\n\n' +
...
function sheetFullCopyBar_(){
  var exampleType = (state.activeTab === 'blog' || state.activeTab === 'threads') ? state.activeTab : '';
  var exampleBtn = '';
  if(exampleType){
    var savedCount = listContentExamples_(exampleType).length;
    exampleBtn = '<button type="button" class="sheet-example-btn" onclick="saveContentExampleFromDraft_(\'' + exampleType + '\')" title="이 글을 모범 예시로 저장하면 재생성 때 톤·구조 참고로 쓰여요">★ 예시로 저장' +
      (savedCount ? ' <span class="sheet-example-count">' + savedCount + '</span>' : '') + '</button>';
  }
...
window.regenSheetField_ = async function(key, btn){
  ...
  try {
    ...
    var val = obj.value;
    captureRegenUndo_(draftId, { kind: 'field', fieldKey: key, label: meta.label || key });
    if(meta.array){
...
function persistDraftContent_(draftId, content){
  ...
  // 콘텐츠 저장 시 소유 초안의 편집 시각을 갱신 → 다른 기기의 오래된 삭제 tombstone이
  //   방금 작성/재생성한 내용을 지우지 못하도록 부활 가드(draftTombstoned_)를 강화한다.
  try {
    for(var ci = 0; ci < CATEGORIES.length; ci++){
      var d = (CATEGORIES[ci].drafts || []).find(function(x){ return x && x.id === draftId; });
      if(d){ d.updatedAt = new Date().toISOString(); break; }
    }
  } catch(e){}
  save({ driveImmediate: true, gasImmediate: true });
}

/* ── 재생성 되돌리기·앞돌리기 (이 기기 localStorage, 초안당 최대 2개) ── */
const REGEN_UNDO_LS_KEY = 'ht_regen_undo_v1';
const REGEN_UNDO_MAX = 2;
function contentUndoFingerprint_(content){
  try { return JSON.stringify(content || null); } catch(e){ return ''; }
}
function readRegenUndoStore_(){
  try {
    var raw = localStorage.getItem(REGEN_UNDO_LS_KEY);
    if(!raw) return {};
    var parsed = JSON.parse(raw);
    return parsed && typeof parsed === 'object' ? parsed : {};
  } catch(e){ return {}; }
}
function writeRegenUndoStore_(store){
  try {
    localStorage.setItem(REGEN_UNDO_LS_KEY, JSON.stringify(store || {}));
    return true;
  } catch(e){
    if(typeof setAppToast === 'function'){
      setAppToast('되돌리기 백업 저장에 실패했어요.\n저장 공간이 부족할 수 있어요.', { duration: 4500, variant: 'err' });
    }
    return false;
  }
}
function normalizeRegenHistoryBucket_(raw){
  if(!raw) return { undo: [], redo: [] };
  if(Array.isArray(raw)) return { undo: raw.slice(), redo: [] };
  return {
    undo: Array.isArray(raw.undo) ? raw.undo.slice() : [],
    redo: Array.isArray(raw.redo) ? raw.redo.slice() : []
  };
}
function getRegenHistoryBucket_(store, draftId){
  return normalizeRegenHistoryBucket_(store[draftId]);
}
function setRegenHistoryBucket_(store, draftId, bucket){
  var undo = (bucket.undo || []).slice(0, REGEN_UNDO_MAX);
  var redo = (bucket.redo || []).slice(0, REGEN_UNDO_MAX);
  if(!undo.length && !redo.length) delete store[draftId];
  else store[draftId] = { undo: undo, redo: redo };
}
function makeRegenHistoryEntry_(entry){
  return {
    at: entry.at || new Date().toISOString(),
    kind: entry.kind || 'draft',
    fieldKey: entry.fieldKey || null,
    label: entry.label || '초안',
    content: cloneSyncValue_(entry.content)
  };
}
function pushRegenStackEntry_(list, entry){
  if(!entry || !entry.content) return list || [];
  var next = (list || []).slice();
  var fp = contentUndoFingerprint_(entry.content);
  if(next[0] && contentUndoFingerprint_(next[0].content) === fp) return next;
  next.unshift(makeRegenHistoryEntry_(entry));
  if(next.length > REGEN_UNDO_MAX) next = next.slice(0, REGEN_UNDO_MAX);
  return next;
}
function pushRegenUndoEntry_(draftId, entry){
  if(!draftId || !entry || !entry.content) return false;
  var store = readRegenUndoStore_();
  var bucket = getRegenHistoryBucket_(store, draftId);
  bucket.undo = pushRegenStackEntry_(bucket.undo, entry);
  bucket.redo = [];
  setRegenHistoryBucket_(store, draftId, bucket);
  return writeRegenUndoStore_(store);
}
function peekRegenUndo_(draftId){
  if(!draftId) return null;
  return getRegenHistoryBucket_(readRegenUndoStore_(), draftId).undo[0] || null;
}
function peekRegenRedo_(draftId){
  if(!draftId) return null;
  return getRegenHistoryBucket_(readRegenUndoStore_(), draftId).redo[0] || null;
}
function hasRegenUndo_(draftId){ return !!peekRegenUndo_(draftId); }
function hasRegenRedo_(draftId){ return !!peekRegenRedo_(draftId); }
function applyRegenHistoryRestore_(draftId, entry, toastMsg){
  if(!draftId || !entry || !entry.content) return false;
  persistDraftContent_(draftId, entry.content);
  var ov = document.getElementById('detail-overlay');
  if(ov && ov.classList.contains('open') && state.selectedId === draftId){
    renderSheetContent(entry.content);
  }
  renderTabs();
  renderMain();
  if(typeof setAppToast === 'function'){
    setAppToast(toastMsg || '내용을 바꿨어요.\n서버에도 반영됩니다.', { duration: 4000, variant: 'ok' });
  }
  return true;
}
/** 재생성으로 덮어쓰기 직전 호출 — 현재 초안 스냅샷을 undo 스택에 보관 */
function captureRegenUndo_(draftId, meta){
  meta = meta || {};
  var content = getDraftContent_(draftId);
  if(!content) return false;
  return pushRegenUndoEntry_(draftId, {
...
window.undoLastRegen_ = function(){
  var draftId = state.selectedId;
  if(!draftId){
    if(typeof setAppToast === 'function') setAppToast('먼저 초안을 열어 주세요.', { duration: 3000, variant: 'err' });
    return;
  }
  if(!peekRegenUndo_(draftId)){
    if(typeof setAppToast === 'function') setAppToast('되돌릴 내용이 없어요.', { duration: 3000, variant: 'err' });
    return;
  }
  var store = readRegenUndoStore_();
  var bucket = getRegenHistoryBucket_(store, draftId);
  if(!bucket.undo.length) return;
  var target = bucket.undo.shift();
  var current = getDraftContent_(draftId);
  if(current){
    bucket.redo = pushRegenStackEntry_(bucket.redo, {
      kind: 'redo',
      label: (target && target.label) ? (String(target.label).replace(/\s·\s재생성본$/, '') + ' · 재생성본') : '재생성본',
      content: current
    });
  }
  setRegenHistoryBucket_(store, draftId, bucket);
  writeRegenUndoStore_(store);
  applyRegenHistoryRestore_(draftId, target, '직전 내용으로 되돌렸어요.\n앞돌리기로 재생성본과 비교할 수 있어요.');
};
window.redoLastRegen_ = function(){
  var draftId = state.selectedId;
  if(!draftId){
    if(typeof setAppToast === 'function') setAppToast('먼저 초안을 열어 주세요.', { duration: 3000, variant: 'err' });
    return;
  }
  if(!peekRegenRedo_(draftId)){
    if(typeof setAppToast === 'function') setAppToast('앞돌릴 내용이 없어요.', { duration: 3000, variant: 'err' });
    return;
  }
  var store = readRegenUndoStore_();
  var bucket = getRegenHistoryBucket_(store, draftId);
  if(!bucket.redo.length) return;
  var target = bucket.redo.shift();
  var current = getDraftContent_(draftId);
  if(current){
    bucket.undo = pushRegenStackEntry_(bucket.undo, {
      kind: 'undo',
      label: (target && target.label) ? (String(target.label).replace(/\s·\s재생성본$/, '') || '재생성 직전') : '재생성 직전',
      content: current
    });
  }
  setRegenHistoryBucket_(store, draftId, bucket);
  writeRegenUndoStore_(store);
  applyRegenHistoryRestore_(draftId, target, '재생성본으로 앞돌렸어요.\n되돌리기로 다시 비교할 수 있어요.');
};
...
/* ── 모범 예시 저장·few-shot (branding.contentExamples 로 동기화) ── */
const CONTENT_EXAMPLE_MAX_PER_TYPE = 3;
const CONTENT_EXAMPLE_FEWSHOT_MAX = 2;
/** 쓰기 전용: 저장 액션에서만 호출 (state를 변형해 저장소를 보장) */
function getContentExamplesStore_(){
  if(!state.branding || typeof state.branding !== 'object') state.branding = {};
  var ex = state.branding.contentExamples;
  if(!ex || typeof ex !== 'object'){
    ex = { blog: [], threads: [] };
    state.branding.contentExamples = ex;
  }
  if(!Array.isArray(ex.blog)) ex.blog = [];
  if(!Array.isArray(ex.threads)) ex.threads = [];
  return ex;
}
/** 읽기 전용: state를 절대 변형하지 않음 (빈 객체 생성 금지 → 동기화로 타 기기 예시 덮어쓰기 방지) */
function listContentExamples_(type){
  var b = state.branding;
  var ex = (b && typeof b === 'object') ? b.contentExamples : null;
  if(!ex || typeof ex !== 'object') return [];
  return Array.isArray(ex[type]) ? ex[type] : [];
}
/** 블로그 구조가 카테고리 종류별로 달라, 같은 종류(일반/전문가/기타)끼리만 예시 재사용 */
function contentExampleKindGroup_(catId){
  var id = parseInt(catId, 10);
  if(isGeneralAudienceCategory(id)) return 'general';
  if(isExpertCourseCategory(id)) return 'expert';
  if(isDailyShareCategory(id)) return 'daily';
  if(isHeiljagyaeCategory(id)) return 'heiljagyae';
  return 'etc';
}
function makeContentExampleId_(){
  return 'ex_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
}
function extractBlogExampleText_(content, catId){
  if(!content || !content.blog) return '';
  return buildBlogSourceText_(content.blog, catId);
}
function extractThreadsExample_(content){
  if(!content || !content.threads) return null;
  var body = getThreadsBodyText_(content.threads);
  var comment = getThreadsCommentText_(content.threads);
  if(!body && !comment) return null;
  return { body: body, comment: comment };
}
/** 현재 초안의 블로그/쓰레드를 모범 예시로 저장 */
window.saveContentExampleFromDraft_ = function(type){
  var draftId = state.selectedId;
  if(!draftId){ setAppToast('먼저 초안을 열어 주세요.', { duration: 3000, variant: 'err' }); return; }
  var content = getDraftContent_(draftId);
  if(content) applySheetEditsForTab_(content, state.activeTab);
  if(!content){ setAppToast('저장할 내용이 없어요.', { duration: 3000, variant: 'err' }); return; }
  var catId = state.selectedCatId != null ? state.selectedCatId : getCatIdFromDraftId_(draftId);
  var cat = CATEGORIES[catId];
  var draft = cat && cat.drafts.find(function(d){ return d.id === draftId; });
  var topic = draft ? (draft.topic || '') : '';
  var ex = getContentExamplesStore_();
  var entry;
  if(type === 'blog'){
    var text = extractBlogExampleText_(content, catId);
    if(!text || text.replace(/제목:|문제 제기:|셀프 케어:|원리 설명:|마무리 CTA:|후킹 오프닝:|목차:|본문:|영상·사진 맥락:|시연·핵심 포인트:|마무리:/g, '').trim().length < 30){
      setAppToast('블로그 내용이 충분하지 않아요.', { duration: 3500, variant: 'err' }); return;
    }
    entry = { id: makeContentExampleId_(), catId: catId, kind: contentExampleKindGroup_(catId), topic: topic, savedAt: new Date().toISOString(), text: text };
  } else if(type === 'threads'){
    var th = extractThreadsExample_(content);
    if(!th || (th.body + th.comment).trim().length < 20){
      setAppToast('쓰레드 내용이 충분하지 않아요.', { duration: 3500, variant: 'err' }); return;
    }
    entry = { id: makeContentExampleId_(), catId: catId, kind: contentExampleKindGroup_(catId), topic: topic, savedAt: new Date().toISOString(), body: th.body, comment: th.comment };
  } else {
    return;
  }
  var list = ex[type];
  function exampleFp_(e){
    return String(type === 'blog' ? (e.text || '') : ((e.body || '') + '\u0001' + (e.comment || ''))).trim();
  }
  var newFp = exampleFp_(entry);
  var dup = list.some(function(e){ return exampleFp_(e) === newFp; });
  if(dup){ setAppToast('이미 같은 내용이 예시로 저장돼 있어요.', { duration: 3500, variant: 'ok' }); return; }
  list.unshift(entry);
  if(list.length > CONTENT_EXAMPLE_MAX_PER_TYPE) ex[type] = list.slice(0, CONTENT_EXAMPLE_MAX_PER_TYPE);
  save({ driveImmediate: true, gasImmediate: true });
  renderSheetContent(content);
  setAppToast((type === 'blog' ? '블로그' : '쓰레드') + '를 모범 예시로 저장했어요.\n재생성 때 톤·구조 참고로 쓰여요.', { duration: 4200, variant: 'ok' });
};
window.removeContentExample_ = function(type, id){
  var ex = getContentExamplesStore_();
  if(!Array.isArray(ex[type])) return;
  var before = ex[type].length;
  ex[type] = ex[type].filter(function(e){ return e.id !== id; });
  if(ex[type].length === before) return;
  save({ driveImmediate: true, gasImmediate: true });
  var content = getDraftContent_(state.selectedId);
  if(content) renderSheetContent(content);
  setAppToast('예시를 삭제했어요.', { duration: 2600, variant: 'ok' });
};
/** 재생성 프롬프트에 붙일 few-shot 예시 블록 (같은 종류 우선, 최대 N개) */
function buildContentExamplePromptBlock_(catId, type){
  var list = listContentExamples_(type);
  if(!list.length) return '';
  var group = contentExampleKindGroup_(catId);
  var sameCat = list.filter(function(e){ return parseInt(e.catId, 10) === parseInt(catId, 10); });
  var sameKind = list.filter(function(e){ return (e.kind || contentExampleKindGroup_(e.catId)) === group; });
  var picked = [];
  function addFrom(arr){
    arr.forEach(function(e){
      if(picked.length >= CONTENT_EXAMPLE_FEWSHOT_MAX) return;
      if(picked.indexOf(e) === -1) picked.push(e);
    });
  }
  addFrom(sameCat);
  // 블로그는 구조가 종류별로 달라 같은 종류만, 쓰레드는 공통이라 전체 허용
  addFrom(type === 'blog' ? sameKind : list);
  if(!picked.length) return '';
  var blocks = picked.map(function(e, i){
    if(type === 'blog'){
      return '예시 ' + (i + 1) + ')\n' + String(e.text || '').trim();
    }
    var parts = [];
    if(e.body) parts.push('[본문]\n' + String(e.body).trim());
    if(e.comment) parts.push('[댓글]\n' + String(e.comment).trim());
    return '예시 ' + (i + 1) + ')\n' + parts.join('\n');
  });
  return '\n\n[모범 예시 — 톤·구조·흐름만 참고. 주제·문장·사실을 그대로 베끼지 말 것]\n' + blocks.join('\n\n') + '\n';
}
...
let blogGuide = getCatPromptForGeneration_(catId, 'blog');
if(blogGuide){
  var blogExamplesBlock = buildContentExamplePromptBlock_(catId, 'blog');
  if(blogExamplesBlock) blogGuide = blogGuide + blogExamplesBlock;
}
...
const text = await callClaudePlanner_(prompt, {
...
if(getDraftContent_(jobDraftId)){
  captureRegenUndo_(jobDraftId, { kind: 'draft', label: opts.userInitiated ? '초안 전체' : '초안' });
}
...
window.genContent = async function(ev){
  ...
  if(isBlogInstaCategory(catId) && tab === 'insta'){
    ...
    var cat = CATEGORIES[catId];
    var draft = cat && cat.drafts.find(function(d){ return d.id === draftId; });
    if(igContent.insta) captureRegenUndo_(draftId, { kind: 'block', label: '인스타 캡션' });
  ...
  if(isBlogInstaCategory(catId) && tab === 'threads'){
    ...
    if(thBlogContent.threads) captureRegenUndo_(draftId, { kind: 'block', label: '쓰레드 글' });
