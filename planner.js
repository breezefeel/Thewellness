 ...
 * (화면의 18~24번처럼 동일 제목 반복 + 발행 0/0 고스트 블록)
 */
function dedupeSubGoalPlanStepsInPlace_(plan){
  if(!plan || !Array.isArray(plan.steps)) return false;
  var seenId = {};
  var seenTitle = {};
  var out = [];
  ...
  plan.steps.forEach(function(step, i){
    ...
    if(seenId[nid]){ changed = true; return; }
    var tkey = stepTitleDedupeKey_(step.title);
    if(tkey && seenTitle[tkey]){ changed = true; return; }
    seenId[nid] = true;
    if(tkey) seenTitle[tkey] = true;
...
function dedupeAllSubGoalPlanSteps_(){
  ...
  var changed = false;
  Object.keys(state.branding.subGoalPlans).forEach(function(k){
    if(dedupeSubGoalPlanStepsInPlace_(state.branding.subGoalPlans[k])) changed = true;
  });
  if(state.pendingSubGoalPlan && state.pendingSubGoalPlan.plan){
    if(dedupeSubGoalPlanStepsInPlace_(state.pendingSubGoalPlan.plan)) changed = true;
  }
  return changed;
}
function getSubGoalPlan_(catId){
  ...
  if(dedupeSubGoalPlanStepsInPlace_(plan)){
    plan.updatedAt = new Date().toISOString();
    if(!state._dedupeStepsSaveScheduled){
      ...
      setTimeout(function(){
        ...
        try {
          save({ skipDriveUpload: true, skipGasPush: false, skipMarkDirty: false });
        } catch(eSave){}
      }, 0);
    }
  }
...
function upsertPlanRowByStableKey_(rows, keyPart, wrapped, missing, touchId){
  ...
  if(found < 0 && wrapped && wrapped.value && wrapped.value.title){
    ...
    for(i = 0; i < rows.length; i++){
      if(rows[i] && String(rows[i].title || '').trim() === title){ found = i; break; }
    }
  }
  if(missing){
...
function schedulePlannerGasPush_(immediate){
  ...
  // 부팅 직후 즉시 푸시하면 JSON.stringify 로 「응답 없는 페이지」가 남
...
function estimatePlannerLocalStorageBytes_(){
  try {
    var raw = localStorage.getItem(SK) || '';
    // UTF-16 roughly 2 bytes/char in localStorage accounting
    return raw.length * 2;
  } catch(e){ return 0; }
}
...
function persistPlannerUnderQuota_(opts){
  ...
  if(ok){
    ...
    var fab = document.getElementById('planner-quota-prune-fab');
    if(fab) fab.remove();
...
function writeLocalStoragePayloadString_(payloadStr){
  function isQuotaErr_(err){
    if(!err) return false;
    var name = String(err.name || '');
    var msg = String(err.message || err);
    return name === 'QuotaExceededError' || name === 'NS_ERROR_DOM_QUOTA_REACHED' ||
      /quota|exceeded|저장 공간|enough storage/i.test(msg);
  }
  function failUi_(bytesHint){
    plannerStorageWriteFailed_ = true;
    var mb = bytesHint ? (bytesHint / (1024 * 1024)).toFixed(1) : '?';
    var tip =
      '브라우저 사이트 저장 한도(보통 5~10MB)를 넘었어요.\n' +
      '휴대폰/PC 용량(수십 GB)과는 별개입니다.\n' +
      '현재 플래너 데이터 약 ' + mb + 'MB · 미발행 AI 초안 본문을 비우면 다시 저장할 수 있어요.';
    if (typeof setAppToast === 'function') {
      setAppToast(tip, { duration: 9000, variant: 'err' });
    } else {
      alert(tip);
    }
    // 이미 편집 중이면 부트스트랩 잠금( inert )으로 앱을 막지 않음
    if(plannerSyncBootstrapReady_){
      ensureQuotaRecoveryFab_();
      return;
    }
    setPlannerBootstrapUi_('failed',
      '브라우저 저장 한도를 초과했어요 (디스크 용량 부족이 아닙니다).\n' +
      '「이 기기 데이터로 열기」 후, 리:얼 무브먼트 등에서 안 쓰는 생성 초안을 지워 주세요.\n' +
  ...
  try {
    ...
    if(!isQuotaErr_(e0)){
      try {
        ...
        failUi_(estimatePlannerLocalStorageBytes_() || (payloadStr.length * 2));
        return false;
      }
    }
    // 1회: 미발행 생성 초안 본문 정리 후 재시도
    var pruned = 0;
    ...
    if(pruned > 0){
      try {
        ...
        plannerStorageWriteFailed_ = false;
        // 이전에 한도 초과로 부트스트랩 실패 잠금이 걸려 있었다면 풀어 줌
        var bootOv = document.getElementById('sync-bootstrap-overlay');
        if(bootOv && bootOv.classList.contains('open') && bootOv.classList.contains('failed')){
          unlockPlannerBootstrapLocal_({
            toast: '브라우저 저장 한도 때문에 미발행 AI 초안 본문 ' + pruned +
              '건을 비우고 저장했어요. 이 기기 데이터로 열었어요.'
          });
        } else if(typeof setAppToast === 'function'){
          setAppToast('브라우저 저장 한도 때문에 미발행 AI 초안 본문 ' + pruned +
            '건을 비우고 저장했어요.\n주제 카드는 남아 있고, 필요하면 다시 생성하면 됩니다.', { duration: 8000, variant: 'ok' });
        }
        return true;
      } catch(e2){}
    }
...
/** 편집 중 한도 초과 시 — 부트스트랩을 다시 잠그지 않고 플로팅 복구 버튼만 표시 */
function ensureQuotaRecoveryFab_(){
  if(document.getElementById('planner-quota-prune-fab')) return;
  var btn = document.createElement('button');
  btn.type = 'button';
  btn.id = 'planner-quota-prune-fab';
  btn.textContent = '저장 한도 초과 · 초안 본문 비우고 저장';
  btn.setAttribute('aria-label', '미발행 초안 본문을 비우고 다시 저장');
  btn.style.cssText = 'position:fixed;left:12px;right:12px;bottom:16px;z-index:2600;padding:14px 16px;border:0;border-radius:12px;background:#92400E;color:#FFFBEB;font-weight:800;font-size:14px;box-shadow:0 8px 24px rgba(0,0,0,.18);cursor:pointer;touch-action:manipulation;';
  btn.onclick = function(){
    pruneQuotaAndContinue_();
    var fab = document.getElementById('planner-quota-prune-fab');
    if(fab && !plannerStorageWriteFailed_) fab.remove();
  };
  document.body.appendChild(btn);
}
window.pruneQuotaAndContinue_ = function(){
  var result = persistPlannerUnderQuota_({ skipPush: false });
  if(result.ok){
    var fab = document.getElementById('planner-quota-prune-fab');
    if(fab) fab.remove();
    unlockPlannerBootstrapLocal_({
      toast: result.stripped
        ? ('저장 한도를 비웠습니다 (정리 ' + result.stripped + '건). 이 기기 데이터로 열었어요.')
        : '저장에 성공했어요. 이 기기 데이터로 열었어요.'
    });
  } else if(typeof setAppToast === 'function'){
    setAppToast('아직도 한도가 부족해요. 브라우저 사이트 데이터를 지우거나, 주제 카드를 더 삭제해 주세요.', { duration: 7000, variant: 'err' });
  }
};
...
function bootPlannerLocalState_(){
  if(state._bootLocalDone) return;
  ...
  var raw = '';
  try { raw = localStorage.getItem(SK) || ''; } catch(eRaw){ raw = ''; }
  if(!raw){
    try { applyBuiltinDraftBrandSeeds_(); } catch(e0){}
    return;
  }
  var origLen = raw.length;
  // 임계값 대폭 하향: 중간 크기부터 parse 전에 generatedOnly 제거
  var heavy = origLen > 60000;
  var critical = origLen > 100000;
  if(critical){
    try {
      var beforeLen = raw.length;
      raw = replaceJsonTopLevelObjectProp_(raw, 'generatedOnly', '{}');
      raw = replaceJsonTopLevelObjectProp_(raw, '_syncOutbox', '[]');
      // published 본문까지 거대하면 메타만 남기기엔 위험 → generatedOnly만
      if(raw.length < beforeLen){
        state._bootStrippedGenerated = Math.max(1, Math.floor((beforeLen - raw.length) / 4000));
      }
    } catch(eStripRaw){}
  }
  var s = null;
  ...
  if(critical && s.generatedOnly && Object.keys(s.generatedOnly).length){
    try {
      state._bootStrippedGenerated = Math.max(
        state._bootStrippedGenerated || 0,
        stripGeneratedContentBodiesInPayload_(s)
      );
    } catch(eStrip){}
  }
  try {
  ...
  if(state._bootStrippedGenerated > 0){
    ...
    setTimeout(function(){
      try {
        save({ skipDriveUpload: true, skipGasPush: true, skipMarkDirty: true, skipEntityStamp: true, forceWrite: true });
        if(typeof setAppToast === 'function'){
          setAppToast('빠른 접속을 위해 미발행 AI 초안 본문을 비웠어요.\n주제 카드는 남아 있고, 필요하면 다시 생성하면 됩니다.', { duration: 9000, variant: 'ok' });
        }
      } catch(eSave){}
    }, 2000);
  }
...
window.onload = () => {
  ...
  var startUi_ = function(){
    try { bootPlannerLocalState_(); } catch(eBoot){ console.warn('[boot]', eBoot); }
 ...
 */
function pruneTopicsWithoutDraft_(catId, opts){
  opts = opts || {};
  var cat = CATEGORIES[catId];
  if(!cat) return { removed: 0, kept: 0 };
  var removedIds = [];
  var kept = [];
  (cat.drafts || []).forEach(function(d){
    if(!d || !d.id) return;
  ...
  if(!removedIds.length) return { removed: 0, kept: kept.length };
  cat.drafts = kept;
  if(!state.deletedDraftIds) state.deletedDraftIds = {};
  var now = new Date().toISOString();
  ...
  if(!opts.skipSave){
    // 일반 save 금지 — 7MB fingerprint/parse 로 멈추고, 한도 때문에 삭제가 롤백처럼 보임
    // onlyCatId: 우선 이 탭 초안만 비움 (다른 탭 미발행 초안 보호)
...
window.pruneTopicsWithoutDraft_ = function(catId){
  ...
  if(typeof setAppToast === 'function') setAppToast('주제 삭제·저장 정리 중…', { duration: 2500, variant: 'ok' });
  setTimeout(function(){
    var result = pruneTopicsWithoutDraft_(catId, { skipUpload: false });
    if(typeof setAppToast === 'function'){
      if(result.saved){
        setAppToast(
          '초안 없는 주제 ' + result.removed + '개를 삭제했어요. 남은 주제 ' + result.kept + '개.' +
          (result.stripped ? '\n저장 한도 때문에 미발행 초안 본문도 함께 정리했습니다.' : ''),
          { duration: 6500, variant: 'ok' }
        );
      } else {
        setAppToast(
