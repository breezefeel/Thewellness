...
function remapIndexedPlanEntityKey_(key, payload){
  ...
  if(m){
    ...
    // 이미 step.id 로 저장된 현대 키는 그대로 둔다 (숫자 id를 index로 오인하면 step 유실)
    for(var si = 0; si < catSteps.length; si++){
      if(catSteps[si] && String(catSteps[si].id) === keyPart) return key;
    }
    // 레거시: 키가 배열 index 이던 경우만 id로 승격
    if(/^\d+$/.test(keyPart)){
      var step = catSteps[parseInt(keyPart, 10)];
      if(step && step.id != null && String(step.id) !== ''){
        return 'plan:subgoal:' + m[1] + ':step:' + step.id;
      }
    }
    return key;
  }
...
function ensurePendingSubGoalPlanFromCurrent_(catId){
  ...
  if(state.pendingSubGoalPlan && sameCatId_(state.pendingSubGoalPlan.catId, catId)){
    ...
    // 1~2단계만 남은 깨진 pending은 버리고 적용본(복구본)을 다시 올림
    if(pendingSteps.length > 0 && pendingSteps.length < 3){
      state.pendingSubGoalPlan = null;
      persistPendingSubGoalPlan_();
    } else {
...
function renderProgramRoadmapHTML_(catId){
  ...
  if(!cat) return '';
  try {
    if(repairIncompleteSubGoalPlan_(catId)){
      save({ driveImmediate: true, gasImmediate: true });
    }
  } catch(eRepMain){}
  var plan = getEffectiveSubGoalPlan_(catId);
