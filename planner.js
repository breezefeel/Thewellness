...
function buildImageGptVisualsJsonExample_(catId, opts){
  ...
  var mangoBriefEx =
    ...
    '"designTone":"' + MANGO_DESIGN_TONE_LINE.replace(/"/g, '\\"') + '",\n' +
...
/** 망고보드 상세페이지 비주얼·카피 — 소개란 끝에 붙여 넣는 지침 */
const MANGO_DESIGN_TONE_LINE =
  '색: 배경 #f0efeb·#eee9de(소프트 아이보리·웜 크림). ' +
  '글·헤드 #3d2c27. 카드·박스 #f7efe9·#f5ebe4. ' +
  '포인트 #6b493d. 구분선 #3d2c27. ' +
  '흑·백 모노톤·민트·세이지·초록·티얼·알록달록·만화·코믹 금지. 로고는 후처리.';
const MANGO_LAYOUT_COPY_RULE = `[레이아웃·카피 — 망고보드 생성 시 필수]
...
- 큰 대전제(한 줄 훅)를 먼저 두고, 그 아래만 짧게. (표지형·원 포인트 슬라이드 환영)
...
- 각 장 카피: 1헤드 + 1~2문장 또는 불릿 3개 이내.
- 색은 #f0efeb·#eee9de 배경, 헤드·구분선 #3d2c27, 포인트 #6b493d, 카드 #f7efe9·#f5ebe4. 흑백 모노·민트·초록 배경 금지.
- 참고사진: 여성·전신·일상/시연 동작 컷 (잘린 제품샷·글자 캡처 금지).`;

const MANGO_LAYOUT_PASTE_BLOCK =
  '표·긴 설명표 금지. 주의·안내형·짧은 STEP·픽토그램 OK.\n' +
  '장마다 헤드 1줄+불릿 1~3개. 블로그 원문·말줄임(…) 붙여넣기 금지. 장별 역할 중복 금지.\n' +
  '큰 대전제 한 줄 표지. 쉬운 말만(가동성→움직이는 범위, 안정성→버티는 힘).\n' +
  '인물=여성 실사 우선. 중년 남성 반복 지양.\n' +
  '전/후 환영. 추상 한발서기만 반복 금지. 일상 동작 비교(앉았다 일어서기·계단·방향 전환 등). 사진만 봐도 비교 이해.\n' +
  '색: 배경 #f0efeb·#eee9de · 헤드·구분선 #3d2c27 · 포인트 #6b493d · 카드 #f7efe9·#f5ebe4. 흑백 모노·민트·초록 금지.\n' +
  '참고사진: 여성·전신·일상 동작 컷.';
...
const MANGO_DETAIL_BRIEF_RULE = `[망고보드 AI 상세페이지 입력 — 필수 형식]
...
레이아웃·톤도 함께 반영:
${MANGO_LAYOUT_COPY_RULE}

intro 맨 끝 필수 (짧게만 — 망고 붙여넣기용):
[디자인]
${MANGO_DESIGN_TONE_LINE}

[레이아웃]
${MANGO_LAYOUT_PASTE_BLOCK}
...
const DEFAULT_HEILJAGYAE_IMAGE_PROMPT = `힐자계 아파트너 — 이미지 GPT 프롬프트 불필요. gptVisuals는 []. 게시글용 사진은 실사·시연 컷을 직접 사용합니다.`;
...
- intro **맨 끝**에 [디자인]+[레이아웃] (배경 #f0efeb·#eee9de, 헤드·구분선 #3d2c27, 포인트 #6b493d, 카드 #f7efe9·#f5ebe4, 흑백 모노·민트 금지, 표 금지, 쉬운 말, 여성 모델, 한발서기만 X·일상 전/후)
- photoTip: 실사·시연 (가능하면 여성) 1~2문장
- slidePlan: 정확히 7개 — 각 장은 한 메시지, 표 없음, copyHint는 짧은 헤드
- designTone: ${MANGO_DESIGN_TONE_LINE}
...
function formatMangoBriefPaste_(brief){
  if(!brief) return '';
  // 소개란 = intro/mangoInputPaste + 디자인 블록(망고보드가 색을 읽도록)
  return ensureMangoIntroHasDesignBlock_(brief.intro || brief.mangoInputPaste || '');
}
...
/** 소개 끝에 [디자인]·[레이아웃] 지침이 없으면 붙임 */
function ensureMangoIntroHasDesignBlock_(intro){
  var body = clampMangoIntro_(intro);
  var designBlock =
    '[디자인]\n' + MANGO_DESIGN_TONE_LINE +
    '\n\n[레이아웃]\n' + MANGO_LAYOUT_PASTE_BLOCK;
  if(!body){
  ...
  var hasLayout = /\[레이아웃\]/.test(body) && /#f0efeb|#eee9de|#6b493d|#3d2c27|말줄임|장마다 헤드|한발서기|중년 남성|참고사진/.test(body);
  var hasDesign = /\[디자인\]/.test(body) && /#f0efeb|#eee9de/.test(body) && /#3d2c27|#6b493d/.test(body);
  // 구팔레트(#eddcd2·테라코타)만 있으면 새 색으로 교체
  if(hasDesign && hasLayout && !/#eddcd2|#c4785a|#b07a62/i.test(body)) return body;
  var stripped = body
    .replace(/\n*\[디자인\][\s\S]*?(?=\n*\[레이아웃\]|$)/i, '')
    .replace(/\n*\[레이아웃\][\s\S]*$/i, '')
    .replace(/\n*흑[·.\s]*백[·.\s]*미니멀[^\n]*/g, '')
    .replace(/\n*색:\s*배경\s*#(eddcd2|f0efeb|eee9de)[^\n]*/gi, '')
    .trim();
  return clampMangoIntro_((stripped || body.replace(/\n*\[디자인\][\s\S]*$/i, '').trim()) + '\n\n' + designBlock);
}

/** 망고보드: 소개란에 한 번에 붙인 뒤, 첫 줄(제목)만 잘라 제품명란으로 옮기기용 */
function formatMangoTitleAndIntroPaste_(title, intro){
  var t = shortenMangoProductName_(title) || clampMangoTitle_(title);
  var body = ensureMangoIntroHasDesignBlock_(intro);
  if(t && body) return t + '\n\n' + body;
  return t || body || '';
}
...
function readMangoTitleAndIntroFromSheet_(){
  ...
  if(!intro) intro = formatMangoBriefPaste_(brief);
  title = shortenMangoProductName_(title) || clampMangoTitle_(title);
  intro = ensureMangoIntroHasDesignBlock_(intro);
  // 화면에 디자인 블록이 없으면 반영
  ...
  return { title: title, intro: intro, paste: formatMangoTitleAndIntroPaste_(title, intro) };
}

function emptyMangoBrief_(){
  return {
    productName: '',
    intro: '',
    features: [],
...
function trimMangoSection_(s, maxLen){
  ...
  var punct = Math.max(cut.lastIndexOf('.'), cut.lastIndexOf('。'), cut.lastIndexOf('요'), cut.lastIndexOf('다'));
  if(punct >= Math.floor(maxLen * 0.45)) return cut.slice(0, punct + 1).trim();
  if(sp >= Math.floor(maxLen * 0.5)) return cut.slice(0, sp).trim();
  return cut.trim();
}

/** 산문에서 앞쪽 문장만 짧게 (이미지 헤드·불릿용) */
...
function mangoExtractSteps_(selfCare, maxSteps){
  ...
  for(var i = 0; i < steps.length && out.length < maxSteps; i++){
    var line = trimMangoSection_(mangoStripStepPrefix_(steps[i]), 40);
...
function buildMangoBriefFromBlog_(content, topic){
  ...
  var coverHead = trimMangoSection_(title || topic || hook || '유연한데 왜 자꾸 삐끗할까요?', 28);
  var problemSents = mangoPickSentences_(problem, 3, 36);
  ...
  var whySents = mangoPickSentences_(explanation, 3, 36);
  ...
  var ctaSents = mangoPickSentences_(cta.replace(/https?:\/\/\S+/g, '').replace(/\?hub=\w+/g, ''), 2, 36);
  ...
  var blocks = [
    ...
    mangoFormatSlideBlock_(6, '주의', '이땐 따라 하지 마세요', cautionBullets, '주의 안내 카드(#6b493d 포인트)'),
...
function fillMangoBriefFromContent_(content, opts){
  opts = opts || {};
  if(!content || !content.images) return content;
  if(isHeiljagyaeCategory(opts.catId != null ? opts.catId : state.selectedCatId)) return content;
  var brief = content.images.mangoBrief;
  if(!brief || typeof brief !== 'object'){
    brief = emptyMangoBrief_();
    content.images.mangoBrief = brief;
  } else {
    brief = normalizeMangoBrief_(brief) || emptyMangoBrief_();
    content.images.mangoBrief = brief;
  }
  ...
  if(needTitle || needIntro){
    var built = buildMangoBriefFromBlog_(content, topic);
  ...
  // 복사·망고보드용: 소개 끝에 #f0efeb·#eee9de 팔레트 디자인 블록 보장
  brief.intro = ensureMangoIntroHasDesignBlock_(brief.intro || brief.mangoInputPaste || '');
  brief.mangoInputPaste = brief.intro;
  brief.designTone = MANGO_DESIGN_TONE_LINE;
  if(!brief.photoTip) brief.photoTip = emptyMangoBrief_().photoTip;
...
function renderMangoDetailInputCard_(brief){
  brief = brief || emptyMangoBrief_();
  var titleVal = formatMangoTitlePaste_(brief);
  var introVal = formatMangoBriefPaste_(brief);
  var html = '';
  html += '<div class="img-section-title">망고보드 AI 상세페이지 입력</div>';
  html += '<p style="font-size:12px;color:#6B7280;margin:0 0 10px;line-height:1.55;">주황 버튼으로 <strong>제목+소개</strong> 복사 → 망고보드 소개란에 붙여넣고 첫 줄만 제품명으로. 소개는 <strong>장별 헤드+짧은 불릿</strong>(블로그 원문·… 붙여넣기 X). 결과물: 배경 <strong>#f0efeb·#eee9de</strong>·헤드·구분선 <strong>#3d2c27</strong>·포인트 <strong>#6b493d</strong>·카드 <strong>#f7efe9·#f5ebe4</strong>(흑백 모노 X), 주의·안내형, 여성 모델·일상 전/후, 픽토그램 OK.</p>';
...
function applySheetImageEdits_(content){
  ...
  if(titleEl || briefEl){
    ...
    if(briefEl){
      var intro = clampMangoIntro_(briefEl.value);
...
async function generateMangoBriefFromBlog_(catId, blog, topic, draft){
  ...
  var prompt =
    ...
    MANGO_DESIGN_TONE_LINE.replace(/"/g, '\\"') + '"}}\n\n' +
    '[블로그 원문]\n' + buildBlogSourceText_(blog, catId);
  ...
  if(!brief || isWeakMangoIntro_(brief.intro || brief.mangoInputPaste)){
    return buildMangoBriefFromBlog_({ blog: blog }, topic);
  }
  if(brief.productName) brief.productName = shortenMangoProductName_(brief.productName);
  brief.intro = ensureMangoIntroHasDesignBlock_(brief.intro || brief.mangoInputPaste || '');
  brief.mangoInputPaste = brief.intro;
...
function getImagePromptTextForData_(content, catId){
  ...
  var briefText = formatMangoBriefPaste_(im.mangoBrief);
