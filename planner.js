...
function getMangoLayoutPasteBlock_(catId){
  ...
  return '표·긴 설명표 금지. 주의·안내형·짧은 STEP·픽토그램 OK.\n' +
    ...
    '인물=여성 실사 우선. 중년 남성 반복 지양. 전/후 환영. 추상 한발서기만 반복 금지.\n' +
...
- 인물 사진이 필요하면 **여성 모델·실사** 우선(일상·클리닉/홈). **중년 남성 반복 지양**.
...
- 참고사진: 여성·전신·일상/시연 동작 컷 (잘린 제품샷·글자 캡처 금지).`;
}
...
5) photoTip: 참고 사진은 **여성·실사·일상/시연 동작** 컷 권장 (잘린 제품샷·글자 캡처 금지).
...
- intro **맨 끝**에 [디자인]+[레이아웃] (배경 ${p.bg}·${p.bg2}, 헤드·구분선 ${p.head}, 포인트 ${p.point}, 카드 ${p.card}·${p.card2}, 지정 외 색·흑백 모노 금지, 표 금지, 쉬운 말, 여성 모델, 한발서기만 X·일상 전/후)
- photoTip: 실사·시연 (가능하면 여성) 1~2문장
...
function ensureMangoIntroHasDesignBlock_(intro, catId){
  ...
  if(hasMatchingDesign && hasFreshLayout && !/#eddcd2|#c4785a|#b07a62/i.test(body)){
...
function emptyMangoBrief_(catId){
  return {
    ...
    photoTip: '여성·전신·일상/시연 동작 컷. 중년 남성·한발서기만 반복 컷 지양. 잘린 제품샷·글자 캡처 금지.',
    mangoInputPaste: ''
  };
}
...
function buildMangoBriefFromBlog_(content, topic, catId){
  ...
  var blocks = [
    mangoFormatSlideBlock_(1, '표지', coverHead, [], '큰 타이포 한 줄 + 여성 실사(선택)'),
    ...
    mangoFormatSlideBlock_(4, '따라하기', '지금 범위에서 이렇게', steps.map(function(s, i){ return 'STEP' + (i + 1) + ' ' + s; }), '픽토그램 STEP 또는 여성 시연'),
...
function fillMangoBriefFromContent_(content, opts){
  ...
  if(!brief.photoTip) brief.photoTip = emptyMangoBrief_(catId).photoTip;
...
async function generateMangoBriefFromBlog_(catId, blog, topic, draft){
  ...
  var prompt =
    ...
    'JSON만:\n' +
    '{"mangoBrief":{"productName":"최대30자","intro":"【1. 표지】…7장+[디자인]+[레이아웃]","mangoInputPaste":"intro와 동일","photoTip":"여성·전신·일상 컷","slidePlan":[{"slide":1,"role":"표지","copyHint":"..."}],"designTone":"' +
    tone.replace(/"/g, '\\"') + '"}}\n\n' +
    '[블로그 원문]\n' + buildBlogSourceText_(blog, catId);
