...
const SK = 'ht_v2';
/** 재생성 직전 초안 스냅샷 (기기 localStorage, 초안당 최대 2개) */
...
function renderSheetContent(content) {
  ...
  var undoBtn = (undoPeek && !undoBusy)
    ? '<button type="button" class="btn-undo" onclick="undoLastRegen_()" title="' +
      escapeHtml((undoPeek.label ? undoPeek.label + ' · ' : '') + '재생성 직전으로 되돌리기') +
      '">되돌리기</button>'
    : '';
  var redoBtn = (redoPeek && !undoBusy)
    ? '<button type="button" class="btn-redo" onclick="redoLastRegen_()" title="' +
      escapeHtml((redoPeek.label ? redoPeek.label + ' · ' : '') + '재생성본으로 앞돌리기') +
      '">앞돌리기</button>'
    : '';
  setSheetActionsHtml_(
...
function enqueueInstaFromBlog_(draftId, catId, topic, blog, opts){
  ...
  if(!state.apiKey) return;
  if(opts.saveUndo) captureRegenUndo_(draftId, { kind: 'block', label: '인스타 캡션' });
...
function enqueueThreadsFromBlog_(draftId, catId, topic, blog, opts){
  ...
  if(!state.apiKey) return;
  if(opts.saveUndo) captureRegenUndo_(draftId, { kind: 'block', label: '쓰레드 글' });
  var startedMs = Date.now();
...
function peekRegenUndo_(draftId){
  if(!draftId) return null;
  var bucket = getRegenHistoryBucket_(readRegenUndoStore_(), draftId);
  return bucket.undo[0] || null;
}
function peekRegenRedo_(draftId){
  if(!draftId) return null;
  var bucket = getRegenHistoryBucket_(readRegenUndoStore_(), draftId);
  return bucket.redo[0] || null;
}
function hasRegenUndo_(draftId){
  return !!peekRegenUndo_(draftId);
}
function hasRegenRedo_(draftId){
  return !!peekRegenRedo_(draftId);
}
...
/** 재생성으로 덮어쓰기 직전 호출 — 초안 전체 스냅샷을 최대 2개까지 보관 */
  ...
  if(current){
    bucket.undo = pushRegenStackEntry_(bucket.undo, {
      ...
      label: (target && target.label)
        ? String(target.label).replace(/\s·\s재생성본$/, '') || '재생성 직전'
        : '재생성 직전',
      content: current
    });
  }
...
var prevDraftContent = getDraftContent_(jobDraftId);
if(prevDraftContent){
  captureRegenUndo_(jobDraftId, {
    kind: opts.userInitiated ? 'draft' : 'draft-overwrite',
    label: opts.userInitiated ? '초안 전체' : '초안'
  });
}
...
window.genContent = async function(ev){
  ...
  if(isBlogInstaCategory(catId) && tab === 'insta'){
    ...
    enqueueInstaFromBlog_(draftId, catId, draft ? draft.topic : '', igContent.blog, { saveUndo: true });
  ...
  if(isBlogInstaCategory(catId) && tab === 'threads'){
    ...
    enqueueThreadsFromBlog_(draftId, catId, draftTh ? draftTh.topic : '', thBlogContent.blog, { force: true, saveUndo: true });
