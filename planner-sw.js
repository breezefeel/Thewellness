...
var PLANNER_CACHE = 'planner-static-v31';
var PLANNER_ASSETS = [
  './planner.css',
  './planner.js'
];
...
function isPlannerStaticRequest(url){
  ...
  return /\/planner\.(css|js)$/.test(path);
}

self.addEventListener('install', function(event){
  event.waitUntil(
    caches.open(PLANNER_CACHE).then(function(cache){
      return cache.addAll(PLANNER_ASSETS).catch(function(err){
...
self.addEventListener('activate', function(event){
  event.waitUntil(
    caches.keys().then(function(keys){
      return Promise.all(keys.filter(function(k){ return k !== PLANNER_CACHE; }).map(function(k){
