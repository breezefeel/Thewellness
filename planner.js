...
망고보드 「상세페이지」 화면 입력란에 그대로 넣는 값만 작성합니다.
1) productName (=「제품명 혹은 주제」): **최대 ${MANGO_DETAIL_TITLE_MAX}자**. 블로그 제목을 짧게. 이모지·과장 금지.
2) intro (=「소개」): **최대 ${MANGO_DETAIL_INTRO_MAX}자**. 이 필드만 상세페이지 본문 프롬프트로 쓰입니다.
3) 결과물은 보통 **7장** 상세페이지이므로, intro 안에 7장 흐름이 드러나게 씁니다.
4) photoTip: 참고 사진 안내 1~2문장 (서비스·시연 장면 / 동작이 잘린 사진 금지).

intro 권장 구성 (합계 ${MANGO_DETAIL_INTRO_MAX}자 이내, 번호·불릿 OK):
1. 표지·한 줄 훅
2. 문제 공감 (blog.problem 요약)
3. 누구를 위한 내용인지
4. 셀프 케어 동작·초·회·분 (blog.selfCare 핵심)
5. 원리·왜 도움이 되는지 (blog.explanation TL;DR)
6. 주의·병원 권고가 필요한 경우 한 줄
7. FAQ 1~2 + CTA(상담·프로필, 과장 예약 압박 금지)
디자인 톤 한 줄: 흑·백·미니멀·전문가, 만화·코믹·알록달록 금지. 로고는 후처리.

mangoInputPaste = intro와 **동일** (소개란에 붙여넣기용). 2000자 초과 금지.`;
...
mangoBrief (망고보드 상세페이지 UI에 맞춤):
- productName: 「제품명 혹은 주제」 **최대 ${MANGO_DETAIL_TITLE_MAX}자**
- intro / mangoInputPaste: 「소개」 **최대 ${MANGO_DETAIL_INTRO_MAX}자**, 동일 내용. **7장** 흐름이 보이게
- photoTip: 참고 사진(최소 1장) — 서비스·시연 장면, ② selfCare 전신 컷 권장
- slidePlan: 정확히 7개
- designTone: 흑·백·미니멀·전문가
...
function formatMangoBriefPaste_(brief){
  ...
  if(brief.features && brief.features.length){
    lines.push('[특장점]');
    brief.features.forEach(function(f){ lines.push('- ' + f); });
  }
  if(brief.slidePlan && brief.slidePlan.length){
    lines.push('', '[7장 구성]');
    brief.slidePlan.forEach(function(s){
      if(!s) return;
      lines.push((s.slide || '') + '. ' + (s.role || '') + (s.copyHint ? ' — ' + s.copyHint : ''));
    });
  }
  if(brief.designTone) lines.push('', '[디자인]', brief.designTone);
  return clampMangoIntro_(lines.join('\n').trim());
}
...
function ensureMangoBriefOnContent_(content){
  if(!content || !content.images) return content;
  if(!content.images.mangoBrief){
    content.images.mangoBrief = emptyMangoBrief_();
  } else {
    content.images.mangoBrief = normalizeMangoBrief_(content.images.mangoBrief) || emptyMangoBrief_();
  }
  return content;
}
...
반드시 images.gptVisuals는 **정확히 3개**(problem·selfCare·explanation). mangoBrief도 채울 것. ②번 동작은 blog.selfCare와 일치. 사진에 글자·로고 금지.`;
...
반드시 images.gptVisuals는 **정확히 3개**. mangoBrief 포함. ②번은 blog.outline·draft의 시연·테크닉과 일치. 사진에 글자·로고 금지.`;
} else {
