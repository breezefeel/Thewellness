...
async function runPlannerBootstrapSync_(){
  ...
  var watchdog = setTimeout(function(){
    ...
    setPlannerBootstrapUi_('loading', '데이터가 많아 정리에 시간이 걸리고 있어요. 잠시 더 기다리거나 「이 기기 데이터로 열기」를 누르세요.');
  }, 8000);
  setPlannerBootstrapUi_('loading', '서버에 연결하는 중…');
  try {
    var bootstrapData = await verifyPlannerServerReachable_();
    await yieldPlannerBootstrapUi_('서버 응답을 받았습니다. 이 기기 데이터와 맞추는 중…');
    var reconcileMode = 'noop';
    if(bootstrapData.result === 'success' && bootstrapData.payload){
      var localPayload = getPersistPayload();
      await yieldPlannerBootstrapUi_('서버·이 기기 데이터를 병합하는 중… (초안이 많으면 수십 초 걸릴 수 있어요)');
      ...
      await yieldPlannerBootstrapUi_('병합 결과를 저장하는 중…');
    ...
    var contFail = document.getElementById('sync-bootstrap-continue-local');
    if(contFail) contFail.style.display = 'block';
  } finally {
...
window.continuePlannerWithLocalData_ = function(){
  try {
    ...
    setPlannerBootstrapUi_('done', '');
    setTimeout(function(){
      try { runDeferredHeavyPlanRepairs_(); } catch(eDef2){}
    }, 50);
    if(typeof setAppToast === 'function'){
      setAppToast('서버 동기화를 건너뛰고 이 기기 데이터로 열었어요.\n나중에 「지금 동기화」로 다시 맞출 수 있어요.', { duration: 6500, variant: 'ok' });
    }
    try { renderMain(); renderTabs(); } catch(eUi){}
  } finally {
    updateSyncStatusUI_();
  }
};
function openPlannerConflict_(local, remote, serverRevision){
  ...
  // 전체 payload deep clone은 초안 수백 건에서 UI를 멈춤 — 참조만 보관, 충돌 항목만 복제
  plannerSyncConflictPending_ = {
    ...
    conflicts: (conflicts || []).map(function(c){
      return {
        key: c.key,
        local: cloneSyncValue_(c.local),
        remote: cloneSyncValue_(c.remote),
        localAt: c.localAt || '',
        remoteAt: c.remoteAt || ''
      };
    })
...
function renderPlannerConflictUi_(pending){
  ...
  // 부트스트랩 오버레이가 남아 있으면 충돌 UI 클릭이 가려질 수 있음
  var boot = document.getElementById('sync-bootstrap-overlay');
  if(boot) boot.classList.remove('open');
...
window.applyPlannerConflictChoices_ = async function(){
  ...
  try {
    ...
    if(typeof setAppToast === 'function'){
      setAppToast('충돌 적용 중 오류가 났어요.\n' + ((err && err.message) || err), { duration: 7000, variant: 'err' });
    }
  } finally {
